...
CREATE PROCEDURE sp_rpt_extract_walmart_disposal_finance_dc (
    @start_date             datetime,
    @end_date               datetime,
...
AS
/-* ***********************************************************
Procedure    : sp_rpt_extract_walmart_disposal_finance_dc
Database     : PLT_AI
Created      : Jan 25 2008 - Jonathan Broome
Description  : Creates a Wal-Mart Disposal Extract

Examples:
    sp_rpt_extract_walmart_disposal_finance_dc '1/1/2009 00:00', '1/31/2009 23:59', 'validation'
    sp_rpt_extract_walmart_disposal_finance_dc '3/1/2009 00:00', '3/30/2009 23:59', 'eq-extract'
    sp_rpt_extract_walmart_disposal_finance_dc '1/1/2010 00:00', '1/31/2010 23:59', 'wm-extract'
    sp_rpt_extract_walmart_disposal_finance_dc '8/1/2010', '8/31/2010', 'eq-extract'
...
History:
	...
	5/6/2011 - JPB
		1. sp_rpt_extract_walmart_disposal_finance_dc created from sp_rpt_extract_walmart_disposal
	...
	6/1/2011 - JPB
		...
		2. Nevermind, Brie said NOT to do this on DC's.
...
TRUNCATE TABLE EQ_TEMP.dbo.WalmartDisposalExtract -- where added_by = @usr
TRUNCATE TABLE EQ_TEMP.dbo.WalmartDisposalReceiptTransporter -- where added_by = @usr
TRUNCATE TABLE EQ_TEMP.dbo.WalmartExtractImages -- where added_by = @usr
TRUNCATE TABLE EQ_TEMP.dbo.WalmartDisposalValidation -- where added_by = @usr
TRUNCATE TABLE EQ_TEMP.dbo.WalmartMissingGenerators -- where added_by = @usr
...
-- Workorder Waste Codes (1)
	...
	select distinct
		...
		xwc.waste_code
	FROM TSDFApprovalWasteCode xwc 
		INNER JOIN WasteCode wc ON xwc.waste_code = wc.waste_code
		INNER JOIN EQ_TEMP.dbo.WalmartDisposalExtract e
...
SELECT
	...
	e.manifest,
	e.manifest_line,
    R.line_weight as pounds, -- was R.net_weight, 2/25/2011
	...
	@usr as added_by,
	@extract_datetime as date_added
FROM EQ_TEMP.dbo.WalmartDisposalExtract e
...
INSERT EQ_Extract..WMDisposalGeneration
  SELECT distinct
	  ...
	  ELSE
			...
			, 1) -- 12/16/2010 - rounding to 0 decimals??
	  END as Weight,
			...
			-- 	approvals that do not contain a RCRA hazardous waste code (D, F, K, P, U)
			  ...
			  ELSE
			  	CASE WHEN NOT EXISTS (
			  		...
			  		FROM TSDFApprovalWasteCode tawc 
			  		INNER JOIN WasteCode wc on tawc.waste_code = wc.waste_code
...
* /
	  ...
	  @usr as added_by,
	  @extract_datetime as date_added
  from EQ_TEMP.dbo.WalmartDisposalExtract f
  -- new on 12/14:
  inner join tsdfapproval tsdfa on f.tsdf_approval_id = tsdfa.tsdf_approval_id
  inner join tsdf on tsdfa.tsdf_code = tsdf.tsdf_code and tsdf.eq_flag = 'F'
  inner join workorderheader woh       on f.receipt_id = woh.workorder_id
			...
			-- 	approvals that do not contain a RCRA hazardous waste code (D, F, K, P, U)
			  ...
			  ELSE
			  	CASE WHEN NOT EXISTS (
			  		...
			  		INNER JOIN WasteCode wc on pwc.waste_code = wc.waste_code
...
* /
  ...
  (
	SELECT DISTINCT 
      ...
      added_by,
      date_added
	FROM EQ_TEMP.dbo.WalmartDisposalExtract 
	  WHERE 1=1
	  and added_by = @usr
	  and date_added = @extract_datetime
  ...
  left outer join ReceiptDetailItem rdi
...
-- Catch generators serviced that aren't in the extracts
	...
	INNER join TripHeader th ON woh.trip_id = th.trip_id
	INNER JOIN generator g on woh.generator_id = g.generator_id
	...
	AND (woh.customer_id IN (select customer_id from #Customer) OR woh.generator_id in (select generator_id from CustomerGenerator where customer_id IN (select customer_id from #Customer)))
	AND coalesce(wos.date_act_arrive, woh.start_date) between @start_date and @end_date
	AND g.generator_id not in (select generator_id from EQ_TEMP.dbo.WalmartDisposalExtract where submitted_flag = 'T')
...
-- Finance version shouldn't have to compare disposal vs generation, if the Disposal extract did that, right?
 ...
 -- Step 1: Create a temp table that contains totals per line to compare against each other.
		 SELECT
			receipt_weight = (
				SELECT sum(pounds) FROM EQ_TEMP.dbo.WalmartDisposalExtract d2
			...
			wodi_weight = (
				SELECT sum(weight) FROM EQ_Extract.dbo.WMDisposalGeneration t2
				WHERE t2.workorder_id = t.workorder_id
				AND t2.company_id = t.company_id
				AND t2.profit_ctr_id = t.profit_ctr_id
				AND t2.resource_type = t.resource_type
				AND t2.sequence_id = t.sequence_id
				and t2.added_by = t.added_by
				and t2.date_added = t.date_added
				and t2.exclude_flag = t.exclude_flag
			),
			wodi_round_weight = (
				SELECT sum(weight) FROM EQ_Extract.dbo.WMDisposalGeneration t2
				WHERE t2.workorder_id = t.workorder_id
		...
		FROM EQ_TEMP.dbo.WalmartDisposalExtract d
		LEFT OUTER JOIN EQ_Extract.dbo.WMDisposalGeneration T
		...
		AND EXISTS (
			...
			inner join profilewastecode pwc ON pqa.profile_id = pwc.profile_id
			inner join wastecode wc on pwc.waste_code = wc.waste_code
			WHERE 
...
from Billing b
	...
	LEFT OUTER JOIN ResourceClassDetail rcd
...
WHERE 
	b.status_code = 'I'
	AND EXISTS (
	SELECT 1 FROM EQ_TEMP.dbo.WalmartDisposalExtract t
...
FROM
#FinanceInfo f
INNER JOIN EQ_Temp..WalmartDisposalExtract t
...
SELECT
	...
	, convert(float, null) as total_generation_rcra_pounds
	, convert(float, null) as total_generation_nonrcra_pounds
	, convert(float, null) as avg_generation_pounds
	, convert(float, null) as avg_generation_rcra_pounds
	, convert(float, null) as avg_generation_nonrcra_pounds
	...
	, convert(float, null) AS avg_manifest_weight
...
WHILE EXISTS (select 1 FROM #CombineInfo WHERE avg_invoice IS null) BEGIN
	...
	UPDATE #CombineInfo SET
		...
		, total_generation_rcra_pounds = (
			SELECT sum(pounds)
			FROM EQ_Temp..WalmartDisposalExtract f
			WHERE site_type = @site_type
			and datepart(yyyy, service_date) = @year
			and datepart(m, service_date) = @month
			AND (
				(
					f.source_table = 'Workorder'
					AND EXISTS (
			  			SELECT tawc.waste_code
			  			FROM TSDFApprovalWasteCode tawc 
			  			INNER JOIN WasteCode wc on tawc.waste_code = wc.waste_code
		...
		, total_generation_nonrcra_pounds = (
			SELECT sum(pounds)
			FROM EQ_Temp..WalmartDisposalExtract f
			WHERE site_type = @site_type
			and datepart(yyyy, service_date) = @year
		...
		, avg_manifest_weight = (
			select CASE WHEN round((sum(wde.pounds) / count(distinct wde.generator_id)),1) BETWEEN 0.0001 AND 1.0 THEN 1.0 ELSE round((sum(wde.pounds) / count(distinct wde.generator_id)),1) END
			FROM EQ_Temp..WalmartDisposalExtract wde
			WHERE site_type = @site_type
			and datepart(yyyy, service_date) = @year
			and datepart(m, service_date) = @month
		)
...
END
	...
	UPDATE #CombineInfo SET
		...
		avg_generation_nonrcra_pounds =
		  CASE WHEN round(total_generation_nonrcra_pounds / store_count,1) BETWEEN 0.0001 AND 1.0 THEN 1.0 ELSE round(total_generation_nonrcra_pounds / store_count,1) END,
		avg_generation_rcra_pounds =
		  CASE WHEN round(total_generation_rcra_pounds / store_count,1) BETWEEN 0.0001 AND 1.0 THEN 1.0 ELSE round(total_generation_rcra_pounds / store_count,1) END,
		avg_invoice = round( grand_total_amt / invoice_count, 2)
	...
	SELECT period_year, period_month, 'Total',
		...
		sum(total_generation_rcra_pounds),
		sum(total_generation_nonrcra_pounds),
		sum(avg_generation_pounds),
		sum(avg_generation_rcra_pounds),
		sum(avg_generation_nonrcra_pounds),
		sum(avg_disposal_amt),
		sum(avg_transportation_amt),
		sum(avg_other_amt),
		sum(avg_invoice),
		sum(avg_manifest_weight),
		sum(store_count),
		sum(invoice_count)
	FROM #CombineInfo
...
CREATE TABLE #outRows (
	...
	INSERT #outRows
		...
		SELECT 'total_generation_rcra_pounds',6, 0 union
		SELECT 'total_generation_nonrcra_pounds',7, 0 union
		SELECT 'avg_generation_pounds',8, 0 union
		SELECT 'avg_generation_rcra_pounds',9, 0 union
		SELECT 'avg_generation_nonrcra_pounds',10, 0 union
		SELECT 'avg_disposal_amt',11, 0 union
		SELECT 'avg_transportation_amt',12, 0 union
		SELECT 'avg_other_amt',13, 0 union
		SELECT 'avg_invoice',14, 0 union
		SELECT 'avg_manifest_weight', 15, 0 union
		SELECT 'store_count',16, 0 union
		SELECT 'invoice_count', 17, 0
...
-- SELECT DISTINCT site_type FROM #CombineInfo
	INSERT #outCols
		SELECT 'SAMS_DC', 'Sams DC', 1, 0 union
		SELECT 'WM_DC', 'Wal-Mart DC', 2, 0 union
		SELECT 'WM_PM_DC', 'Wal-Mart PMDC', 3, 0 union
		SELECT 'Grand_Total', 'Total', 4, 0
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_extract_walmart_disposal_finance_dc] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_extract_walmart_disposal_finance_dc] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_extract_walmart_disposal_finance_dc] TO [EQAI]
    AS [dbo];

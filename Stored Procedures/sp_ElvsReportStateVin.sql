CREATE PROCEDURE sp_ElvsReportStateVin (						
...
AS						
--======================================================
-- Description: Report on Elvs Data: By State, THEN by Valid Vin
-- Parameters :
-- Returns    :
-- Requires   : *.PLT_AI.*
--
...
BEGIN
	...
	SELECT distinct						
			s.state_name,				
			Count(d.container_id) AS vins,				
			Sum(IsNull(d.abs_switches, 0) + IsNull(d.light_switches,0)) AS switches				
	FROM						
		ElvsContainer c					
...
END -- CREATE PROCEDURE sp_ElvsReportStateVin

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_ElvsReportStateVin] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_ElvsReportStateVin] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_ElvsReportStateVin] TO [EQAI]
    AS [dbo];

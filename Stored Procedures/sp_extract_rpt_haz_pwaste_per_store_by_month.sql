...
CREATE PROCEDURE sp_extract_rpt_haz_pwaste_per_store_by_month
...
/*
	This report displays total *federal* & *haz* pounds for each month/year and generator site for 'P waste' (pharmaceutical waste)
	and whether or not his exceeds a pre-determined threshold.
	
	Revision:
	12/11/2009	RJG	Created
*/
...
IF @start < 1 SET @start = 1
  ...
  SET @ubound = @start + @maxct
	...
	SET @main_sql = 'SELECT 
				    ...
				    * FROM view_extract_rpt_haz_pwaste_per_store_by_month         
			WHERE 1=1
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_haz_pwaste_per_store_by_month] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_haz_pwaste_per_store_by_month] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_haz_pwaste_per_store_by_month] TO [EQAI]
    AS [dbo];

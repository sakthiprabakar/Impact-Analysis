CREATE PROCEDURE sp_rpt_steers_export_report
	@company_id		int
,	@profit_ctr_id 	int
,	@date_from		datetime
,	@date_to		datetime
...
09/23/2018 PRK  DevOps:10591  Modified output to first go to a temp table, then summed for duplicate lines of matching state waste code, EPA codes and weight unit per TCEQ spec

sp_rpt_steers_detail_report 46, 0,  '3/1/2018', '5/20/2018'
sp_rpt_steers_export_report 46, 0,  '5/1/2018', '9/1/2018'
sp_rpt_steers_detail_report 46, 0,  '5/1/2018', '9/1/2018'
sp_rpt_steers_detail_report 55, 0,  '7/1/2019', '8/1/2019'
sp_rpt_steers_export_report 55, 0,  '7/1/2019', '8/1/2019'
...
SELECT 
    ...
    --'"' + RIGHT(CONVERT(VARCHAR(10),r.receipt_date, 105) , 7) +'"' as 'report_period',  --how is this determined?
    '"' + RIGHT('0' + CAST(MONTH(r.receipt_date) AS NVARCHAR(2)), 2) + '/'  + RIGHT(CAST(YEAR(r.receipt_date) AS NVARCHAR(4)), 2)  +'"' as 'report_period',  --how is this determined?    
    '""' as 'no_shipments_received',			--does anything appear here?
	...
	--	'"' + CONVERT (varchar(30), dbo.fn_receipt_weight_line (r.receipt_id,r.line_id,r.profit_ctr_id,r.company_id) ) + '"'  as inv_wt,  --Need to change this to round to 3 digits
	CAST (dbo.fn_receipt_weight_line (r.receipt_id,r.line_id,r.profit_ctr_id,r.company_id) AS decimal (18,3)) as inv_wt,
	'"P"' as wt_unit,
    case when 
    ...
    else  '"' +  '"' end as system_type_code, 
	--'"' + CONVERT(VARCHAR(15),r.receipt_date , 110) + '"' as date_shipped,
	'"' + RIGHT('0' + CAST(MONTH(r.receipt_date) AS NVARCHAR(2)), 2) + '/' + RIGHT('0' + CAST(DAY(r.receipt_date) AS NVARCHAR(2)), 2) + '/' + RIGHT(CAST(YEAR(r.receipt_date) AS NVARCHAR(4)), 2)  +'"' as date_shipped,
	...
	'"' + SUBSTRING(LTRIM(RTRIM(ISNULL(tr.transporter_EPA_ID,''))), 1, 12)+ '"'  as trans_epa_id,
	'"B"' as 'rpt_method',	
	'"A"' as 'action_code',
...
select 
	site_state_id, report_type, for_internal_use, report_period, no_shipments_received, state_manifest, generator_state_id, generator_epa_id, state_code, epa_code1, epa_code2, epa_code3, epa_code4, 
	'"' + CONVERT (varchar(30), sum(inv_wt) ) + '"'  as inv_wt,  --insert into the temp table was without the formatting, this adds the export formatting
	wt_unit, system_type_code, date_shipped, trans_state_id, trans_epa_id, rpt_method, action_code, industrial_flag
from #temp_steers
group by 
	site_state_id, report_type, for_internal_use, report_period, no_shipments_received, state_manifest, generator_state_id, generator_epa_id, state_code, epa_code1, epa_code2, epa_code3, epa_code4, --inv_wt, 
	wt_unit, system_type_code, date_shipped, trans_state_id, trans_epa_id, rpt_method, action_code, industrial_flag

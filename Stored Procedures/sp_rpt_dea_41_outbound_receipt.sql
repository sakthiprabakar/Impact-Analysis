...
create procedure sp_rpt_dea_41_outbound_receipt  
 @company_id int,  
 @profit_ctr_id int,  
 @receipt_id int  
as
/*************************************************************************************************
PB Object : d_rpt_receipt_dea_41_destroyed

12/09/2015 rb Created
7/18/2024 Prakash US118193 Modified the logic for merchandise_quantity and partial_count columns.   
*************************************************************************************************/
...
--return results  
select left(rdi.merchandise_code,5) + '-' + substring(rdi.merchandise_code,6,4) + '-' + right(rdi.merchandise_code,2) as merchandise_code,  
 'N/A' as batch_number,  
 ...
 --'N/A' as partial_count,
 case when isnull(dbo.fn_GetNumeric(isnull(m.package_size, '')), 0) = isnull(dbo.fn_GetNumeric(rdi.contents), 0)
	then isnull(rdi.merchandise_quantity, 0) else 0 end as merchandise_quantity,
 case when isnull(dbo.fn_GetNumeric(isnull(m.package_size, '')), 0) <> isnull(dbo.fn_GetNumeric(rdi.contents), 0)
	then isnull(rdi.merchandise_quantity, 0) * isnull(dbo.fn_GetNumeric(rdi.contents), 0) else 0 end as partial_count,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_dea_41_outbound_receipt] TO [EQAI]
    AS [dbo];

...
-- drop proc if exists sp_packback_certificate_of_recycling
go

CREATE PROCEDURE sp_packback_certificate_of_recycling (
	@order_id int
) AS
/***************************************************************
sp_packback_certificate_of_recycling

	Copy & Mod of sp_retail_email_boxes_recycled
	...
	************** TEST CASES *****************
	...
	exec sp_packback_certificate_of_recycling 1191


	SELECT  * FROM    Message WHERE  subject like 'US Ecology Certificate Of Recycling%'
		and html like '%Verified Weight%'
	
**********************/	


-- declare @order_id int = 1191

select
	isnull(convert(varchar(50), sum(oi.quantity_returned)), 'N/A') as quantity_returned,
	...
	oi.line_id,
	oi.sequence_id,
	Cast( oi.order_id as varChar(20)) + '-' + cast(oi.line_id as varchar(20)) + '-' + cast(oi.sequence_id as varchar(20)) as formatted_order_id,
...
where
	...
	and oi.outbound_receipt_id is not null
	and exists (
		select s.image_id 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].sp_packback_certificate_of_recycling TO [EQAI]
    AS [dbo];

GO
GRANT EXECUTE
    ON OBJECT::[dbo].sp_packback_certificate_of_recycling TO [COR_USER]
    AS [dbo];

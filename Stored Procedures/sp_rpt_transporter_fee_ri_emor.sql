CREATE PROCEDURE sp_rpt_transporter_fee_ri_emor 
	@date_from			datetime
,	@date_to			datetime
...
Electronic Monthly Operating Report (EMOR) - Rhode Island

Filename:		L:\Apps\SQL\EQAI\sp_rpt_transporter_fee_ri_emor.sql
PB Object(s):	r_transporter_fee_ri_emor
		
03/09/2005 JDB	Created (copied from MA EMOR)
...
08/10/2006 RG   Revised or changes in tsdfapproval and profile
11/12/2007 RG   Revised for workorder status.  Submitted workorders are now status of A
                and submitted_flag of T
10/28/2008 JDB	Modified to meet new MA/RI DEP requirements (Gemini 8976)
...
01/06/2010 JDB	Updated to get treatment_id from ProfileQuoteApproval instead of Profile table.
				Actually made the changes for submitted_flag that RG said were made on 11/12/07.
07/16/2010 KAM	Updated the SQL to not include voided workorderdetail rows	
07/16/2010 KAM  Updated the SQL to get the transporter from the new TransporterStateLicense Table	
...
03/29/2012 JDB	Added "AND pqd.resource_class_code IN ('FEERIHW', 'FEERI')" to the join to ProfileQuoteDetail so that this report would
...
sp_rpt_transporter_fee_ri_emor '9/1/2013', '9/30/2013', 1, 999999, '0', 'zzz', 'MAD084814136', 'C'
sp_rpt_transporter_fee_ri_emor '3/1/2017', '4/30/2017', 1, 999999, '0', 'zzz', 'MAD084814136', 'S'
***********************************************************************/
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_transporter_fee_ri_emor] TO [EQAI]
    AS [dbo];

...
CREATE PROCEDURE sp_extract_rpt_haz_waste_per_store_by_month
...
/*
	This report displays total *federal* & *haz* pounds for each month/year and generator site
...
IF @start < 1 SET @start = 1
  ...
  SET @ubound = @start + @maxct
	...
	SET @main_sql = 'SELECT
				    ...
				    * FROM view_extract_rpt_haz_waste_per_store_by_month
			WHERE 1=1
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_haz_waste_per_store_by_month] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_haz_waste_per_store_by_month] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_haz_waste_per_store_by_month] TO [EQAI]
    AS [dbo];

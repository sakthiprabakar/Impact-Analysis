CREATE  PROCEDURE [dbo].[sp_Validate_GeneratorKnowledge_Form]
...
/* ******************************************************************
	...
	Object Name		: [sp_Validate_GeneratorKnowledge_Form]
...
Samples:
 EXEC [sp_Validate_GeneratorKnowledge_Form] @form_id,@revision_id
 EXEC [sp_Validate_GeneratorKnowledge_Form] 569059, 1
...
BEGIN
	...
	declare @specific_gravity float, 
		...
		from FormGeneratorKnowledge where form_id = @form_id and revision_id = @revision_id
		...
		IF(NOT EXISTS(SELECT 1 FROM FormSectionStatus WHERE form_id = @form_id AND revision_id = @revision_id AND section = 'GK'))
		BEGIN
		  INSERT INTO FormSectionStatus VALUES (@form_id,@revision_id,'GK','C',getdate(),@web_userid,getdate(),@web_userid,1)
		END
		ELSE 
		BEGIN
		   UPDATE FormSectionStatus SET section_status = @completed_status,date_modified=getdate(),modified_by=@web_userid WHERE form_id = @form_id AND revision_id = @revision_id AND section = 'GK'
		END
...
GO
	...
	GRANT EXEC ON [dbo].[sp_Validate_GeneratorKnowledge_Form] TO COR_USER;

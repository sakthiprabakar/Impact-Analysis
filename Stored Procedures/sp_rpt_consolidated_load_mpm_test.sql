...
create procedure sp_rpt_consolidated_load_mpm_test
    @trip_id int,
...
begin
    ...
    begin
        ...
        if @AESOP_waste_stream is not null and @AESOP_waste_stream > '' and @AESOP_profile_id is not null and @AESOP_profile_id > 0
			set @approval_list = @approval_list + ' (' + convert(varchar(5), @AESOP_profile_id) + '-' + @AESOP_waste_stream + ')'
    ...
    -- add rows to report
    insert #results
    select distinct consolidated_container_id, consolidated_dot_shipping_desc, @approval_list, @description_list, container_size, container_type, 1
    from #work
    where consolidated_container_id = @ccid
...
select @tsdf_name as tsdf_name,
        ...
        r.consolidated_dot_shipping_desc,
        r.approval_list,
        r.description_list,
        r.container_size,
        r.container_type,
        r.count_rollup,
        round(sum(isnull(wdu.quantity,0) * (wdc.percentage / 100.0)),2)
from #results r
join WorkOrderHeader wh
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_consolidated_load_mpm_test] TO [EQAI]
    AS [dbo];

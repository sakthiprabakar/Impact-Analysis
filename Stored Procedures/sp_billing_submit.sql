DROP PROCEDURE IF EXISTS [dbo].[sp_billing_submit]
GO

CREATE OR ALTER PROCEDURE [dbo].[sp_billing_submit]
	@debug				int,
	@company_id			int,
	@profit_ctr_id		int,
	@trans_source		char(1),
...
Filename:		L:\Apps\SQL-Deploy\Prod\NTSQL1\PLT_AI\Procedures\sp_billing_submit.sql
Loads to:		Plt_AI
...
05/29/2016  AM  Added AX fields to #BillingDetail. Added logic to write into audit when insert into billingdetail.
05/25/2017 MPM	Replaced the COMPUTE clause with SQL Server 2014-compatible code.
05/31/2017 MPM	Added logic for customer billing discount, if applicable.
06/20/2017 MPM	Corrected calculations for customer billing discount.
02/15/2018 MPM	Added currency_code column to #Billing and #BillingDetail.
04/03/2018 AM   GEM:49391 - Added profitcenter logic to AX_Dimension_2.  
11/10/2019 AM   DevOps:12961 - Modified fnValidateFinancialDimension function for 3d65 call (AX_Dimension_5_part_1 = '' and AX_Dimension_5_part_2 = '')
01/29/2020 AM   DevOPs:13953 - Submit Procedure separating the dimension 5 project code incorrectly
12/14/2020 MPM	DevOps 18137 - Modified to populate Billing.service_desc_2 with ProfileQuoteDetail.service_desc for receipt disposal lines, if 
				service_desc is populated and if ProfileQuoteDetail.print_desc_on_invoice_flag = 'T'.
01/22/2021 MPM	DevOps 18939 - Corrected join to ProfileQuoteDetail.
06/17/2022 MPM	DevOps 41806 - Add a "Completed" record to the WorkOrderTracking table when a work order is submitted.
11/30/2022 AGC  DevOps 58319 - Add ERF/FRF fees
07/10/2023 MPM	DevOps 61059 - Modified so that workorder lines that has “Inv. Print?” checked (WOD.print_on_invoice_flag = 'T') appear on the invoice 
				even if WOH.fixed_price_flag = ‘T’.
08/21/2023 MPM	DevOps 71840 - Reverted the DevOps 61059 changes.
03/11/2024 Subhrajyoti DEVOPS# 78372 - 'CT' & 'NY' Sales Tax - Exempt from Taxes/Regulated Fees
03/29/2024 AGC  DevOps 78372 - EEC Fee should be charged sales tax
07/04/2024 KS	Rally US116983 & Rally US116984 - Modified #billing table's columns service_desc_1 and service_desc_2 to be VARCHAR(100).
				Modified service_desc_1 and service_desc_2 values to be upto 100 chars.
10/24/2024 Prakash Rally US126675 - Commented out the code refrencing ResourceClassGLAccount Table.
10/25/2024 Sailaja Rally DE35804 - EQAI Sales Tax Calculation - Included Billing table and modified the Join to 
									include billing.workorder_sequence_id and workorderdetail.sequence_id
10/28/2024	KS	Rally US126814 - Work Order Management > 'Fixed Price' Logic: Added logic to pull D365 account string
				from Company, ProfitCenter, ResourceClassHeader and ResourceClassDetail for resources if we have values
				defined.
11/25/2024 KS	Rally US127045 - Billing Adjustment > Receipt Unsubmission from Billing Adjustment Screen
				Added logic to populate invoice_id, invoice_code, invoice_date in Billing table for Receipt, if Receipt was unsubmitted from Adjustment screen with
				Remove from Invoice checkbox unchecked.
11/26/2024 Sailaja Rally #US127391 - EQAI Tracking feature addition in Inbound Receipt Screen - 11/26/2024
12/06/2024 KS	Rally US131329 - Updated the logic for Receipts to update BillingComment.date_added, BillingComment.date_modified, BillingComment.added_by,
				BillingComment.modified_by.
01/13/2025 UY	Rally US133512 - Improvements to 'sp_billing_submit'
...
sp_billing_submit 1, 27, 0, 'W', 83700, '10-15-10 14:25', NULL, 'JASON_B', '1'
sp_billing_submit 1, 14, 6, 'W', 4912700, '6/24/13 14:02', NULL, 'SMITA_K', ''
...
--EXEC sp_billing_submit 1, 3, 0, 'R', 1213591, '02-17-12 14:45', NULL, 'JASON_B', ''
EXEC sp_billing_submit 1, 3, 0, 'R', 1213591, '02-17-12 14:45', NULL, 'JASON_B', '63'

EXEC sp_billing_unsubmit 1, 3, 0, 1213616, 'R', 'JASON_B'
--EXEC sp_billing_submit 1, 3, 0, 'R', 1213616, '02-17-12 16:50', NULL, 'JASON_B', ''
EXEC sp_billing_submit 1, 3, 0, 'R', 1213616, '02-17-12 16:50', NULL, 'JASON_B', '63'

EXEC sp_billing_unsubmit 1, 21, 0, 1164500, 'W', 'JASON_B'
EXEC sp_billing_submit 1, 21, 0, 'W', 1164500, '02-20-12 16:00', NULL, 'JASON_B', ''

EXEC sp_billing_unsubmit 1, 25, 0, 50151, 'R', 'JASON_B'
EXEC sp_billing_submit 1, 25, 0, 'R', 50151, '02-22-12 11:00', NULL, 'JASON_B', ''

EXEC sp_billing_unsubmit 1, 21, 0, 919114, 'R', 'JASON_B'
EXEC sp_billing_submit 1, 21, 0, 'R', 919114, '05-31-13 8:33', NULL, 'JASON_B', ''

EXEC sp_billing_submit 1, 14, 9, 'W', 3439900, '04-4-14 8:39', NULL, 'JASON_B', ''

EXEC sp_billing_submit 1, 14, 9, 'W', 3439900, '04-4-14 8:39', NULL, 'JASON_B', ''

EXEC sp_billing_submit 0,14,6,'W',9129100, '5/26/2016 16:23:37', 'H', 'ANITHA_M', ''

EXEC sp_billing_submit 1,14,0,'W',21448900, '6/09/2016', 'H', 'ANITHA_M', '' 
EXEC sp_billing_submit 1,14,0,'W',2205100, '6/21/2016', 'H', 'ANITHA_M', ''  
EXEC sp_billing_submit 1,14,4,'W',15812001, '7/27/2016', 'H', 'ANITHA_M', '' 

EXEC sp_billing_submit 1,21,0,'W',1984300, '11/4/2016', 'H', 'ANITHA_M', ''  

EXEC sp_billing_unsubmit 1, 21, 0, 1177596, 'R', 'MARTHA_M'
EXEC sp_billing_submit 1, 21, 0, 'R', 1177596, '6/5/2017', NULL, 'MARTHA_M', ''
...
DECLARE	@discount_flag	char(1),
	...
	@d365_flag			char(1) ,
	@max_tracking_id	int = 0,
	@business_minutes	int,
	@min_time_in		datetime,
	@tracking_contact	varchar(10),
	--Devops # 78372
	@customer_id int, 
	@generator_id int,
	@cust_tax_flag char(1),
	@gen_tax_flag Char(1),
	@trans_mode	Char(1),
	@billingcomment_customer_id INT
			
BEGIN TRANSACTION SubmitToBilling

SELECT @discount_flag = ISNULL(discount_flag,'F') 
FROM profitcenter 
...
CREATE TABLE #Billing  (
	...
	ERF_percent				money NULL,
	FRF_percent				money NULL,
	ERF_extended_amt		money NULL,
	FRF_extended_amt		money NULL
)

-- Prepare Line item from #Billing records.
DROP TABLE IF EXISTS #tmp_line_id;
CREATE TABLE #tmp_line_id (
	company_id				smallint NOT NULL,
	profit_ctr_id			smallint NOT NULL,
	receipt_id				int NULL,
	resource_sort			int	NULL,
...
BEGIN
 ...
 IF @sync_invoice_jde = 1 
  BEGIN 
	INSERT INTO ReceiptAudit(company_id, 
		profit_ctr_id, 
		receipt_id, 
		line_id, 
		price_id, 
	...
	SELECT	r.company_id, 
		...
		'Receipt', 
		'gl_account_code', 
		r.gl_account_code, dbo.fn_get_receipt_glaccount(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id), 
		'Updated from sp_billing_submit (from Treatment)', 
		@user_code, 
		'BS', 
		@submit_date
	FROM Receipt r
	...
	UPDATE Receipt 
	SET gl_account_code = dbo.fn_get_receipt_glaccount(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id)
	FROM Receipt r
	JOIN Treatment t ON t.company_ID = r.company_id
		AND t.profit_ctr_ID = r.profit_ctr_id
		AND t.treatment_id = r.treatment_id
	...
	SELECT	r.company_id, 
		...
		'gl_account_code', 
		r.gl_account_code, 
		dbo.fn_get_receipt_glaccount(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id), 
		'Updated from sp_billing_submit (from Product)', 
		@user_code, 
		'BS', 
		@submit_date
	FROM Receipt r
 ...
 -- 	INSERT INTO ReceiptAudit
	--SELECT r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id, 0, 'Receipt', 'gl_account_code', r.gl_account_code, 
 --    ( convert ( varchar(32) ,dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'MAIN')+
 --      dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM1')+ 
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM2')+ 
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM3')+
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM4')+
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM6')+
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM5')+ 
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DI52') )),
	-- 'Updated from sp_billing_submit (from Treatment)', @user_code, 'BS', @submit_date
	--FROM Receipt r
	--JOIN Treatment t ON t.company_ID = r.company_id
	--	AND t.profit_ctr_ID = r.profit_ctr_id
	--	AND t.treatment_id = r.treatment_id
	...
	--UPDATE Receipt 
	--SET gl_account_code = convert ( varchar(32), 
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'MAIN')+ 
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM1')+
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM2')+  
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM3')+ 
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM4')+  
 ...
 --      dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM1')+ 
	...
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM5')+ 
	--   dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DI52') ) ),
	--   'Updated from sp_billing_submit (from Product)', @user_code, 'BS', @submit_date
	--FROM Receipt r
	--WHERE r.company_id = @company_id
	--AND r.profit_ctr_id = @profit_ctr_id
	--AND r.receipt_id = @receipt_id
	--AND r.trans_mode = 'I'
	--AND r.trans_type IN ('S')
	--AND r.gl_account_code <> (convert ( varchar(32),dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'MAIN')+
	--						 dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM1')+
	--						 dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM2')+
	--						 dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM3')+
	--						 dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id, r.line_id,'DIM4')+
	...
	--UPDATE Receipt SET gl_account_code = convert ( varchar(32), 
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'MAIN')+ 
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM1')+
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM2')+  
	--  dbo.fn_get_receipt_AX_gl_account(r.company_id, r.profit_ctr_id, r.receipt_id,r.line_id,'DIM3')+ 
  ...
  --END
	...
	AND r.profit_ctr_id = @profit_ctr_id
	AND r.receipt_id = @receipt_id
	IF @debug = 1 print 'Receipt:' 
				+ ' customer_id: ' + Convert(varchar(15), ISNULL(@customer_id, 0))

	-- IF a BillingComment record already exists for this receipt, populate
	-- All the Billing records with the Invoice information.  It means that
	-- This receipt has already been invoiced and is being resubmitted because
	-- of an adjustment
	SELECT @invoice_id = invoice_id, 
		@invoice_code = invoice_code, 
		@invoice_date = invoice_date,
		@billingcomment_customer_id = customer_id
	FROM BillingComment
	...
	IF @customer_id <> @billingcomment_customer_id AND @billingcomment_customer_id IS NOT NULL
	BEGIN
	...
	SELECT	receipt.company_id,
		...
		CASE WHEN receipt.billing_link_id > 0 THEN 'H'
			WHEN @billing_status IS NOT NULL THEN @billing_status 
			WHEN ISNULL(receipt.submit_on_hold_flag,'F') = 'T' THEN 'H' 
			ELSE 'S'  			   
		END AS status_code,
...
-- SK 02/20		--CASE WHEN receiptPrice.sr_type = 'E' THEN ''
		...
		CASE WHEN receipt.trans_type = 'S' THEN ISNULL(REPLACE(receipt.service_desc,'''', ''),'') 
		     ELSE SUBSTRING(ISNULL(REPLACE(receipt.approval_code,'''', ''),'') + ' ' + ISNULL(REPLACE(Profile.approval_desc,'''', ''),''), 1, 100) 
		     END AS service_desc_1,
		CASE WHEN IsNull(ProfileQuoteDetail.print_desc_on_invoice_flag, 'F') = 'T' AND Len(ProfileQuoteDetail.service_desc) > 0 THEN ProfileQuoteDetail.service_desc ELSE '' END AS service_desc_2,
		0 AS cost,
		...
		0 AS ERF_percent,
		0 AS FRF_percent,
		0 AS ERF_extended_amt,
		0 AS FRF_extended_amt
	FROM Receipt
	...
	LEFT OUTER JOIN ProfileQuoteDetail   
		ON ProfileQuoteDetail.company_id = Receipt.company_id 
		AND ProfileQuoteDetail.profit_ctr_id = Receipt.profit_ctr_id
		AND ProfileQuoteDetail.profile_id = Receipt.profile_id
		AND Receipt.trans_type = 'D' 
		AND ProfileQuoteDetail.bill_unit_code = ReceiptPrice.bill_unit_code
		AND (ProfileQuoteDetail.ref_sequence_id = 0 OR ProfileQuoteDetail.ref_sequence_id IS NULL)
		AND (ProfileQuoteDetail.bill_method <> 'B' OR ProfileQuoteDetail.bill_method IS NULL) 
	...
	AND (receipt.submitted_flag = 'F' OR receipt.submitted_flag IS NULL)
	AND ((receipt.optional_flag = 'F' OR receipt.optional_flag IS NULL) OR receipt.optional_flag = 'T' AND receipt.apply_charge_flag = 'T')
	...
	IF @debug = 1 print 'Selecting billing receipt records:'
	IF @debug = 1 SELECT * FROM #Billing
	
	IF @receipt_count > 0
	BEGIN
		-- Get Billing Comments 
		INSERT INTO #BillingComment(company_id, 
			profit_ctr_id, 
			trans_source, 
			receipt_id, 
			receipt_status, 
		...
		LEFT OUTER JOIN receiptComment
			...
			AND NOT ( (receiptComment.invoice_comment_1 = '' OR receiptComment.invoice_comment_1 IS NULL)
				AND  (receiptComment.invoice_comment_2 = '' OR receiptComment.invoice_comment_2 IS NULL)
				AND  (receiptComment.invoice_comment_3 = '' OR receiptComment.invoice_comment_3 IS NULL)
				AND  (receiptComment.invoice_comment_4 = '' OR receiptComment.invoice_comment_4 IS NULL)
				AND  (receiptComment.invoice_comment_5 = '' OR receiptComment.invoice_comment_5 IS NULL) )
	...
	-- Add a "Completed" tracking record to the ReceiptTracking table.
	-- If there is at least one row already in ReceiptTracking, then also update the time_out 
	-- and business_minutes of the latest tracking record.
	...
	IF @trans_mode = 'I' 
	Begin
		SELECT @max_tracking_id = ISNULL(MAX(tracking_id), 0)
		FROM ReceiptTracking
		WHERE company_id = @company_id
		AND profit_ctr_id = @profit_ctr_id
		AND receipt_id = @receipt_id

		IF @debug = 1 print '@max_tracking_id:'
		IF @debug = 1 print @max_tracking_id
		
		IF @max_tracking_id > 0
		BEGIN
			IF @debug = 1 print 'Receipt has at least 1 tracking record'

			-- Get min time in for later update of WorkOrderHeader tracking days and tracking business days
			...
			IF @debug = 1 print '@min_time_in:'
			IF @debug = 1 print @min_time_in

			-- Get tracking contact for later insert into WorkOrderAudit
			SELECT @tracking_contact = tracking_contact
			FROM Receipt
			WHERE company_id = @company_id
			AND profit_ctr_id = @profit_ctr_id
			AND receipt_id = @receipt_id
			...
			IF @debug = 1 print '@tracking_contact:'
			IF @debug = 1 print @tracking_contact
			...
			AND profit_ctr_id = @profit_ctr_id
			AND receipt_id = @receipt_id	
			AND tracking_id = @max_tracking_id
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error updating ReceiptTracking.'
				GOTO END_OF_PROC
			END

			-- Write audit record for the update of ReceiptTracking.business_minutes
			INSERT ReceiptAudit (company_id, 
				profit_ctr_id, 
				receipt_id, 
				line_id, 
				price_id, 
			...
			VALUES (@company_id, 
				...
				'business_minutes', 
				'(blank)', 
				CAST(@business_minutes AS VARCHAR(255)),
				'tracking_id = ' + CAST(@max_tracking_id AS VARCHAR(255)) + ' updated', 
				@user_code, 
				'SB', 
				@submit_date)
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error inserting ReceiptAudit record for update of ReceiptTracking.business_minutes.'
				GOTO END_OF_PROC
			END
			...
			VALUES (@company_id, 
				...
				'time_out', 
				'(blank)', 
				CAST(@submit_date AS VARCHAR(255)),
				'tracking_id = ' + CAST(@max_tracking_id AS VARCHAR(255)) + ' updated', 
				@user_code, 
				'SB', 
				@submit_date)
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error inserting ReceiptAudit record for update of ReceiptTracking.time_out.'
				GOTO END_OF_PROC
			END
		END

		INSERT INTO ReceiptTracking (company_id, 
			profit_ctr_id, 
			receipt_id, 
			tracking_id, 
			tracking_status, 
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error inserting ReceiptTracking record.'
			GOTO END_OF_PROC
		END
		...
		VALUES (@company_id, 
			...
			'(no record)', 
			'(new record added)',
			'''Completed'' tracking record added', 
			@user_code, 
			'SB', 
			@submit_date)
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error inserting ReceiptAudit record for insert into ReceiptTracking.'
			GOTO END_OF_PROC
		END
	END
...
BEGIN
	...
	BEGIN
		...
		SELECT 
			...
			1 AS line_id,
			1 AS price_id,
			CASE WHEN WorkorderHeader.billing_link_id > 0 THEN 'H'
				WHEN @billing_status IS NOT NULL THEN @billing_status 
				WHEN WorkorderHeader.submit_on_hold_flag = 'T' THEN 'H' 
				ELSE 'S'
			END AS status_code,
...
--					+ WorkOrderTypeHeader.gl_seg_4,
			...
			CASE WHEN WorkorderHeader.billing_link_id IS NOT NULL AND WorkorderHeader.billing_link_id = 0 AND WorkorderHeader.submit_on_hold_reason IS NULL
				...
				THEN 'WorkorderHeader is member of Billing Link ' + Convert(varchar(10), WorkorderHeader.billing_link_id)
			     WHEN WorkorderHeader.submit_on_hold_flag = 'T' AND WorkorderHeader.submit_on_hold_reason IS NULL
				THEN 'Submitted on Hold with no supporting reason.'
			     WHEN WorkorderHeader.submit_on_hold_flag = 'T' AND WorkorderHeader.submit_on_hold_reason IS NOT NULL
				THEN WorkorderHeader.submit_on_hold_reason
			     WHEN @submit_status IS NOT NULL
				THEN 'Submitted on Hold with no supporting reason.'
			END AS hold_reason,
			CASE WHEN WorkorderHeader.billing_link_id > 0 OR @submit_status IS NOT NULL OR WorkorderHeader.submit_on_hold_flag = 'T'
				THEN @user_code
				ELSE NULL
			END AS hold_userid,
			CASE WHEN WorkorderHeader.billing_link_id > 0 OR @submit_status IS NOT NULL OR WorkorderHeader.submit_on_hold_flag = 'T'
			...
			0.00 AS ERF_percent,
			0.00 AS FRF_percent,
			0.00 AS ERF_extended_amt,
			0.00 AS FRF_extended_amt
		FROM WorkOrderHeader
		...
		WHERE WorkorderHeader.fixed_price_flag = 'T'
		AND WorkorderHeader.total_price > 0
		AND WorkorderHeader.company_id = @company_id
  ...
  --      IF @debug = 1 print 'Selecting fixed price billing workorder records:'
	 --   IF @debug = 1 SELECT * FROM #Billing
		-----------------------------------------------------------------------------
		-- New insert added 10/12/10 for new BillingDetail table
		-----------------------------------------------------------------------------
		;WITH CteWorkOrderTypeDetail AS ( 
		SELECT DISTINCT 
				workorder_type_id
				, company_id
				, profit_ctr_id
		...
		INSERT #BillingDetail(billing_uid,
		...
		SELECT 1 AS billing_uid,
			...
			AX_MainAccount = dbo.fn_get_workorder_AX_main_account(WorkOrderHeader.workorder_type_id,@company_id, @profit_ctr_id,'O', WorkOrderHeader.workorder_ID),
			...
			AX_Dimension_2 = (CASE WHEN ProfitCenter.wo_bu_configuration_flag = 'T' THEN 
                            COALESCE( CASE WHEN CteWorkOrderTypeDetail.AX_Dimension_2 =  '' THEN ProfitCenter.AX_Dimension_2 
                                    ELSE   CteWorkOrderTypeDetail.AX_Dimension_2 end, CteWorkOrderTypeDetail.AX_Dimension_2) 
                            else ProfitCenter.AX_Dimension_2 end),	
			AX_Dimension_3 = CteWorkOrderTypeDetail.AX_Dimension_3,
			AX_Dimension_4 = dbo.fn_get_workorder_AX_dim4_LOB(@company_id, @profit_ctr_id,@receipt_id),
			AX_Dimension_5_part_1 = ISNull( case when dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id) <> '' 
                        then substring(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id), 1, 
                        case when charindex('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
						charindex('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1)=0
                        then len(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id)) 
                        else charindex('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
						charindex('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1)-1 
                        end) end  , '' ),
                        --  else 'X' end,
			AX_Dimension_5_part_2 = ISNULL ( case when charindex('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
			charindex('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1) = 0
                                    then null 
                                    else substring(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id), 
                                    charindex('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
									charindex('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1)+1
                                                                , len(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id)))
                                                    end, '' ),
			AX_Dimension_6 = CteWorkOrderTypeDetail.AX_Dimension_6, --NULLIF(WorkOrderTypeDetail.AX_Dimension_6,' '),
			AX_Project_Required_Flag = CteWorkOrderTypeDetail.AX_Project_Required_Flag,
			NULL AS disc_amount,
			WorkOrderHeader.currency_code
		FROM WorkorderHeader
		JOIN ProfitCenter ON ProfitCenter.profit_ctr_id = WorkorderHeader.profit_ctr_id
		...
		LEFT OUTER JOIN CustomerBilling ON WorkorderHeader.customer_id = CustomerBilling.customer_id
			AND WorkorderHeader.billing_project_id = CustomerBilling.billing_project_id
		LEFT OUTER JOIN Generator ON WorkorderHeader.generator_id = Generator.generator_id
		LEFT OUTER JOIN CteWorkOrderTypeDetail ON CteWorkOrderTypeDetail.workorder_type_id = WorkOrderHeader.workorder_type_id 
...
--			'EQWO' AS waste_code,
		...
		JOIN WorkorderDetail ON WorkorderHeader.company_id = WorkorderDetail.company_id
			...
			AND WorkorderDetail.bill_rate >= 0
			AND NOT(WorkorderDetail.resource_type IN ('E','L','S') AND (RTRIM(WorkorderDetail.group_code) <> '' AND WorkorderDetail.group_code IS NOT NULL))
		JOIN WorkOrderTypeHeader ON WorkOrderHeader.workorder_type_id = WorkOrderTypeHeader.workorder_type_id
...
--			'EQWO' AS waste_code,
		...
		JOIN WorkOrderDetailUnit ON WorkorderDetail.company_id = WorkOrderDetailUnit.company_id
			...
			AND ((WorkOrderDetailUnit.extended_price > 0 AND WorkorderDetail.bill_rate > 0)
				OR (WorkOrderDetailUnit.extended_price = 0 AND WorkorderDetail.print_on_invoice_flag = 'T' AND WorkorderDetail.bill_rate >= 0))
		JOIN WorkOrderTypeHeader ON WorkOrderHeader.workorder_type_id = WorkOrderTypeHeader.workorder_type_id
		JOIN WorkorderResourceType ON WorkOrderDetail.resource_type = WorkOrderResourceType.resource_type
		LEFT OUTER JOIN CustomerBilling ON WorkorderHeader.customer_id = CustomerBilling.customer_id
			AND WorkorderHeader.billing_project_id = CustomerBilling.billing_project_id
		...
		INSERT INTO #tmp_line_id(company_id,
			profit_ctr_id,
			receipt_id,
			resource_sort,
			bill_sequence,
		...
		UPDATE #Billing 
		SET	line_id = #tmp_line_id.line_id
		FROM #Billing b
		...
		SELECT	b.billing_uid,
				...
				AX_MainAccount = CASE WHEN dbo.fn_get_resource_AX_main_account(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL
										THEN dbo.fn_get_workorder_AX_main_account(WorkOrderHeader.workorder_type_id,@company_id, @profit_ctr_id,b.workorder_resource_type, WorkOrderHeader.workorder_ID)
										ELSE dbo.fn_get_resource_AX_main_account(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))
									END,
				AX_Dimension_1 = CASE WHEN dbo.fn_get_resource_AX_dim1(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL
										THEN ProfitCenter.AX_Dimension_1
										ELSE dbo.fn_get_resource_AX_dim1(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))
									END,
				--AX_Dimension_2 = case when ProfitCenter.wo_bu_configuration_flag = 'T' 
							--		then COALESCE(NULLIF(WorkOrderTypeDetail.AX_Dimension_2, ''), WorkOrderTypeDetail.AX_Dimension_2) 
							--		else ProfitCenter.AX_Dimension_2 end ,	
				AX_Dimension_2 = CASE WHEN dbo.fn_get_resource_AX_dim2(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL
									THEN (CASE WHEN ProfitCenter.wo_bu_configuration_flag = 'T' 
												THEN COALESCE(CASE WHEN CteWorkOrderTypeDetail.AX_Dimension_2 =  '' 
																	THEN ProfitCenter.AX_Dimension_2 
																	ELSE CteWorkOrderTypeDetail.AX_Dimension_2 
																END, CteWorkOrderTypeDetail.AX_Dimension_2) 
												ELSE ProfitCenter.AX_Dimension_2
											END)
									ELSE dbo.fn_get_resource_AX_dim2(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))
								END,
				AX_Dimension_3 = CASE WHEN dbo.fn_get_resource_AX_dim3(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL
										THEN CteWorkOrderTypeDetail.AX_Dimension_3
										ELSE dbo.fn_get_resource_AX_dim3(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))
									END,
				AX_Dimension_4 = CASE WHEN dbo.fn_get_resource_AX_dim4(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL
										THEN dbo.fn_get_workorder_AX_dim4_LOB(@company_id, @profit_ctr_id,@receipt_id)
										ELSE dbo.fn_get_resource_AX_dim4(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))
								END,		
				AX_Dimension_5_part_1 = CASE WHEN dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL						   
											THEN (ISNULL(CASE WHEN dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id) <> '' 
																THEN SUBSTRING(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id), 1, 
																CASE 
																WHEN CHARINDEX('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
																	CHARINDEX('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1)=0
																	THEN LEN(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id)) 
																	ELSE CHARINDEX('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
																	CHARINDEX('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1)-1 
																END) 
															END, ''))
											ELSE (ISNULL(CASE WHEN dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) <> '' 
																THEN SUBSTRING(dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')), 1, 
																CASE 
																WHEN CHARINDEX('.', dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')),
																	CHARINDEX('.', dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')))+1)=0
																	THEN LEN(dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))) 
																	ELSE CHARINDEX('.', dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')),
																	CHARINDEX('.',dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')))+1)-1 
																END) 
															END, ''))
										END,
										--  else 'X' end,
				AX_Dimension_5_part_2 = CASE WHEN dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')) IS NULL						   
												THEN (ISNULL (CASE WHEN CHARINDEX('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
																	CHARINDEX('.',dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id))+1) = 0
																THEN NULL 
																ELSE SUBSTRING(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id), 
																	CHARINDEX('.', dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id),
																	...
																	, LEN(dbo.fn_get_workorder_AX_dim5_project (@company_id, @profit_ctr_id,WorkorderHeader.workorder_id)))
														END, ''))
												ELSE (ISNULL (CASE WHEN CHARINDEX('.', dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')),
																	CHARINDEX('.',dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')))+1) = 0
																THEN NULL 
																ELSE SUBSTRING(dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')), 
																	CHARINDEX('.', dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')),
																	CHARINDEX('.',dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F')))+1)+1
																	, LEN(dbo.fn_get_resource_AX_dim5_project(@company_id, @profit_ctr_id, WorkorderDetail.resource_type, WorkorderDetail.resource_class_code, WorkorderDetail.bill_unit_code, ISNULL(WorkorderHeader.fixed_price_flag, 'F'))))
														END, ''))
										END,
				AX_Dimension_6 = CteWorkOrderTypeDetail.AX_Dimension_6, --ISNULL(NULLIF(WorkOrderTypeDetail.AX_Dimension_6,' '), NULL),
				AX_Project_Required_Flag = CteWorkOrderTypeDetail.AX_Project_Required_Flag,
				NULL AS disc_amount,
				b.currency_code
		FROM WorkorderHeader
		...
		LEFT OUTER JOIN WorkOrderResourceType ON WorkOrderResourceType.resource_type = WorkorderDetail.resource_type 
	...
	END
		--IF @debug = 1 print 'Selecting #BillingDetail records:'
		--IF @debug = 1 SELECT * FROM #BillingDetail
	...
	-- DevOps 41806 - Add a "Completed" tracking record to the WorkOrderTracking table.
	-- If there is at least one row already in WorkOrderTracking, then also update the time_out 
	-- and business_minutes of the latest tracking record.
	SELECT @max_tracking_id = ISNULL(MAX(tracking_id), 0)
	...
	IF @max_tracking_id > 0
	BEGIN
		IF @debug = 1 print 'WO has at least 1 tracking record'
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error updating WorkOrderTracking.'
			GOTO END_OF_PROC
		END

		-- Write audit record for the update of WorkOrderTracking.business_minutes
		INSERT WorkOrderAudit (company_id, 
			profit_ctr_id, 
			workorder_id, 
			resource_type, 
			sequence_id, 
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error inserting WorkOrderAudit record for update of WorkOrderTracking.business_minutes.'
			GOTO END_OF_PROC
		END
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error inserting WorkOrderAudit record for update of WorkOrderTracking.time_out.'
			GOTO END_OF_PROC
		END
	END
	...
	BEGIN		
		-- Set message for RAISEERROR and go to the end
		SET @error_msg = 'Error inserting WorkOrderTracking record.'
		GOTO END_OF_PROC
	END
	...
	BEGIN		
		-- Set message for RAISEERROR and go to the end
		SET @error_msg = 'Error inserting WorkOrderAudit record for insert into WorkOrderTracking.'
		GOTO END_OF_PROC
	END
...
--------------------------------------------------------------------------------------------------------------------
-- Apply customer billing discount, if applicable, to #Billing and #BillingDetail tables
--------------------------------------------------------------------------------------------------------------------

-- For receipts:

UPDATE #BillingDetail
	SET extended_amt = Round(extended_amt * (100 - dbo.fn_get_customer_billing_discount_percent(#Billing.billing_date,
                                                                                         #Billing.customer_id,
                                                                                         #Billing.billing_project_id)) / 100, 2),
       applied_percent = - dbo.fn_get_customer_billing_discount_percent(#Billing.billing_date,
                                                                        #Billing.customer_id,
                                                                        #Billing.billing_project_id),
       disc_amount = Round(extended_amt * dbo.fn_get_customer_billing_discount_percent(#Billing.billing_date,
                                                                                         #Billing.customer_id,
                                                                                         #Billing.billing_project_id) / 100, 2)
FROM #Billing
WHERE #BillingDetail.billing_uid = #Billing.billing_uid
AND #BillingDetail.trans_source = 'R'
AND ((#BillingDetail.billing_type IN ('Disposal', 'Wash') AND #BillingDetail.ref_billingdetail_uid IS NULL)	
	OR (#BillingDetail.billing_type IN ('Product') AND #BillingDetail.ref_billingdetail_uid IS NOT NULL))		
 
UPDATE #Billing
	SET discount_percent = dbo.fn_get_customer_billing_discount_percent(#Billing.billing_date,
                                                                       #Billing.customer_id,
                                                                       #Billing.billing_project_id)
FROM #BillingDetail bd
WHERE bd.billing_uid = #Billing.billing_uid
AND bd.trans_source = 'R'
AND ((bd.billing_type IN ('Disposal', 'Wash') AND bd.ref_billingdetail_uid IS NULL)	
	OR (bd.billing_type IN ('Product') AND bd.ref_billingdetail_uid IS NOT NULL))		

-- Update for disposal lines
UPDATE #Billing
	SET waste_extended_amt = (SELECT SUM(extended_amt) 
							   FROM #BillingDetail bd 
							  WHERE bd.billing_uid = #Billing.billing_uid 
							    AND bd.trans_source = 'R'
							    AND ((bd.billing_type IN ('Disposal', 'Wash') AND bd.ref_billingdetail_uid IS NULL)	
                                 OR (bd.billing_type IN ('Product') AND bd.ref_billingdetail_uid IS NOT NULL))),
	   total_extended_amt = (SELECT SUM(extended_amt) 
							   FROM #BillingDetail bd 
							  WHERE bd.billing_uid = #Billing.billing_uid
							    AND bd.trans_source = 'R')
FROM #BillingDetail bd
...
-- Update for product service lines
UPDATE #Billing
	SET waste_extended_amt = (SELECT SUM(extended_amt) 
							   FROM #BillingDetail bd 
							  WHERE bd.billing_uid = #Billing.billing_uid 
							    AND bd.trans_source = 'R'
							    AND bd.billing_type IN ('Product')),
	   total_extended_amt = (SELECT SUM(extended_amt) 
							   FROM #BillingDetail bd 
							  WHERE bd.billing_uid = #Billing.billing_uid
							    AND bd.trans_source = 'R')
...
WHERE bd.billing_uid = #Billing.billing_uid
AND bd.billing_type IN ('Product')
AND #billing.trans_type = 'S'      

-- For work orders:
 
UPDATE #BillingDetail
	SET extended_amt = Round(extended_amt * (100 - dbo.fn_get_customer_billing_discount_percent(b.billing_date,
                                                                                         b.customer_id,
                                                                                         b.billing_project_id)) / 100, 2),
       applied_percent = - dbo.fn_get_customer_billing_discount_percent(b.billing_date,
                                                                        b.customer_id,
                                                                        b.billing_project_id),
       disc_amount = Round(extended_amt * dbo.fn_get_customer_billing_discount_percent(b.billing_date,
                                                                                         b.customer_id,
                                                                                         b.billing_project_id) / 100, 2)
FROM #Billing b
INNER JOIN WorkOrderDetail wod ON wod.company_id = b.company_id
	...
	AND wod.resource_type = 'D'					-- only applies to disposal lines
INNER JOIN TSDF t
	ON t.TSDF_code = wod.TSDF_code
	AND t.use_flag = 'T'						-- only applies to disposal at USE facilities
WHERE #BillingDetail.billing_uid = b.billing_uid
AND #BillingDetail.trans_source = 'W'
AND #BillingDetail.billing_type IN ('WorkOrder') 

UPDATE #Billing
	SET discount_percent = dbo.fn_get_customer_billing_discount_percent(b.billing_date,
                                                                       b.customer_id,
                                                                       b.billing_project_id)
FROM #BillingDetail bd
...
UPDATE #Billing 
	SET ERF_extended_amt = (SELECT SUM(extended_amt) 
							FROM #BillingDetail bd
							WHERE bd.billing_uid = #Billing.billing_uid	
							AND bd.billing_type = 'ERF'),
		ERF_percent = (SELECT MAX(applied_percent)
						FROM #BillingDetail bd 
						WHERE bd.billing_uid = #Billing.billing_uid 
						AND bd.billing_type = 'ERF')

UPDATE #Billing 
	SET FRF_extended_amt = (SELECT SUM(extended_amt) 
							FROM #BillingDetail bd
							WHERE bd.billing_uid = #Billing.billing_uid	
							AND bd.billing_type = 'FRF'),
		FRF_percent = (SELECT MAX(applied_percent)
						FROM #BillingDetail bd 
						WHERE bd.billing_uid = #Billing.billing_uid 
						AND bd.billing_type = 'FRF')

-- This is for AX - 05/24/2016
-- AX Required Flag is True then check project and sub project (Dimension_5_part_1 and Dimension_5_part_2) values. 
-- if values are null then stop from Submitting to Billing.
...
IF @debug = 1 PRINT '@AX_Project_Required_Flag_count = ' + CONVERT(varchar(4), @AX_Project_Required_Flag_count)
  
IF @sync_invoice_ax = 1 AND @trans_source = 'W'
BEGIN
 IF @AX_Project_Required_Flag_count > 0
  BEGIN
	...
	IF @sync_ax_Service = 1 
	  BEGIN
		INSERT INTO #acc(AX_MainAccount,
			AX_Dimension_1,
			AX_Dimension_2,
			AX_Dimension_3,
			AX_Dimension_4,
			...
			AX_Dimension_5_part_2,
			status)
		SELECT DISTINCT bd.AX_MainAccount,
			bd.AX_Dimension_1,
			bd.AX_Dimension_2,
			bd.AX_Dimension_3,
			bd.AX_Dimension_4,
		...
		set status = dbo.fnValidateFinancialDimension (@ax_web_service,AX_MainAccount,AX_Dimension_1,AX_Dimension_2,AX_Dimension_3,AX_Dimension_4,AX_Dimension_6,
							case @d365_flag when '1' then '' else AX_Dimension_5_part_1 end,
							case @d365_flag when '1' then '' else AX_Dimension_5_part_2 end)
                                                            
		INSERT INTO WorkOrderAudit (company_id, 
			profit_ctr_id, 
			workorder_id, 
			resource_type, 
			sequence_id, 
		...
		SELECT DISTINCT bd.company_id, 
			...
			'', 
			'', 
			bd.billing_type AS before_value
			,LEFT ( bd.AX_MainAccount + '-' + bd.AX_Dimension_1 + '-' + bd.AX_Dimension_2 + '-' + bd.AX_Dimension_3 + '-' + bd.AX_Dimension_4 + '-' + bd.AX_Dimension_6 + '-' + bd.AX_Dimension_5_part_1+ CASE WHEN bd.AX_Dimension_5_part_2 <> '' THEN '.' + bd.AX_Dimension_5_part_2 ELSE '' END  + 
			dbo.fnValidateFinancialDimension(@ax_web_service,bd.AX_MainAccount, bd.AX_Dimension_1, bd.AX_Dimension_2, bd.AX_Dimension_3, bd.AX_Dimension_4, bd.AX_Dimension_6,	case @d365_flag when '1' then '' else bd.AX_Dimension_5_part_1 end,
							case @d365_flag when '1' then '' else bd.AX_Dimension_5_part_2 end ), 255)  AS after_value
			,'Submit to Billing Failed. AX Dimension 5 (Project - Subproject) missing.'  AS audit_reference, 
			@user_code, 
			@submit_date
		FROM #BillingDetail bd
	...
	ELSE
		...
		SELECT DISTINCT bd.company_id, 
			...
			'', 
			bd.billing_type AS before_value
			,LEFT (bd.AX_MainAccount + '-' + bd.AX_Dimension_1 + '-' + bd.AX_Dimension_2 + '-' + bd.AX_Dimension_3 + '-' + bd.AX_Dimension_4 + '-' + bd.AX_Dimension_6 + '-' + bd.AX_Dimension_5_part_1 + CASE WHEN bd.AX_Dimension_5_part_2 <> '' THEN '.' + bd.AX_Dimension_5_part_2 ELSE '' END , 255 ) AS after_value
			,'Submit to Billing Failed. AX Dimension 5 (Project - Subproject) missing.'  AS audit_reference, 
			@user_code, 
			@submit_date
		FROM #BillingDetail bd
  ...
  --  WHERE ( AX_MainAccount = '' OR AX_Dimension_1 = '' OR AX_Dimension_2 = '' OR AX_Dimension_3 = '' OR AX_Dimension_4 = '' OR AX_Dimension_6 = '')
 ...
 --   BEGIN
	...
	IF @sync_ax_Service = 1 
	 BEGIN
		INSERT INTO #acc1(AX_MainAccount, 
			AX_Dimension_1, 
			AX_Dimension_2,
			bd.AX_Dimension_3, 
			AX_Dimension_4, 
		...
		SELECT DISTINCT bd.company_id, 
			...
			'', 
			bd.billing_type AS before_value
			,LEFT ( bd.AX_MainAccount + '-' + bd.AX_Dimension_1 + '-' + bd.AX_Dimension_2 + '-' + bd.AX_Dimension_3 + '-' + bd.AX_Dimension_4  + '-' + ISNULL(bd.AX_Dimension_6, '') + '-' + ISNULL(bd.AX_Dimension_5_part_1, '') + CASE WHEN ISNULL(bd.AX_Dimension_5_part_2, '') <> '' THEN '.' + bd.AX_Dimension_5_part_2 ELSE '' END  + 
				( dbo.fnValidateFinancialDimension(@ax_web_service,bd.AX_MainAccount, bd.AX_Dimension_1, bd.AX_Dimension_2, bd.AX_Dimension_3, bd.AX_Dimension_4, bd.AX_Dimension_6, case @d365_flag when '1' then '' else bd.AX_Dimension_5_part_1 end,
						case @d365_flag when '1' then '' else bd.AX_Dimension_5_part_2 end) ) , 255 ) AS after_value
			,'Submit to Billing Failed. AX Dimension Invalid. '  AS audit_reference, 
			@user_code, 
			@submit_date
		FROM #BillingDetail bd
		JOIN #Billing b ON b.billing_uid = bd.billing_uid
		JOIN #acc1 a on bd.AX_MainAccount = a.AX_MainAccount and bd.AX_Dimension_1 = a.AX_Dimension_1 and bd.AX_Dimension_2 = a.AX_Dimension_2
		AND bd.AX_Dimension_3 = a.AX_Dimension_3 and  bd.AX_Dimension_4 = a.AX_Dimension_4
		AND bd.AX_Dimension_6 = a.AX_Dimension_6 and  bd.AX_Dimension_5_Part_1 = a.AX_Dimension_5_Part_1 and bd.AX_Dimension_5_Part_2 = a.AX_Dimension_5_Part_2
		WHERE bd.trans_source = 'W'
		AND UPPER (a.status ) <> 'VALID'
		--AND ( dbo.fnValidateFinancialDimension(bd.AX_MainAccount, bd.AX_Dimension_1, bd.AX_Dimension_2, bd.AX_Dimension_3, bd.AX_Dimension_4, bd.AX_Dimension_6, bd.AX_Dimension_5_part_1, bd.AX_Dimension_5_part_2 ) ) <> 'Valid' 

		SELECT @invalid_ax_count =  @@ROWCOUNT, @error_value = @@ERROR  
	 END 	
	 IF @sync_ax_Service = 0
	 BEGIN
	    INSERT INTO WorkOrderAudit (company_id, 
			profit_ctr_id, 
			workorder_id, 
		...
		SELECT DISTINCT bd.company_id, 
			...
			'', 
			bd.billing_type AS before_value
			,LEFT (bd.AX_MainAccount + '-' + bd.AX_Dimension_1 + '-' + bd.AX_Dimension_2 + '-' + bd.AX_Dimension_3 + '-' + bd.AX_Dimension_4 + '-' + ISNULL(bd.AX_Dimension_6, '') + '-' + ISNULL(bd.AX_Dimension_5_part_1, '') + CASE WHEN ISNULL(bd.AX_Dimension_5_part_2, '') <> '' THEN '.' + bd.AX_Dimension_5_part_2 ELSE '' END ,255)  AS after_value
			,'Submit to Billing failed. AX Dimension Invalid. '  AS audit_reference, 
			@user_code, 
			@submit_date
		FROM #BillingDetail bd
  ...
  END 
 --END
 IF @debug = 1 PRINT ' WorkOrder @invalid_ax_count = ' + CONVERT(varchar(4), @invalid_ax_count)
 -- END of  AX - 05/24/2016

 -- Below for Receipt
 IF @sync_invoice_ax = 1 AND @trans_source = 'R'
BEGIN
 --IF @AX_Project_Required_Flag_count > 0
 -- BEGIN
	--SELECT @invalid_ax_count = @invalid_ax_count + COUNT(*) FROM #BillingDetail WHERE (AX_Dimension_5_part_1 = 'X' OR AX_Dimension_5_part_2 = 'Z')
 ...
 --   BEGIN
	...
	--	SELECT DISTINCT bd.company_id, bd.profit_ctr_id, bd.receipt_id, bd.line_id, bd.price_id, '', '', bd.billing_type AS before_value, bd.AX_Dimension_5_part_1 + '.' + bd.AX_Dimension_5_part_2 AS after_value, 'Submit to Billing Failed. AX Dimension 5 (Project - Subproject) missing.' AS audit_reference, @user_code, 'BS SP', @submit_date
	--	FROM #BillingDetail bd
	--	JOIN #Billing b ON b.billing_uid = bd.billing_uid
	--	WHERE 1=1
	--	AND bd.trans_source = 'R'
 ...
 --IF @invalid_ax_count > 0
  --BEGIN 
	...
	--	------------------------------------
		IF @sync_ax_Service = 1 
		 BEGIN
			INSERT INTO #acc2(AX_MainAccount, 
				AX_Dimension_1, 
				AX_Dimension_2, 
				AX_Dimension_3, 
				AX_Dimension_4, 
			...
			SELECT DISTINCT bd.company_id, 
				...
				@submit_date
			FROM #BillingDetail bd
			 JOIN #acc2 a on bd.AX_MainAccount = a.AX_MainAccount and bd.AX_Dimension_1 = a.AX_Dimension_1 and bd.AX_Dimension_2 = a.AX_Dimension_2
			  AND bd.AX_Dimension_3 = a.AX_Dimension_3 and  bd.AX_Dimension_4 = a.AX_Dimension_4
		      AND bd.AX_Dimension_6 = a.AX_Dimension_6 and  bd.AX_Dimension_5_Part_1 = a.AX_Dimension_5_Part_1 and bd.AX_Dimension_5_Part_2 = a.AX_Dimension_5_Part_2
			JOIN #Billing b ON b.billing_uid = bd.billing_uid
			WHERE bd.trans_source = 'R'
...
BEGIN
	...
	BEGIN
		...
		SELECT DISTINCT bd.company_id, 
			...
			'', 
			bd.billing_type AS before_value, 
			bd.JDE_BU + '-' + bd.JDE_object AS after_value, 
			'Submit to Billing Failed.  Invalid JDE GL Account.' AS audit_reference, 
			@user_code, 
			'BS SP', 
			@submit_date
		FROM #BillingDetail bd
		...
		SELECT @invalid_gl_count = @invalid_gl_count + @@ROWCOUNT, @error_value = @@ERROR
			IF @debug = 1 PRINT '@invalid_gl_count = ' + convert ( varchar(20), @invalid_gl_count)
	END
	ELSE 
	IF @trans_source = 'W' 
	BEGIN
	...
	END
END
--IF @debug = 1 PRINT '@invalid_gl_count =  @invalid_gl_count ' + CONVERT(varchar(4), @invalid_gl_count) 
--IF @debug = 1 PRINT '@invalid_gl_count =  @receipt_count ' + CONVERT(varchar(4), @receipt_count)

IF @receipt_count > 0 AND @invalid_gl_count = 0 AND @invalid_ax_count = 0 
BEGIN
	...
	BEGIN
		...
		SELECT DISTINCT sales_tax_state
		INTO #tmp_tax_state
		FROM SalesTax s 
		JOIN #SalesTax t
		ON s.sales_tax_id = t.sales_tax_id
		
		--Select * from #tmp_tax_state
		
		IF EXISTS (SELECT 1 FROM #tmp_tax_state where sales_tax_state IN ('NY','CT')) 
		BEGIN
			...
			AND company_id = @company_id
			AND profit_ctr_id =  @profit_ctr_id
			PRINT 'customer_id =' + Convert(varchar(10),@customer_id)
			--Select @customer_id
			
			IF @customer_id IS NOT NULL 
			BEGIN
				SELECT top 1 @cust_tax_flag =  tax_flag
				FROM CustomerSalesTax c
				INNER JOIN SalesTax s
				ON c.sales_tax_id = s.sales_tax_id
				WHERE c.customer_id = @customer_id
				AND s.sales_tax_state IN ('NY','CT')
				AND tax_flag = 'T'
			END
			--Select @cust_tax_flag
			IF @generator_id IS NOT NULL
			BEGIN
				SELECT top 1  @gen_tax_flag = tax_flag
				FROM GeneratorSalesTax g
				INNER JOIN SalesTax s
				ON g.sales_tax_id = s.sales_tax_id
				WHERE g.generator_id = @generator_id
				AND s.sales_tax_state IN ('NY','CT')
				AND tax_flag = 'T'
			END

			IF @cust_tax_flag = 'T' OR @gen_tax_flag = 'T'
			BEGIN
				Create table #billingdetail_uid (billingdetail_uid int)
				-- Rally # DE35804 - Included Billing table and modified the Join to include billing.workorder_sequence_id and workorderdetail.sequence_id - 10/25/2024
				Insert #billingdetail_uid
				SELECT bd.billingdetail_uid
				FROM BillingDetail bd
				INNER JOIN Billing b ON b.billing_uid = bd.billing_uid
				INNER JOIN WorkOrderDetail wod 
					ON wod.company_id = b.company_id
					AND wod.profit_ctr_id = b.profit_ctr_id
					AND wod.workorder_id = b.receipt_id
					AND wod.resource_type = b.workorder_resource_type
					AND wod.sequence_id = b.workorder_sequence_id
					AND wod.resource_type = bd.trans_type
				JOIN ResourceClass rc
					...
					WHERE rc.regulated_fee  = 'T'
					AND wod.workorder_ID =  @receipt_id
					AND wod.company_id =  @company_id
					AND wod.profit_ctr_ID = @profit_ctr_id
					AND bd.extended_amt > 0 
					AND bd.trans_source = 'W'
				--DevOps 78372 - EEC Fee should be charged sales tax, commented out UNION
				--UNION
				--SELECT b.billingdetail_uid
				--FROM BillingDetail b 
				--WHERE b.receipt_id =  @receipt_id
				--AND b.company_id =  @company_id
				--AND b.profit_ctr_ID = @profit_ctr_id
				--AND b.extended_amt > 0 
				--AND b.trans_source = 'W'
				--AND billingtype_uid = 11


				IF EXISTS (SELECT 1 FROM BillingDetail INNER JOIN #billingdetail_uid 
						   ON ref_billingdetail_uid = #billingdetail_uid.billingdetail_uid  
						   WHERE billingtype_uid = 10 AND company_id = @company_id AND profit_ctr_id = @profit_ctr_id)
					
					DELETE BillingDetail
					FROM BillingDetail b
					INNER JOIN #billingdetail_uid bu
				    ON ref_billingdetail_uid = bu.billingdetail_uid 
					WHERE billingtype_uid = 10
					AND company_id = @company_id 
					AND profit_ctr_id = @profit_ctr_id
					...
					BEGIN		
						-- Set message for RAISEERROR and go to the end
						SET @error_msg = 'Error deleting from Billingdetail.'
						GOTO END_OF_PROC
					END
				END
			END
	...
	IF (@invoice_id IS NULL AND @trans_source <> 'R') OR (@trans_source = 'R' AND @invoice_id IS NULL AND @billingcomment_customer_id IS NULL)
	BEGIN
	...
	BEGIN
		UPDATE BillingComment SET
			...
			customer_id = CASE WHEN @trans_source = 'R' THEN #BillingComment.customer_id END,
			invoice_id = CASE WHEN @trans_source = 'R' THEN @invoice_id END,
			invoice_code = CASE WHEN @trans_source = 'R' THEN @invoice_code END,
			invoice_date = CASE WHEN @trans_source = 'R' THEN @invoice_date END
		FROM BillingComment
...
IF @receipt_count = @billing_count AND @receipt_count <> 0 
BEGIN
	IF @trans_source = 'R'  
	BEGIN
		-- Show all receipt lines AS submitted, even if some
		-- weren't due to being Voided or no print on invoice 
		-- Sailaja - Rally #US127391 - EQAI Tracking feature addition in Inbound Receipt Screen - 11/26/2024
		-- Included tracking_Days, tracking_bus_days, tracking_id and tracking_contact in receipt table
		...
		VALUES (@company_id, 
			...
			'date_submitted', 
			'(blank)', 
			CONVERT(varchar, @submit_date,109), 
			'Submitted to Billing',
			@user_code, 
			'SB', 
			@submit_date)
		...
		-- Sailaja - Rally #US127391 - EQAI Tracking feature addition in Inbound Receipt Screen - 11/26/2024 - Start
		-- Audit record for update of Receipt.tracking_id
		If @trans_mode = 'I' 
		Begin
			INSERT ReceiptAudit (company_id, 
				profit_ctr_id, 
			...
			VALUES (@company_id, 
				...
				'Receipt', 
				'tracking_id', 
				CASE WHEN @max_tracking_id IS NOT NULL THEN CAST(@max_tracking_id AS VARCHAR(255)) ELSE '(blank)' END,
				CASE WHEN @max_tracking_id IS NOT NULL THEN CAST(@max_tracking_id + 1 AS VARCHAR(255)) ELSE '1' END, 
				'''Completed'' tracking record added', 
				@user_code, 
				'SB', 
				@submit_date)
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error inserting ReceiptAudit record for update of Receipt.tracking_id.'
				GOTO END_OF_PROC
			END

			IF @min_time_in IS NOT NULL
			BEGIN
				-- Audit record for update of Receipt.tracking_days
				INSERT ReceiptAudit (company_id, 
					profit_ctr_id, 
					receipt_id, 
					line_id, 
				...
				VALUES (@company_id, 
					...
					'tracking_days', 
					'(blank)', 
					CAST(DATEDIFF(dd, @min_time_in, @submit_date) AS VARCHAR(255)), 
					'''Completed'' tracking record added', 
					@user_code, 
					'SB', 
					@submit_date)
				...
				BEGIN		
					-- Set message for RAISEERROR and go to the end
					SET @error_msg = 'Error inserting ReceiptAudit record for update of Receipt.tracking_days.'
					GOTO END_OF_PROC
				END

				-- Audit record for update of Receipt.tracking_bus_days
				INSERT ReceiptAudit (company_id, 
					profit_ctr_id, 
					receipt_id, 
					line_id, 
				...
				VALUES (@company_id, 
					...
					'tracking_bus_days', 
					'(blank)', 
					CAST(dbo.fn_business_days(@min_time_in, @submit_date) AS VARCHAR(255)),
					'''Completed'' tracking record added', 
					@user_code, 
					'SB', 
					@submit_date)
				...
				BEGIN		
					-- Set message for RAISEERROR and go to the end
					SET @error_msg = 'Error inserting ReceiptAudit record for update of Receipt.tracking_bus_days.'
					GOTO END_OF_PROC
				END
			END

			IF @tracking_contact IS NOT NULL
			BEGIN
				-- Audit record for update of Receipt.tracking_contact
				INSERT ReceiptAudit (company_id, 
					profit_ctr_id, 
					receipt_id, 
					line_id, 
				...
				BEGIN		
					-- Set message for RAISEERROR and go to the end
					SET @error_msg = 'Error inserting ReceiptAudit record for update of Receipt.tracking_contact.'
					GOTO END_OF_PROC
				END
			END
		End
	...
	ELSE
	IF @trans_source = 'W' AND @invalid_ax_count = 0 
	BEGIN
		-- DevOps 41806 - Added update for new columns tracking_id, tracking_days, tracking_bus_days, and tracking_contact
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error updating WorkOrderHeader for submitted_flag, date_submitted, submitted_by, and tracking info.'
			GOTO END_OF_PROC
		END
		...
		-- DevOps 41806
		-- Audit record for update of WorkOrderHeader.tracking_id
		INSERT WorkOrderAudit (company_id, 
			profit_ctr_id, 
			workorder_id, 
			resource_type, 
		...
		BEGIN		
			-- Set message for RAISEERROR and go to the end
			SET @error_msg = 'Error inserting WorkOrderAudit record for update of WorkOrderHeader.tracking_id.'
			GOTO END_OF_PROC
		END

		IF @min_time_in IS NOT NULL
		BEGIN
			-- Audit record for update of WorkOrderHeader.tracking_days
			INSERT WorkOrderAudit (company_id, 
				profit_ctr_id, 
				workorder_id, 
				resource_type, 
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error inserting WorkOrderAudit record for update of WorkOrderHeader.tracking_days.'
				GOTO END_OF_PROC
			END

			-- Audit record for update of WorkOrderHeader.tracking_bus_days
			INSERT WorkOrderAudit (company_id, 
				profit_ctr_id, 
				workorder_id, 
				resource_type, 
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error inserting WorkOrderAudit record for update of WorkOrderHeader.tracking_bus_days.'
				GOTO END_OF_PROC
			END
		END
		...
		IF @tracking_contact IS NOT NULL
		BEGIN
			-- Audit record for update of WorkOrderHeader.tracking_contact
			INSERT WorkOrderAudit (company_id, 
				profit_ctr_id, 
				workorder_id, 
				resource_type, 
			...
			BEGIN		
				-- Set message for RAISEERROR and go to the end
				SET @error_msg = 'Error inserting WorkOrderAudit record for update of WorkOrderHeader.tracking_contact.'
				GOTO END_OF_PROC
			END
		END
...
BEGIN
	...
	END
END
  -- IF @error_value <> 0 AND @sync_invoice_ax = 1 AND @invalid_ax_count > 0 
	 --BEGIN
		...
		--SELECT DISTINCT bd.company_id, bd.profit_ctr_id, bd.receipt_id, b.workorder_resource_type, b.workorder_sequence_id, '', '', bd.billing_type AS before_value, bd.AX_Dimension_5_part_1 + '.' + bd.AX_Dimension_5_part_2 AS after_value, 'Submit to Billing Failed.  Invalid AX Account.' AS audit_reference, @user_code, @submit_date
		--FROM #BillingDetail bd
		--JOIN #Billing b ON b.billing_uid = bd.billing_uid
		--WHERE 1=1
		--AND bd.trans_source = 'W'
			
		--SELECT @invalid_ax_count = @invalid_ax_count + @@ROWCOUNT, @error_value = @@ERROR
	 -- END 
	 IF @debug = 1 PRINT '@@error_value = ' + CONVERT(varchar(4), @error_value) 
 ...
 BEGIN
  IF @trans_source = 'W' 
    BEGIN
      IF @sync_ax_Service = 1 
        BEGIN
			INSERT INTO #acc3(AX_MainAccount, 
				AX_Dimension_1, 
				AX_Dimension_2,
				bd. AX_Dimension_3, 
				AX_Dimension_4, 
			...
			FROM #BillingDetail bd
			JOIN #Billing b ON b.billing_uid = bd.billing_uid
			 JOIN #acc3 a on bd.AX_MainAccount = a.AX_MainAccount and bd.AX_Dimension_1 = a.AX_Dimension_1 and bd.AX_Dimension_2 = a.AX_Dimension_2
			AND bd.AX_Dimension_3 = a.AX_Dimension_3 and  bd.AX_Dimension_4 = a.AX_Dimension_4
			AND bd.AX_Dimension_6 = a.AX_Dimension_6 and  bd.AX_Dimension_5_Part_1 = a.AX_Dimension_5_Part_1 and bd.AX_Dimension_5_Part_2 = a.AX_Dimension_5_Part_2
			WHERE bd.trans_source = 'W'
			AND UPPER (a.status ) <> 'VALID'
  ...
  IF @trans_source = 'R'
   BEGIN
     IF @sync_ax_Service = 1 
      BEGIN
			INSERT INTO #acc4(AX_MainAccount, 
				AX_Dimension_1, 
				AX_Dimension_2,
				bd. AX_Dimension_3, 
				AX_Dimension_4, 
			...
			FROM #BillingDetail bd
			JOIN #Billing b ON b.billing_uid = bd.billing_uid
			JOIN #acc4 a on bd.AX_MainAccount = a.AX_MainAccount and bd.AX_Dimension_1 = a.AX_Dimension_1 and bd.AX_Dimension_2 = a.AX_Dimension_2
				AND bd.AX_Dimension_3 = a.AX_Dimension_3 and  bd.AX_Dimension_4 = a.AX_Dimension_4
				AND bd.AX_Dimension_6 = a.AX_Dimension_6 and  bd.AX_Dimension_5_Part_1 = a.AX_Dimension_5_Part_1 and bd.AX_Dimension_5_Part_2 = a.AX_Dimension_5_Part_2
			WHERE bd.trans_source = 'R'
			AND UPPER (a.status ) <> 'VALID'
...
GRANT EXECUTE
    ON[dbo].[sp_billing_submit] TO [EQAI];
GO

-- drop proc if exists sp_cor_dashboard_service_count_spend_detail

go

create proc sp_cor_dashboard_service_count_spend_detail (
	...
	, @schedule_flag	varchar(20) = 'both' -- ignored in this SP
    , @customer_id_list varchar(max)=''  /* Added 2019-07-15 by AA */
	, @generator_id_list varchar(max)=''  /* Added 2019-07-15 by AA */
)
as
/* ***********************************************************
sp_cor_dashboard_service_count_spend_detail 


sp_cor_dashboard_service_count_spend_detail 'nyswyn100'
...
exec sp_cor_dashboard_service_count_spend_detail
	@web_userid = 'court_c'
	, @date_start = '1/1/2021'
	, @date_end = '12/31/2021'
...
insert @time
SELECT year(cte_start_date), month(cte_start_date), sf
FROM CTE
cross join (select 'scheduled' as sf union select 'on demand' as sf) x
...
left join ContactCORStatsServiceCountSpend b
	on t.m_yr = b._year
	and t.m_mo = b._month
	and t.scheduled_flag = case isnull(b.offschedule_service_flag, 'F')
			when 'F' then 'scheduled'
			when 'T' then 'on demand'
		end
...
ORDER BY 
 ...
 select s._year
	, s._month
	, s._count as scheduled_count
	, d._count as on_demand_count
	, s._total as scheduled_total
	, d._total as on_demand_total
	, s._avg_cost_per_stop as scheduled_avg_cost_per_stop
	, d._avg_cost_per_stop as on_demand_avg_cost_per_stop
	, s.currency_code
from #fmt s
	join #fmt d on s._year = d._year
		and s._month = d._month
		and s.scheduled_flag = 'scheduled'
		and d.scheduled_flag = 'on demand'
ORDER BY 
s._year
	, s._month
...
grant execute on sp_cor_dashboard_service_count_spend_detail to eqai, eqweb, cor_user
go

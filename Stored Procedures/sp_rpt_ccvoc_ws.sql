DROP PROCEDURE IF EXISTS [dbo].[sp_rpt_ccvoc_ws]
GO

CREATE PROCEDURE [dbo].[sp_rpt_ccvoc_ws]
	@company_id		int
,	@profit_ctr_id	int
,	@date_from		datetime
,	@date_to		datetime
...
/***************************************************************************************
Filename:	F:\EQAI\SQL\EQAI\sp_rpt_ccvoc_ws.sql
...
01/27/2022 MPM	DevOps 27253 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).
				Also noticed that VOC and dependent numbers were displaying as empty space on the report instead of "0.0000", 
				so I replaced "r.ddvoc" in the insert into #tmp with "ISNULL(r.ddvoc, 0)".
07/31/2023 Dipankar DevOps 50330 - Modified per DBA's observation comments

06/28/2022 GDE  DevOps 42732; CCVOC Worksheet - Report update
sp_rpt_ccvoc_ws 21, 0, '1-1-2005', '12-31-2005', 0, 999999
sp_rpt_ccvoc_ws 21, 0, '10-01-2010', '10-01-2010', 0, 999999 
...
,	ROUND((ISNULL(r.ccvoc, 0)/1000000), 8) AS VOC
...
/*	MPM - 1/26/2022 - DevOps 27253 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).  
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_ccvoc_ws] TO [EQAI]
    AS [dbo];

CREATE PROCEDURE sp_rpt_outbounds_without_scanned_returned_manifest
	@company_id    int,
	@profit_ctr_id int,
	@date_from datetime,
	@date_to   datetime
...
/***************************************************************************************
Loads to:		PLT_AI
PB Object(s):	r_outbounds_without_scanned_returned_manifest

06/26/2017 AM 	Created - Create a New report for outbounds without a scanned returned manifest

sp_rpt_outbounds_without_scanned_returned_manifest 21,0,'10/1/2016','10/10/2016'
...
and not exists (select 1 from plt_image..scan
				...
				and type_id in (select type_id from ScanDocumentType where scan_type = 'receipt' and document_type = 'Returned Manifest')
				) 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_outbounds_without_scanned_returned_manifest] TO [EQAI]
    AS [dbo];

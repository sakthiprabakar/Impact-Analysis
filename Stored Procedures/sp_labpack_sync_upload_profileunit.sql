create procedure [dbo].[sp_labpack_sync_upload_profileunit]
...
as
/***************************************************************************************
 this procedure records a connection from lab pack field device to upload profile detail
...
set @sql = 'if not exists (select 1 from ProfileQuoteDetail'
+ ' where profile_id = ' + convert(varchar(20),@profile_id)
...
+ ', ''A'''
+ ', (select coalesce(max(sequence_id),0) + 1 from ProfileQuoteDetail where profile_id = ' + convert(varchar(20),@profile_id) + ' and company_id = ' + convert(varchar(20),@company_id)+ ' and profit_ctr_id = ' + convert(varchar(20),@profit_ctr_id) + ')'
+ ', ''D'''

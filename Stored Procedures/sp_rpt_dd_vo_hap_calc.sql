DROP PROCEDURE IF EXISTS [dbo].[sp_rpt_dd_vo_hap_calc]
GO

CREATE PROCEDURE [dbo].[sp_rpt_dd_vo_hap_calc]
 	@company_id		int
,	@profit_ctr_id	int
,	@date_from		datetime
,	@date_to		datetime
...
10/07/2003 JDB		Changed sp name from sp_rpt_wcvoc to sp_rpt_dd_vo_hap_calc.
...
03/11/2015 AM	Modified code to call new function to get calculated ddvoc value Instead of receipt.ddvoc 
07/14/2015 SK	Renamed the sp to sp_rpt_dd_vo_hap_calc. This will be used as a calculated version. The value for DD VO HAP 
				will not be taken from profile but will be system calculated.
04/14/2022 MPM	DevOps 27257 - Corrected the calculation of emissions.
06/28/2022 GDE  DevOps 42737 - DD VO HAP Report (Calculated) - Report update
08/01/2023 Dipankar - DevOps 50330 - Implemented DBA Observation comments

sp_rpt_dd_vo_hap_calc 21, 0, '1-1-2005', '12-31-2005', 1, 999999
sp_rpt_dd_vo_hap_calc 21, 0, '10-01-2010', '10-03-2010', 0, 999999
sp_rpt_dd_vo_hap_calc 21, 0, '01-14-2014', '01-14-2014', 0, 999999
****************************************************************************************/
...
,   Round (( dbo.fn_calculate_constituents_ddvoc (r.company_id,r.profit_ctr_id,r.receipt_id,r.line_id)/1000000),8) as DDVOC
...
SELECT
	...
	,	cd.disposal_date
	,	0 AS processed_flag
	,   Round (( dbo.fn_calculate_constituents_ddvoc (@company_id,@profit_ctr_id,cd.receipt_id,cd.line_id)/1000000),8) as DDVOC
	FROM ContainerDestination cd
	JOIN ProcessLocation pl
		ON pl.company_id = cd.company_id
		AND pl.profit_ctr_id = cd.profit_ctr_id
...
WITH source_containers AS
	(SELECT fcs.*, tmp.treatment_id, tmp.final_location as location, tmp.location_report_flag as location_rpt_flag, tmp.disposal_date as disposal_dt, tmp.ddvoc
...
--,	ROUND((r.ddvoc/1000000), 8) AS DDVOC
,   tr.ddvoc as ddvoc
...
JOIN #tmp_receipts tr
	...
	AND ((tr.ddvoc > 0 AND tr.ddvoc IS NOT NULL) OR @rpt_zero_flag = 'T')
...
UPDATE #tmp SET tons_voc = ROUND((pounds * DDVOC * @pound_ton_conversion), 8)

/*	MPM - 4/14/2022 - DevOps 27257 - Corrected the calculation of emissions.  
...
SELECT	
	#tmp.company_id,
	#tmp.profit_ctr_id,
	#tmp.receipt_id as receipt_id, -- rb 02/14/2013
	#tmp.line_id as line_id, -- rb 02/14/2013
...
AND (( DDVOC > 0 AND DDVOC IS NOT NULL) OR @rpt_zero_flag = 'T')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_dd_vo_hap_calc] TO [EQAI]
    AS [dbo];

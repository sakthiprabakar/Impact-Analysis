...
CREATE PROCEDURE sp_MessageType_report2 (
...
sp_MessageType_report2 null, null, 0, 2

-- 7/21 - 7/25: No fails or problems. So unless the @force flag is given, no email report comes out.
sp_MessageType_report2 @start_date = '7/21/2012', @end_date = '7/25/2012', @force_report_send = 0, @debug = 1 
...
if @debug > 1 begin
	...
	select 
		...
		isnull(sum (case when status = 'F' or (status = 'N' and isnull(date_to_send, date_added) < getdate()) then 1 else 0 end), 0) as fail_count,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_MessageType_report2] TO PUBLIC
    AS [dbo];

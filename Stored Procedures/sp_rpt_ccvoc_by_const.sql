...
AS
/***********************************************************************
This procedure runs for the CCVOC constituent Worksheet.

Filename:	L:\Apps\SQL\EQAI\sp_rpt_ccvoc_by_const.sql
PB Object(s):	r_ccvoc_by_const_worksheet
		
This report was inherited from the TRI report and has similar logic in it.  
however this report only has a report type 1 (workorsheet) and contains more information.

03/10/2008 rg  added treatment_category to report criteria
03/12/2008 rg  added new columns for ddvoc and voc
01/04/2011 SK  added company_id as input arg, modified to run for Plt_AI
01/10/2011 SK  moved to Plt_AI
08/21/2013 SM	Added wastecode uid condition
03/11/2022 AM DevOps:17098 - Added 'ug/kg', 'ppb' and 'ug/L' calculation
05/13/2022 MPM	DevOps 27250 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).
...
JOIN Treatment
	...
	--AND (@treatment_category = -99 OR Treatment.reportable_category = ISNULL(@treatment_category, treatment.reportable_category))
	AND Treatment.reportable_category <> 4
JOIN TreatmentCategory
	ON TreatmentCategory.reportable_category = Treatment.reportable_category	
JOIN ReceiptConstituent
...
BEGIN
	...
	PRINT 'UPDATE #tri_work_table SET container_size'
END
...
UPDATE #tri_work_table SET container_size = bill_unit_code WHERE container_size = '' OR bulk_flag = 'T' -- Combined above statements for #50330
...
WHERE #tri_work_table.container_size = b.bill_unit_code
	...
	AND ((#tri_work_table.bill_unit_code IS NULL) OR 
		 (#tri_work_table.bill_unit_code <> #tri_work_table.container_size) OR 
		 (#tri_work_table.pound_conv = 999999999999.99999))
...
UPDATE  #tri_work_table 
SET c_density = CASE WHEN consistency LIKE 'solid%' THEN 12.5
					 WHEN consistency LIKE 'sludge%' OR consistency LIKE 'semi-solid%' THEN 10
					 WHEN consistency LIKE 'liquid%' THEN 8.3453
					 WHEN consistency LIKE 'dust%' THEN 7.5
					 WHEN consistency LIKE 'debris%'THEN 5 ELSE 12.5 END
WHERE c_density IS NULL OR c_density = 0
...
UPDATE #tri_work_table SET pounds_constituent = 0 
...
UPDATE #tri_work_table SET ppm_concentration = 0 
...
/*	MPM - 5/17/2022 - DevOps 27250 - Corrected the calculation of pounds_emission.  The formula is:
	...
	The values for emission_factor and control_efficiency_value in #tri_work_table are set in the union of select statements that 
...
-- 	SELECT * FROM #tri_work_table ORDER BY const_id, CAS_code, treatment_id, approval_code
SELECT
...
WHERE #tri_work_table.pounds_constituent > 0.000005
GROUP BY 
	#tri_work_table.company_id,	
	#tri_work_table.profit_ctr_id,
	Company.company_name,

...
create proc sp_target_disposal_extract_build (
	@start_date				datetime
	,@end_date				datetime
...
/* ******************************************************************
sp_target_disposal_extract_build

	This SP does the heavy lifting to build a Target extract.
	Other SP's handle the output & validation afterward.
...
Samples:
	sp_target_disposal_extract_build  '11/28/2012', '12/18/2012 23:59'
...
sp_target_disposal_extract_build  '2/1/2013', '4/1/2013 23:59'
...
CREATE TABLE #TargetBuildCustomer (
	customer_id 	int
)
INSERT #TargetBuildCustomer values (@customer_id)
...
DELETE FROM EQ_TEMP.dbo.TargetDisposalExtract where date_added <= @extract_datetime -2
...
WHERE 1=1
AND (w.customer_id IN (select customer_id from #TargetBuildCustomer)
    OR w.generator_id IN (SELECT generator_id FROM customergenerator  (nolock) WHERE customer_id IN (select customer_id from #TargetBuildCustomer))
)
...
*/
    ...
    select 
        ...
        r.company_id,
        r.profit_ctr_id,
        b.receipt_id as receipt_workorder_id,
    ...
    where
        (
			b.customer_id IN (select customer_id from #TargetBuildCustomer)
			or
	        b.generator_id in (select generator_id from customergenerator  (nolock) where customer_id IN (select customer_id from #TargetBuildCustomer))
		)
...
--    INTO #ReceiptTransporter     
    ...
    where
		(
			r.customer_id IN (select customer_id from #TargetBuildCustomer)
			OR
            r.generator_id in (select generator_id from customergenerator  (nolock) where customer_id IN (select customer_id from #TargetBuildCustomer))
		)
        and b.invoice_date between @start_date AND @end_date
union
    select 
    ...
    where
        ...
        --    or r.generator_id in (select generator_id from customergenerator  (nolock) where customer_id IN (select customer_id from #TargetBuildCustomer))
        and coalesce(b.invoice_date, rt1.transporter_sign_date, r.receipt_date) between @start_date AND @end_date
        and not exists (
...
INSERT EQ_TEMP.dbo.TargetDisposalExtract
    ...
    (
		...
		where bd.billing_uid = bi.billing_uid
    ) as billing_amt	 
	FROM WorkOrderHeader w (nolock) 
	INNER JOIN WorkOrderDetail d  (nolock) ON w.workorder_id = d.workorder_id and w.company_id = d.company_id and w.profit_ctr_id = d.profit_ctr_id
	INNER JOIN billing bi (nolock)
...
-- Workorder Waste Codes --(1)
	insert #Waste_Codes (source, tsdf_approval_id, sequence_id, origin, waste_code)
	select distinct
		e.source_table,
...
-- Workorder Waste Codes (2+)
	...
	WHERE e.source_table = 'Workorder'
		and e.submitted_flag = 'T'
		AND wc.waste_code_origin in ('F', 'S')
		and xwc.waste_code not in (select waste_code from #Waste_Codes
...
-- Receipt Waste Codes (1)
	insert #Waste_codes (source, receipt_id, line_id, company_id, profit_ctr_id, sequence_id, origin, waste_code)
	select distinct
		e.source_table,
		xwc.receipt_id,
		xwc.line_id,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_target_disposal_extract_build] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_target_disposal_extract_build] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_target_disposal_extract_build] TO [EQAI]
    AS [dbo];

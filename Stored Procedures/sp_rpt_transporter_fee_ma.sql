CREATE PROCEDURE sp_rpt_transporter_fee_ma
	@date_from				datetime
,	@date_to				datetime
...
AS
/***********************************************************************
Hazardous Waste Transporter Fee Report - Massachusetts

Load to:		Plt_AI
PB Object(s):	r_transporter_fee_ma_summ,
				r_transporter_fee_ma_worksheet

06/12/2003 JDB	Created
11/11/2004 MK	Changed generator_code to generator_id
03/08/2005 JDB	Changed fee schedule
...
07/31/2006 RG	revised for profiles
11/29/2006 MK	Added wod.manifest_line to selects to make sure disposal lines are not removed due to identical information (distinct, union)
...
05/04/2010 JDB	Fixed join again from 4/7/10 on ResourceClass.
07/16/2010 KAM  Updated the select to not include voided workorderdetail rows
10/26/2010 SK	Moved to Plt_AI , replaced where clause with joins, return company_id, company_name, profit_ctr_name
02/03/2011 SK	Removed company_id/profit_ctr_id(runs for all companies)
				Changed to run by user selected EPA ID, not TransporterCode
				Changed to fetch StateLicenseCode from new table: TransporterStateLicense
				Removed Company Name, Profit Ctr Name from the resultset
...
sp_rpt_transporter_fee_ma '5/7/2013', '5/9/2013', 10673, 10673, '010277982JJK', '010277982JJK', 'MAD084814136','S'
***********************************************************************/
...
-- SET @fee_lb = 0.0182
-- SET @fee_ton = 0.0182
-- SET @fee_ton_metric = 0.0182
-- SET @fee_liter = 0.1820
-- SET @fee_cubic_yard = 0.0182
-- SET @fee_cubic_meter = 0.0182
-- SET @fee_kg = 0.0182

SET @fee_gal = 0.2640
SET @fee_lb = 0.0264
SET @fee_ton = 0.0264
SET @fee_ton_metric = 0.0264
SET @fee_liter = 0.2640
SET @fee_cubic_yard = 0.0264
SET @fee_cubic_meter = 0.0264
SET @fee_kg = 0.0264
...
SET @gal_conv_liter = 0.2643
SET @pound_conv_cubic_yard = 2000
SET @pound_conv_cubic_meter = 2515.9
SET @pound_conv_kg = 2.205

--SET @ma_license_num = 71
SELECT @ma_license_num = Cast(state_license_code AS INT) FROM TransporterStateLicense
	WHERE State = 'MA' AND EPA_ID = @EPA_ID
...
SELECT @pound_conv_cubic_yard = ISNULL(pound_conv, 2000) FROM BillUnit WHERE bill_unit_code = 'CYB'
SELECT @gal_conv_liter = ISNULL(gal_conv, 0.2643) FROM BillUnit WHERE bill_unit_code = 'LITR'		-- Currently NULL
SELECT @pound_conv_kg = ISNULL(pound_conv, 2.205) FROM BillUnit WHERE bill_unit_code = 'KG'
-- SELECT @pound_conv_cubic_meter = pound_conv FROM BillUnit WHERE bill_unit_code = ''		-- Currently no bill unit for cubic meter
...
,	@ma_license_num AS ma_license_num
--,	wod.quantity_used
--,	wod.bill_unit_code
...
,	@pound_conv_cubic_yard AS pound_conv_cubic_yard
,	@pound_conv_cubic_meter AS pound_conv_cubic_meter
...
--	AND tap.record_type = 'R'
--	--AND ISNULL(tap.fee_exempt_flag, 'F') = 'F'
...
--	AND rc.resource_class_code = 'FEEMAHW'
JOIN billunit b
...
INNER JOIN WorkOrderHeader woh
	...
	AND woh.profit_ctr_ID = wom.profit_ctr_ID
	AND woh.workorder_ID = wom.workorder_ID
	AND woh.workorder_status in ('A','C')
	AND (woh.submitted_flag = 'T' OR @work_order_status = 'C')
	AND woh.customer_id BETWEEN @customer_id_from AND @customer_id_to
	AND woh.start_date BETWEEN @date_from AND @date_to
...
WHERE (TSDF.TSDF_state = 'MA' OR g.generator_state = 'MA')
...
----	ON pqd.company_id = wod.profile_company_id
----	AND pqd.profit_ctr_id = wod.profile_profit_ctr_id
----	AND pqd.profile_id = wod.profile_idMAD084814136
----	AND pqd.record_type = 'R'
----	AND ISNULL(pqd.fee_exempt_flag, 'F') = 'F'
----INNER JOIN ProfileQuoteApproval pa
...
----	ON pqd.bill_unit_code = b.bill_unit_code
--INNER JOIN BillUnit b
--	ON wodum.bill_unit_code = b.bill_unit_code
--INNER JOIN Transporter t
--	ON t.transporter_EPA_ID = @EPA_ID
--	AND t.eq_flag = 'T'
--INNER JOIN WorkOrderTransporter wot1
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_transporter_fee_ma] TO [EQAI]
    AS [dbo];

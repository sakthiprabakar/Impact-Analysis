...
create proc sp_Equipment_UnImported_List
as
/-* *******************************************************************
sp_Equipment_UnImported_List
...
******************************************************************* *-/
	...
	LEFT JOIN assetexplorer.assetexplorer.dbo.ResourceStateHistory History
	...
	where 1=1
		...
		and not exists (
			select 1
			from EquipmentSet es
			join EquipmentXEquipmentSet exes on es.equipment_set_id = exes.equipment_set_id
			join Equipment e on exes.equipment_id = e.equipment_id
			join EquipmentSetXMessage exm on exm.equipment_set_id = es.equipment_set_id and es.status <> 'V'
			where es.user_code = AAALogin.NAME
			and es.status <> 'V'
			and e.equipment_desc = case CIType.Typename 
	...
	group by 
		...
		, AAALogin.NAME
	order by
		MIN(convert(date, DATEADD(second, convert(bigint, left(convert(varchar(50), starttime),10)),{d '1970-01-01'})))


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_Equipment_UnImported_List] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_Equipment_UnImported_List] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_Equipment_UnImported_List] TO [EQAI]
    AS [dbo];

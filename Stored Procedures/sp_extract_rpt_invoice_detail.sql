...
CREATE PROCEDURE sp_extract_rpt_invoice_detail
...
IF @start < 1 SET @start = 1
  ...
  SET @ubound = @start + @maxct
	...
	SET @main_sql = 'SELECT
				    ...
				    * FROM view_extract_rpt_invoice_detail
			WHERE 1=1
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_invoice_detail] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_invoice_detail] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_extract_rpt_invoice_detail] TO [EQAI]
    AS [dbo];

-- drop proc sp_cor_dashboard_on_time_final_manifest_detail
go

CREATE PROCEDURE sp_cor_dashboard_on_time_final_manifest_detail (
...
BEGIN
/* **************************************************************
sp_cor_dashboard_on_time_final_manifest_detail
...
-- SELECT  * FROM    #rex
	...
	select 
		...
		, isnull(g.generator_country, 'USA') generator_country
		, g.site_code
		, r.manifest
		, r.service_date
		, case when r.scan_date is null then 'No' else 'Yes' end as scan_available
		, r.scan_date
		, case when isnull(_datediff, 999) <= 30 then 1 else 0 end as on_time
		, case when isnull(_datediff_modified, 999) <= 30 then 1 else 0 end as on_time_modified
	from #rex r
	join generator g on r.generator_id = g.generator_id

	order by r.service_date, g.generator_name
...
GRANT EXEC ON sp_cor_dashboard_on_time_final_manifest_detail TO EQAI;
GO
GRANT EXEC ON sp_cor_dashboard_on_time_final_manifest_detail TO EQWEB;
GO
GRANT EXEC ON sp_cor_dashboard_on_time_final_manifest_detail TO COR_USER;
GO

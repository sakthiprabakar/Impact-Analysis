...
create proc sp_rite_aid_prebilling_worksheet (
	@trip_id int
) as
/* *************************************************************************
sp_rite_aid_prebilling_worksheet
...
Sample:
	...
	go
	sp_rite_aid_prebilling_worksheet 21616
...
, d.generator_state
, d.approval_code
, case when isnull(d.billing_status_code, '') = '' then '--- Receipt Not Billed Yet! ---' else d.stop_fee_description end as stop_fee_description
, d.category
, case when isnull(d.billing_status_code, '') = '' then '--- Receipt Not Billed Yet! ---' else d.approval_desc end as approval_desc
, case when isnull(d.billing_status_code, '') = '' then 0 else d.detail_weight end as detail_weight
, case when isnull(d.billing_status_code, '') = '' then 0 else d.price_per_pound end as price_per_pound
, case when isnull(d.billing_status_code, '') = '' then 0 else d.weight_charge end as weight_charge

, case when isnull(d.billing_status_code, '') = '' then 0 else case when d.row_number = 1 then isnull((select max(sum_detail_weight) from #summary where site_code = d.site_code and category = 'FE' ), 0) else null end end as FRONT_END_total_weight
, case when isnull(d.billing_status_code, '') = '' then 0 else case when d.row_number = 1 then isnull((select max(sum_detail_weight) from #summary where site_code = d.site_code and category = 'Rx' ), 0) else null end end as RX_total_weight
, case when isnull(d.billing_status_code, '') = '' then 0 else case when d.row_number = 1 then isnull((select max(stop_fee_charge) from #summary where site_code = d.site_code and category = 'FE'), 0) else null end end as FRONT_END_stop_fee_charge
, case when isnull(d.billing_status_code, '') = '' then 0 else case when d.row_number = 1 then isnull((select max(stop_fee_charge) from #summary where site_code = d.site_code and category = 'Rx'), 0) else null end end as RX_stop_fee_charge
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rite_aid_prebilling_worksheet] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rite_aid_prebilling_worksheet] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rite_aid_prebilling_worksheet] TO [EQAI]
    AS [dbo];

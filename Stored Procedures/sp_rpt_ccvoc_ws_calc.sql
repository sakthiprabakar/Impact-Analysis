DROP PROCEDURE IF EXISTS [dbo].[sp_rpt_ccvoc_ws_calc]
GO

CREATE PROCEDURE [dbo].[sp_rpt_ccvoc_ws_calc]
	@company_id		int
,	@profit_ctr_id	int
,	@date_from		datetime
,	@date_to		datetime
...
/***************************************************************************************
Filename:	F:\EQAI\SQL\EQAI\sp_rpt_ccvoc_ws_calc.sql
PB Object(s):	d_rpt_voc_worksheet
...
12/21/2010 SK	Added emissions factor & control efficiency for EQ Oklahama
12/22/2010 SK	Made Changes to include consolidated containers/receipts info.
01/03/2011 SK	Fixed the perfomance issue by removing cursor and #tmp_containers
05/19/2011 RWB	Fixed bug by removing join Receipt.bill_unit_code = BillUnit.bill_unit_code
				...
				Removed outdated bulk_flag/receipt_date reference from where clause.
...
07/13/2011 SK	remove the above enforced r.ccvoc > 0							
...
07/14/2015 SK	Renamed the sp to sp_rpt_ccvoc_ws_calc. This will be used as a calculated version. The value for CCVOC 
				will not be taken from profile but will be system calculated.
01/27/2022 MPM	DevOps 27254 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).
06/28/2022 GDE  DevOps 42733 - CCVOC Worksheet (Calculated) - Report update
07/31/2023 Dipankar DevOps 50330 - Modified per DBA's observation comments

sp_rpt_ccvoc_ws_calc 21, 0, '1-1-2005', '12-31-2005', 0, 999999
sp_rpt_ccvoc_ws_calc 21, 0, '10-01-2010', '10-01-2010', 0, 999999 
sp_rpt_ccvoc_ws_calc 21, 0, '01-01-2015',  '01-20-2015', 0, 999999 
sp_rpt_ccvoc_ws_calc 21, 0, '03/13/2015' , '03/13/2015', 0, 999999 
sp_rpt_ccvoc_ws_calc 2, 0, '1/1/2021' , '1/31/2021', 0, 999999 
...
-- Get values from new ProfitCenterCCVOCDDVOHAP 
 ...
 /*	MPM - 1/26/2022 - DevOps 27254 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).  
...
--rb	bill_unit_code, 
	...
	Treatment.Treatment_process_process as treatment_desc, --rb
	COALESCE ((SELECT  distinct 'Haz'
     ...
     p.profit_ctr_name
FROM #tmp
JOIN company c ON c.company_id = #tmp.company_id
JOIN profitcenter p ON p.company_id = #tmp.company_id
	 AND p.profit_ctr_id = #tmp.profit_ctr_id
JOIN Treatment
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_ccvoc_ws_calc] TO [EQAI]
    AS [dbo];

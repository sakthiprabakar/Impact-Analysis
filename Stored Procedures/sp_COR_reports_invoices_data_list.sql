drop PROCEDURE sp_COR_reports_invoices_data_list
go

CREATE PROCEDURE sp_COR_reports_invoices_data_list
...
AS
/* ***************************************************************************************************
sp_COR_reports_invoices_data_list:
...
exec [sp_COR_reports_invoices_data_list]
...
-- if isnull(@excel_output, 0) = 0 
	...
	join (
	  ...
	  left join workorderquoteheader  qh 
		...
		update #lpxdata set _labpack_pricing_rollup = @ilpx_rollup_override
		where @ilpx_rollup_override <> 'X'
		...
		from #lpxdata
		where isnull(_labpack_pricing_rollup, 'L') in ('T', 'S')
		...
		update #lpxdata set price = null, ext_price = null where isnull(_labpack_pricing_rollup, 'L') in ('T', 'S')
...
GRANT EXECUTE ON sp_COR_reports_invoices_data_list TO EQAI, EQWEB, COR_USER
GO

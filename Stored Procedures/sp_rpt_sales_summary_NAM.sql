--
drop proc if exists sp_rpt_sales_summary_NAM
go

CREATE PROCEDURE [dbo].[sp_rpt_sales_summary_NAM]
	@date_from		datetime
,	@date_to		datetime
...
----------------------------------------------------------------------------------------------------------------------
/* 
Created new Summary Report by NAM and service category. This pulls all the data from Territory Work Calculation and then
...
-- NEW:
	EXECUTE dbo.sp_rpt_sales_summary_NAM
	  @date_from = '1-1-2021',
	  @date_to = '1-31-2021',
	  @copc_list='ALL',
	  @filter_field	= 'NAM_ID',
	  @filter_list = '0,1,2,3,4,5,6,7,8',
	  @debug = 1

-- before changes:
-- 641 rows
-- Total extended amt #TerritoryWork:				8529435.47000002
-- Total extended amt #NAMServiceCategorySummary:	8529435.47000001

-- after changes:
-- 643 rows
-- Total extended amt #TerritoryWork:				8529435.47000005
-- Total extended amt #NAMServiceCategorySummary:	8529435.47000001
...
CREATE TABLE #NAMServiceCategorySummary (
	Dist_Company_ID 		int			NULL
,	Dist_Profit_Ctr_ID		int			NULL
,	Customer_ID				int			NULL
,	Cust_Name				varchar(75) NULL
...
INSERT INTO #NAMServiceCategorySummary
...
,	Total_Unassigned				
,	Total_Amount				
From #NAMServiceCategorySummary NS
JOIN ProfitCenter PC 
	ON PC.company_id = NS.Dist_Company_ID
	AND PC.profit_ctr_id = NS.Dist_Profit_Ctr_ID
ORDER BY Nam_User_Name, business_segment_code, dist_company_id, dist_profit_ctr_id, customer_id
--order by dist_company_id, dist_profit_ctr_id, customer_id, Nam_User_Name, job_type, business_segment_code
...
BEGIN
	...
	PRINT 'Total extended amt #NAMServiceCategorySummary:'
	Select SUM(IsNull(total_amount, 0.00)) FROM #NAMServiceCategorySummary
END 

-- delete temp table
DROP TABLE #TerritoryWork
DROP TABLE #NAMServiceCategorySummary


GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_sales_summary_NAM] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_sales_summary_NAM] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_sales_summary_NAM] TO [EQAI]
    AS [dbo];

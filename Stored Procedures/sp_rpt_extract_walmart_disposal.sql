...
drop proc if exists sp_rpt_extract_walmart_disposal
go

CREATE PROCEDURE sp_rpt_extract_walmart_disposal (
    @start_date             datetime,
    @end_date               datetime,
...
AS
/* ***********************************************************
Procedure    : sp_rpt_extract_walmart_disposal
Database     : PLT_AI
Created      : Jan 25 2008 - Jonathan Broome
Description  : Creates a Wal-Mart Disposal Extract
 
Examples:
    sp_rpt_extract_walmart_disposal '5/1/2013 00:00', '05/31/2013 23:59', 130445
    
    
    select * from eq_extract..extractlog order by date_added desc

	SELECT * FROM EQ_Extract..WalmartDisposalExtract where date_added = '2013-07-05 12:35:38.760' and state_waste_code_2 <> '' and site_code = '2954'
	SELECT * FROM EQ_Extract..WalmartDisposalExtract where date_added = '2013-07-03 14:17:18.940' and state_waste_code_2 <> '' and site_code = '2954'
...
Output Routines:
    declare @extract_id int = 837 -- (returned above)
			...
			sp_rpt_extract_walmart_disposal_output_validation2_jpb 799
			...
			sp_rpt_extract_walmart_disposal_output_trip_jpb 850
...
Changes to WM Format Extract per WM... GEM:13064
	...
	12/07/2010 - JPB
		...
		5. Typical after hours run: 3min, 42s.
			...
			-- Disposal Validation output
			sp_rpt_extract_walmart_disposal_output_validation1 514

			-- Disposal vs Trip Validation output
			sp_rpt_extract_walmart_disposal_output_validation2 514
			
			-- Generators List (Validation) output
			sp_rpt_extract_walmart_disposal_output_generators 514

			-- Manifest output
			sp_rpt_extract_walmart_disposal_output_manifests 514

			-- EQ format of data output
			sp_rpt_extract_walmart_disposal_output_eqformat 514

			-- WM format of data output
			sp_rpt_extract_walmart_disposal_output_wmformat 514

			-- Trip Export format of data output
			sp_rpt_extract_walmart_disposal_output_trip 514
	...
	1/12/2011 - JPB
		1. 	Bug
		Msg 8115, Level 16, State 6, Procedure sp_rpt_extract_walmart_disposal, Line 1687
   ...
   8/18/2011 - JPB
      Added Export To Excel Code
      
   9/14/2011 - JPB
      Converted #ApprovalNoWorkorder cases to use plt_ai..WalmartApprovalCodeRule instead.
      Updated rounding per project request, hundredths for some approvals, 10ths for rest.
      OH yeah, by the way... anything in the generation log that isn't rounding to hundredths should be whole numbers now.  Woo.
      
   10/13/2011 - JPB
		Modified to quit multiplying pounds + ounces * qty.
		Per rob, the only change to the calculation was the merchandise_qty is no longer a multiplier, so it's commented out now.
		There was no past data conversion.
		Gah, testing shows this getting blocked constant. why??? Tempdb. ## table suck.
		WMPHW01U approvals should round to hundredth of pound - Brie.
			- convert all rounding before the plisted check to 2 decimal places.
			- at the time of the plisted check, convert these approvals to 2 decimal places, plisted to 1, else to 0
      
    12/15/2011 - JPB
		Modified the order of sequence_id's in the #WM_Waste_codes table to
		match ordering by P codes first, then alphabetically.
		
	1/17/2012 - JPB
		Adding handling of Empty Bottle Approval calculations so we can get 9-decimal place numbers.
		UPDATE plt_ai..WalmartApprovalCodeRule set description = '9-digit weight reporting') where approval_code = 'WMPHW01'
	
	03/22/2012 - SK
		Added a fix to the waste code renumbering logic from 12/15/2011, the partition to restart numbering needs to be on 
		source, receipt, line & waste_code_origin. So that federal origin waste codes and state origin codes are numbered in diff partitions
		otherwise state waste code gets squenceid 1(as starts with a number) and federal gets sequence id 2 instead of 1.
		Changed the line .."row_number() OVER (PARTITION BY ..."
		
	08/14/2012 - JPB
		Change per Brie: Exclude billing_project_id 5486.
		
	11/01/2012 - JPB
		String format changes per Brie...
		
			On the WM Disposal - WM Format extract run on EQIP (Shipment Data Extract) please make the following format adjustments:

			DOT Description
			Remove RQ Reason on all profiles, except WMHW01 and those for customer #12650
			Do not auto-populate commas in shipping description if UN/NA number, packaging group or hazard class are not entered.
			Example: approval WMHW07, WMHW09 - no packaging group, no comma needed; WMHW19S4 UN number not entered in field because starts with zero and system won't currently recognize, remove comma; WMNHW01 doesn't have a UN/NA number do not automatically enter
 in commas in front of shipping description

			Management Code
			Change management codes that list LIW to NONE

			These items are required to be manually adjusted prior to submission of report to Walmart Corp.
		
	5/13/2013 - JPB
			To clarify treat all approvals to a hundredth of a pound except the WMPHW01 for empty bottles which reports to 9 decimal places.
			Thanks, Brie
			- THIS MEANS NO MORE rounding up to 1 for weights between 0.0001 and 1
			
	5/14/2013 - JPB
			In response to bad data entry where many ReceiptDetailItem months records are entered with no weights, 
			We have changed the GenerationLog export routine to NOT EXPORT RECORDS WHERE THE SUM OF THE WEIGHT FOR THE MONTH = 0.
			
	5/21/2013 - JPB
			The 5/13 change of weight handling affected both the disposal extract and the generation log - should only
			have affected the generation log.  Revert revert revert.

	7/3/2013 - JPB
			Modified the existing manifest-line count validation so that we look for cases with counts > 1, instead of > 2.
			
	7/5/2013 - JPB			
			Modified waste code order statements (rank) so that state waste codes for the generator's own state come before
			other states' waste codes in output.
			
	9/10/2013 - JPB
			TX Waste Code changes - added waste_code_uid and tsdf_state to the #WM_Waste_Cdoes table so we can filter out
			state waste codes that  don't belong to the generator/tsdf states.  Converted joins to use _uid
			Did not convert to display_name yet.. .not ready yet.

	9/24/2013 - JPB
			Phase 2 should be ok for using display_name for waste_codes.  Added it here.
	8/22/2014 - JPB	- GEM:-29706 - Modify Validations: ___ Not-Submitted only true if > $0

	01/28/2015 - JPB - GEM:31327 - Changed vendor names to 'USE'
	
	09/19/2016 - JPB
			Re: change made on 03/22/2012... Found the partition logic did not include company_id, profit_ctr_id.
			This means in the very unlikely case 3 1/2 years later when there are TWO instances of the SAME receipt_id, line_id
			in the extract set from DIFFERENT companies, their waste code sequences will get confused and create goofy data.
			True story.  It finally happened.  Simple fix: add company_id, profit_ctr_id to the partition field list.
...
if @report_log_id is not null and len(@usr) > 10
   select @usr = user_code from plt_ai..reportlog where report_log_id = @report_log_id
...
null as state_waste_code_4,     -- Populated later
    ...
    replace(
      CASE 
          WHEN t.disposal_service_id = (select disposal_service_id from plt_ai.dbo.DisposalService (nolock) where disposal_service_desc = 'Other') THEN 
...
FROM PLT_AI..WorkOrderHeader w (nolock) 
INNER JOIN PLT_AI..WorkOrderDetail d  (nolock) ON w.workorder_id = d.workorder_id and w.company_id = d.company_id and w.profit_ctr_id = d.profit_ctr_id
INNER JOIN plt_ai..Generator g  (nolock) ON w.generator_id = g.generator_id
LEFT OUTER JOIN PLT_AI..workorderdetailunit u (nolock) on d.workorder_id = u.workorder_id and d.sequence_id = u.sequence_id and d.company_id = u.company_id and d.profit_ctr_id = u.profit_ctr_id and u.billing_flag = 'T'
LEFT OUTER JOIN plt_ai..BillUnit b  (nolock) ON isnull(u.bill_unit_code, d.bill_unit_code) = b.bill_unit_code
LEFT OUTER JOIN plt_ai..TSDFApproval t  (nolock) ON d.tsdf_approval_id = t.tsdf_approval_id
    AND d.company_id = t.company_id
    AND d.profit_ctr_id = t.profit_ctr_id
LEFT OUTER JOIN plt_ai..TSDF t2  (nolock) ON d.tsdf_code = t2.tsdf_code
LEFT OUTER JOIN plt_ai..GeneratorSiteType gst  (nolock) ON g.site_type = gst.generator_site_type
LEFT OUTER JOIN plt_ai..WorkOrderTransporter wot1 (nolock) ON w.workorder_id = wot1.workorder_id and w.company_id = wot1.company_id and w.profit_ctr_id = wot1.profit_ctr_id and d.manifest = wot1.manifest and wot1.transporter_sequence_id = 1
LEFT OUTER JOIN plt_ai..WorkOrderTransporter wot2 (nolock) ON w.workorder_id = wot2.workorder_id and w.company_id = wot2.company_id and w.profit_ctr_id = wot2.profit_ctr_id and d.manifest = wot2.manifest and wot2.transporter_sequence_id = 2
LEFT OUTER JOIN plt_ai..DisposalService ds  (nolock) ON t.disposal_service_id = ds.disposal_service_id
LEFT OUTER JOIN plt_ai..WorkOrderStop wos (nolock) ON wos.workorder_id = w.workorder_id
...
AND (w.customer_id IN (select customer_id from #Customer)
    OR w.generator_id IN (SELECT generator_id FROM plt_ai..customergenerator  (nolock) WHERE customer_id IN (select customer_id from #Customer))
    OR w.generator_id IN (
        SELECT generator_id FROM plt_ai..generator (nolock) where site_type IN (
...
*/
    ...
    select distinct
        ...
        @usr,
        @extract_datetime        
    from plt_ai..workorderheader wo (nolock) 
    inner join plt_ai..billinglinklookup bll  (nolock) on
        wo.company_id = bll.source_company_id
        and wo.profit_ctr_id = bll.source_profit_ctr_id
        and wo.workorder_id = bll.source_id
    inner join plt_ai..receipt r  (nolock) on bll.receipt_id = r.receipt_id
        and bll.profit_ctr_id = r.profit_ctr_id
        and bll.company_id = r.company_id
    left outer join plt_ai..receipttransporter rt1  (nolock) on rt1.receipt_id = r.receipt_id
        and rt1.profit_ctr_id = r.profit_ctr_id
        and rt1.company_id = r.company_id
        and rt1.transporter_sequence_id = 1
    left outer join plt_ai..receipttransporter rt2  (nolock) on rt2.receipt_id = r.receipt_id
        and rt2.profit_ctr_id = r.profit_ctr_id
        and rt2.company_id = r.company_id
        and rt2.transporter_sequence_id = 2
    inner join plt_ai..generator g  (nolock) on r.generator_id = g.generator_id
    where
        wo.start_date between @start_date AND @end_date
        and (1=0
            or wo.customer_id IN (select customer_id from #Customer)
            or wo.generator_id in (select generator_id from plt_ai..customergenerator  (nolock) where customer_id IN (select customer_id from #Customer))
            OR wo.generator_id IN (SELECT generator_id FROM plt_ai..generator  (nolock) where site_type IN (SELECT site_type from #SiteTypeToInclude))
            or r.customer_id IN (select customer_id from #Customer)
            or r.generator_id in (select generator_id from plt_ai..customergenerator  (nolock) where customer_id IN (select customer_id from #Customer))
            OR r.generator_id IN (SELECT generator_id FROM plt_ai..generator  (nolock) where site_type IN (SELECT site_type from #SiteTypeToInclude))
        )
    ...
    where
        ...
        and not exists (
            select receipt_id from plt_ai..billinglinklookup bll (nolock) 
...
SELECT distinct
    ...
    (select epa_id from plt_ai..profitcenter  (nolock) where company_id = r.company_id and profit_ctr_id = r.profit_ctr_id) AS receiving_facility_epa_id,
    wrt.receipt_id,
    replace(
...
FROM plt_ai..Receipt r (nolock) 
INNER JOIN plt_ai..ReceiptPrice rp  (nolock) ON
    R.receipt_id = rp.receipt_id
    and r.line_id = rp.line_id
    and r.company_id = rp.company_id
    and r.profit_ctr_id = rp.profit_ctr_id
INNER JOIN plt_ai..Generator g  (nolock) ON r.generator_id = g.generator_id
INNER JOIN plt_ai..BillUnit b  (nolock) ON rp.bill_unit_code = b.bill_unit_code
...
INNER JOIN plt_ai..ProfitCenter pr (nolock) on r.company_id = pr.company_id and r.profit_ctr_id = pr.profit_ctr_id
LEFT OUTER JOIN plt_ai..Profile p  (nolock) ON r.profile_id = p.profile_id
LEFT OUTER JOIN plt_ai..TreatmentDetail tr  (nolock) ON r.treatment_id = tr.treatment_id
	AND r.company_id = tr.company_id and r.profit_ctr_id = tr.profit_ctr_id
LEFT OUTER JOIN plt_ai..GeneratorSiteType gst  (nolock) ON g.site_type = gst.generator_site_type
LEFT OUTER JOIN plt_ai..ProfileQuoteApproval pqa  (nolock)
    on r.profile_id = pqa.profile_id 
    and r.company_id = pqa.company_id 
    and r.profit_ctr_id = pqa.profit_ctr_id 
LEFT OUTER JOIN plt_ai..DisposalService ds  (nolock)
...
INSERT EQ_TEMP.dbo.WalmartDisposalExtract
	...
	INNER JOIN plt_ai..Generator g  (nolock) ON w.generator_id = g.generator_id
    LEFT OUTER JOIN plt_ai..GeneratorSiteType gst  (nolock) ON g.site_type = gst.generator_site_type
    LEFT OUTER JOIN plt_ai..WorkOrderStop wos (nolock) ON w.workorder_id = wos.workorder_id and w.company_id = wos.company_id and w.profit_ctr_id = wos.profit_ctr_id 
    	and wos.stop_sequence_id = 1
	WHERE 1=1
	AND (w.customer_id IN (select customer_id from #Customer)
	    OR w.generator_id IN (SELECT generator_id FROM plt_ai..customergenerator  (nolock) WHERE customer_id IN (select customer_id from #Customer))
	    OR w.generator_id IN (SELECT generator_id FROM plt_ai..generator  (nolock) where site_type IN (SELECT site_type from #SiteTypeToInclude))
	)
	AND w.start_date BETWEEN @start_date AND @end_date
...
FROM 
	...
	left outer join plt_ai..Transporter t1 (nolock)
		on e.transporter1_code = t1.transporter_code
	left outer join plt_ai..Transporter t2 (nolock)
		on e.transporter2_code = t2.transporter_code
WHERE e.added_by = @usr AND e.date_added = @extract_datetime and e.submitted_flag = 'T'

-- 11/01/2012 - JPB
-- Format DOT Description per Brie:
	-- Remove RQ Reason on all profiles, except WMHW01 and those for customer #12650
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = left(dot_description, charindex(', (RQ', dot_description)) where dot_description like '%, (RQ%' and approval_or_resource <> 'WMHW01' and customer_id <> 12650 AND added_by = @usr AND date_added = @extract_datetime
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = left(dot_description, charindex(', RQ', dot_description)) where dot_description like '%, RQ%' and approval_or_resource <> 'WMHW01' and customer_id <> 12650 AND added_by = @usr AND date_added = @extract_datetime
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = left(dot_description, charindex(',  RQ', dot_description)) where dot_description like '%,  RQ%' and approval_or_resource <> 'WMHW01' and customer_id <> 12650 AND added_by = @usr AND date_added = @extract_datetime

	-- don't print un-needed commas
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = replace(dot_description, ',,', ',') where dot_description like '%,,%' AND added_by = @usr AND date_added = @extract_datetime
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = replace(dot_description, ', ,', ',') where dot_description like '%, ,%' AND added_by = @usr AND date_added = @extract_datetime
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = replace(dot_description, ',  ,', ',') where dot_description like '%,  ,%' AND added_by = @usr AND date_added = @extract_datetime

	-- Trim trailing/leading blanks
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = ltrim(rtrim(dot_description)) WHERE added_by = @usr AND date_added = @extract_datetime

	-- Don't print leading or trailing commas
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = ltrim(rtrim(left(dot_description, len(dot_description)-1))) where dot_description like '%,' AND added_by = @usr AND date_added = @extract_datetime
	update EQ_TEMP.dbo.WalmartDisposalExtract set dot_description = ltrim(rtrim(right(dot_description, len(dot_description)-1))) where dot_description like ',%' AND added_by = @usr AND date_added = @extract_datetime

-- 11/01/2012 - JPB
-- Format Management Description per Brie:
	-- Change management codes that list LIW to NONE
	update EQ_TEMP.dbo.WalmartDisposalExtract set management_code = 'NONE' where management_code = 'LIW' AND added_by = @usr AND date_added = @extract_datetime
...
-- Create a table for waste codes
	...
	create index idx_3 on #WM_Waste_Codes (waste_code_uid)

-- Workorder Waste Codes --(1)
	insert #WM_Waste_Codes (source, tsdf_approval_id, sequence_id, origin, generator_state, waste_code_state, waste_code, waste_code_uid, tsdf_state)
	select distinct
		e.source_table,
	...
	FROM Plt_AI..TSDFApprovalWasteCode xwc (nolock)
		INNER JOIN Plt_AI..WasteCode wc (nolock) ON xwc.waste_code_uid = wc.waste_code_uid
...
-- Workorder Waste Codes (2+)
	...
	WHERE e.source_table = 'Workorder'
		and e.submitted_flag = 'T'
		AND wc.waste_code_origin in ('F', 'S')
		and wc.display_name not in (select waste_code from #WM_Waste_Codes
...
-- Receipt Waste Codes (1)
	insert #WM_Waste_codes (source, receipt_id, line_id, company_id, profit_ctr_id, sequence_id, origin, generator_state, waste_code_state, waste_code, waste_code_uid, tsdf_state)
	select distinct
		e.source_table,
		xwc.receipt_id,
		xwc.line_id,
		...
		, wc.waste_code_uid
		, left(isnull(receiving_facility_epa_id, ''), 2)
	FROM plt_ai..ReceiptWasteCode xwc (nolock)
	INNER JOIN plt_ai..wastecode wc (nolock) on xwc.waste_code_uid = wc.waste_code_uid
	INNER JOIN EQ_TEMP.dbo.WalmartDisposalExtract e (nolock)
...
delete from #WM_Waste_Codes from #WM_Waste_Codes w
...
if @debug > 0 begin
   print 'Waste Codes Cleanup, Finished'
   PRINT 'Elapsed time is ' + convert(varchar(40), datediff(ms, @timer, getdate())) + 'ms'
   Print 'Current step elapsed time is ' + convert(varchar(40), datediff(ms, @steptimer, getdate())) + 'ms'
end
set @steptimer = getdate()
...
**********************************************/
update #WM_Waste_codes SET 
	sequence_id = w.new_order
FROM
	#WM_Waste_codes x INNER JOIN
	(
	SELECT 
		row_number() OVER (PARTITION BY source, company_id, profit_ctr_id, receipt_id, line_id, origin ORDER by CASE WHEN left(waste_code, 1) = 'P' then 1 else 2 end, waste_code) as new_order,
		source			,
		tsdf_approval_id,
		receipt_id		,
		line_id			,
...
FROM
	...
	(
	SELECT 
		row_number() OVER (PARTITION BY source, company_id, profit_ctr_id, receipt_id, line_id, origin ORDER by CASE WHEN isnull(generator_state, '') = isnull(waste_code_state, '') then 1 else case when isnull(tsdf_state, '') = isnull(waste_code_state, '') then 2 else 3 end end, waste_code) as new_order,
		source			,
		tsdf_approval_id,
		receipt_id		,
		line_id			,
...
AND generator_id NOT IN (select generator_id FROM plt_ai..generator (nolock) where status <> 'A')
AND added_by = @usr 
AND date_added = @extract_datetime
...
INSERT EQ_Extract..WMDisposalGeneration
  SELECT distinct
			...
			*/
			  ...
			  ELSE
			  	CASE WHEN NOT EXISTS (
			  		...
			  		INNER JOIN WasteCode wc (nolock) on tawc.waste_code_uid = wc.waste_code_uid
			  		...
			  		AND left(wc.display_name, 1) in ('D', 'F', 'P', 'K', 'U')
			  		AND wc.haz_flag = 'T'
			  	) THEN
			  		'T'
			  	ELSE
	  ...
	  @usr as added_by,
	  @extract_datetime as date_added
      , CASE WHEN CHARINDEX(',P', ',' + waste_code_1 + ',' +waste_code_2 + ',' +waste_code_3 + ',' +waste_code_4 + ',' +waste_code_5 + ',' +waste_code_6 + ',' +waste_code_7 + ',' +waste_code_8 + ',' +waste_code_9 + ',' +waste_code_10 + ',' +waste_code_11
 + ',' +waste_code_12) <= 0 THEN
				''
			ELSE
			'P'
		END AS plisted
		 ...
		 convert(numeric(20,9), 
			(sum(rdi.merchandise_quantity) * (select pound_conv from CustomerTypeEmptyBottleApproval where approval_code = f.approval_or_resource))
			 ...
			 CASE WHEN (
   			...
   			ELSE
   				(
   					select round(isnull(line_weight, 0),2)
   					FROM receipt 
...
SET weight = 
	case when tsdf_approval_code in (select approval_code from WalmartApprovalCodeRule where handling_rule = '9-digit weight reporting') then
		convert(numeric(20,9), weight)
	else
		round(convert(numeric(7,2), weight), 2)
	end
WHERE
...
if @debug > 0 begin
   print 'Updating EQ_Extract..WMDisposalGeneration weights rounded to 1/10 or 1/100th where plisted, Finished'
   PRINT 'Elapsed time is ' + convert(varchar(40), datediff(ms, @timer, getdate())) + 'ms'
   Print 'Current step elapsed time is ' + convert(varchar(40), datediff(ms, @steptimer, getdate())) + 'ms'
end
set @steptimer = getdate()
...
-- One last case discovered 9/15/2011: Generation log records that had no Detail record (ReceiptDetailItem or WorkorderDetailItem) 
-- will have an isnull(weight,0) = 0 condition here.  When that happens, they should use their receipt's line_weight if possible.
UPDATE EQ_Extract..WMDisposalGeneration
SET weight = 
		case when g.tsdf_approval_code in (select approval_code from WalmartApprovalCodeRule where handling_rule = '9-digit weight reporting') then
			convert(numeric(20,9), 
				R.quantity * (select pound_conv from CustomerTypeEmptyBottleApproval where approval_code = g.tsdf_approval_code)
			)
		else
			round(isnull(R.line_weight, 0), 2)
		end
FROM EQ_Extract..WMDisposalGeneration g
INNER JOIN PLT_AI..Receipt r
   ON g.workorder_id = r.receipt_id
   and g.company_id = r.company_id
   and g.profit_ctr_id = r.profit_ctr_id
   and g.sequence_id = r.line_id
   and g.tsdf_approval_code = r.approval_code
WHERE
	g.date_added = @extract_datetime
	  AND g.added_by = @usr
...
-- Create list of missing transporter info
    ...
    FROM EQ_TEMP.dbo.WalmartDisposalExtract (nolock) 
    WHERE 
    	ISNULL((select transporter_name from plt_ai..transporter (nolock) where transporter_code = EQ_TEMP.dbo.WalmartDisposalExtract.transporter1_code), '') = ''
...
-- Create list of missing Service Dates
    ...
    where
	    ...
	    AND approval_or_resource not in (
	    	select approval_code from WalmartApprovalCodeRule where handling_rule = 'No Workorder Required'
	    )
...
-- Create list of receipts missing workorders
    ...
    WHERE
    	...
    	AND approval_or_resource not in (select approval_code from WalmartApprovalCodeRule where handling_rule = 'No Workorder Required')
	    AND added_by = @usr 
	    and date_added = @extract_datetime
	    AND submitted_flag = 'T'
...
-- Catch generators serviced that aren't in the extracts
	...
	LEFT OUTER JOIN plt_ai..WorkOrderStop wos (nolock) ON wos.workorder_id = woh.workorder_id
...
-- Create list of look-alike generators not assigned to WM yet:
    ...
    AND generator_id NOT IN (SELECT generator_id FROM plt_ai.dbo.CustomerGenerator (nolock) WHERE customer_id IN (select customer_id from #Customer))
		 ...
		 SELECT
			...
			wodi_weight = (
				CASE WHEN d.source_table = 'Workorder' THEN (
                    SELECT 
						case when d.approval_or_resource in (select approval_code from WalmartApprovalCodeRule where handling_rule = '9-digit weight reporting') then
						 convert(numeric(20,9), 
							(sum(wodi.merchandise_quantity) * (select pound_conv from CustomerTypeEmptyBottleApproval where approval_code = d.approval_or_resource))
						 )
						else
						  round(convert(numeric(7,2), round(sum( ( (isnull(wodi.pounds,0) * 1.0) + (isnull(wodi.ounces,0)/16.0) )  /* * isnull(wodi.merchandise_quantity,1) */ ),2) ), 2)
              			end
				...
				ELSE -- source_table = 'Receipt'...
				  case when d.approval_or_resource in (select approval_code from WalmartApprovalCodeRule where handling_rule = '9-digit weight reporting') then
				   (
						SELECT 
						 convert(numeric(20,9), sum(rdi.merchandise_quantity) * (select pound_conv from CustomerTypeEmptyBottleApproval where approval_code = d.approval_or_resource))
						FROM
...
set @steptimer = getdate()
	...
	UPDATE #DisposalVsTripValidation SET
		exported_flag = 2 
	WHERE 1=1
	AND abs(round(isnull(receipt_weight, -1111),2) - round(isnull(wodi_weight, -2222),2)) > 1
...
---------------------------------
   ...
   truncate table eq_temp..sp_rpt_extract_walmart_disposal_1	

   insert eq_temp..sp_rpt_extract_walmart_disposal_1	
	SELECT
   	problem,
   	source,
   	company_id,
   	...
   	receipt_id,
   	extra
   FROM EQ_Extract.dbo.WMDisposalValidation
   WHERE
      added_by = @usr
      AND date_added = @extract_datetime
   ORDER BY
   ...
   select @tmp_filename = 'WM Disposal Validation - ' +
   		...
   		+ '.xls',
   	@tmp_desc = 'WM Disposal Validation Export: ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)

   /*	Write to Excel: */
   exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'eq_temp..sp_rpt_extract_walmart_disposal_1',
   	@template	     = 'sp_rpt_extract_walmart_disposal.DisposalValidation',
...
---------------------------------
	...
	-- sp_rpt_extract_walmart_disposal_output_validation2_jpb 850

	truncate TABLE eq_temp..sp_rpt_extract_walmart_disposal_2

   insert eq_temp..sp_rpt_extract_walmart_disposal_2
   SELECT 
   	problem,
   	receipt_company_id,
   	...
   	receipt_weight,
   	wodi_weight,
   	corrected_wodi_weight
   FROM EQ_Extract.dbo.WMDisposalGenerationValidation
   WHERE
       added_by = @usr
       AND date_added = @extract_datetime
   GROUP BY    
   	...
   	corrected_wodi_weight
   ORDER BY
   	CASE WHEN problem like '%calculated%' THEN 2 ELSE 1 END,
   	manifest,
   	receipt_approval

   select @tmp_filename = 'WM Generation Validation - ' +
   		convert(varchar(4), datepart(yyyy, @start_date)) 
   		+ '-'
   		+ right('00' + convert(varchar(2), datepart(mm, @start_date)),2)
   		+ '-'
   		...
   		+ convert(varchar(10), @extract_id)
   		+ '.xls',
   	@tmp_desc = 'WM Generation Validation Export: ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)


   /*	Write to Excel: */
   exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'eq_temp..sp_rpt_extract_walmart_disposal_2',
   	@template	     = 'sp_rpt_extract_walmart_disposal.GenerationValidation',
   	@filename	     = @tmp_filename,
   	@added_by	     = @usr,
   	@export_desc     = @tmp_desc,
   	@report_log_id   = @report_log_id,
...
---------------------------------
	...
	TRUNCATE TABLE eq_temp..sp_rpt_extract_walmart_disposal_3
	
	insert eq_temp..sp_rpt_extract_walmart_disposal_3
   SELECT
       generator_id,
       epa_id,
       generator_name,
   ...
   select @tmp_filename = 'WM Possibly Unlinked Generators - ' +
   		convert(varchar(10), getdate(), 110) 
   		+ ' - '
   		+ convert(varchar(10), @extract_id)
   		+ '.xls',
   	@tmp_desc = 'WM Possibly Unlinked Generators Export: ' + convert(varchar(10), getdate(), 110)

   /*	Write to Excel: */
   exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'eq_temp..sp_rpt_extract_walmart_disposal_3',
   	@template	     = 'sp_rpt_extract_walmart_disposal.GeneratorList',
   	@filename	     = @tmp_filename,
   	@added_by	     = @usr,
   	@export_desc     = @tmp_desc,
   	@report_log_id   = @report_log_id,
...
---------------------------------
-- Manifest output (skipped for a bit)
---------------------------------

   -- sp_rpt_extract_walmart_disposal_output_manifests_jpb 850

---------------------------------
-- EQ format of data output (not used)
---------------------------------
...
---------------------------------
   ...
   -- sp_rpt_extract_walmart_disposal_output_wmformat_jpb 850
	...
	TRUNCATE TABLE eq_temp..sp_rpt_extract_walmart_disposal_4

   INSERT eq_temp..sp_rpt_extract_walmart_disposal_4
   SELECT
      ...
      -- ISNULL(NULLIF(manifest_line, 1), '') AS 'Manifest Line',
      ISNULL(NULLIF(manifest_line, 0), '') AS 'Manifest Line',
      SUM(ISNULL(pounds, 0)) AS 'Weight',
      CASE WHEN waste_desc = 'No waste picked up' THEN
          ISNULL(bill_unit_desc, '')
      ELSE
   	dbo.fn_wm_extract_line_convert(manifest, manifest_line, @usr, @extract_datetime)
          ...
          WHEN 'Other'                THEN 'PROBLEM: Other' + 
               CASE WHEN profile_id is not null then ' - Profile ID: ' + convert(varchar(20), profile_id) else
                    CASE WHEN tsdf_approval_id is not null then ' - TSDF Approval ID: ' + convert(varchar(20), tsdf_approval_id) else
                         'No Profile ID or TSDF Approval ID found for ' + source_table + ' ' + convert(varchar(20), receipt_id)
                    end
               end
                  ...
                  (
                      SELECT -- disposal_service_desc
                        ...
                        CASE WHEN tsdf_approval_id is not null then ' - TSDF Approval ID: ' + convert(varchar(20), tsdf_approval_id) else
                                  ...
                                  END
                              ELSE ISNULL(disposal_service_desc, '') 
                          END
                      ...
                      where pqass.profile_id = WalmartDisposalExtract.profile_id
                          AND pqass.company_id = WalmartDisposalExtract.company_id
                          AND pqass.profit_ctr_id = WalmartDisposalExtract.profit_ctr_id
                  ), 'PROBLEM: TSDF Disposal Service not identified in profile_id: ' + convert(varchar(10), WalmartDisposalExtract.company_id) + '-' + convert(varchar(10), WalmartDisposalExtract.profit_ctr_id) + ':' + convert(varchar(20), WalmartDisposalExtract.profile_id)
              )
          ELSE CASE WHEN waste_desc = 'No waste picked up' THEN '' ELSE ISNULL(disposal_service_desc, '') END
      END AS 'Disposal Method',
      'USE' as 'Vendor Name'
   FROM EQ_Extract.dbo.WalmartDisposalExtract WalmartDisposalExtract
   WHERE
      added_by = @usr
      AND date_added = @extract_datetime
   GROUP BY            
      ...
      receipt_id,
      disposal_service_desc,
      WalmartDisposalExtract.profile_id,
      WalmartDisposalExtract.company_id,
      WalmartDisposalExtract.profit_ctr_id,
      WalmartDisposalExtract.tsdf_approval_id,
      WalmartDisposalExtract.source_table
   ORDER BY
      generator_state,
      generator_city,
      site_code,
   ...
   select @tmp_filename = 'WM Disposal - WM Format - ' +
   		convert(varchar(4), datepart(yyyy, @start_date)) 
   		+ '-'
   		+ right('00' + convert(varchar(2), datepart(mm, @start_date)),2)
   		+ '-'
   		...
   		+ convert(varchar(10), @extract_id)
   		+ '.xls',
   	@tmp_desc = 'WM Disposal - WM Format - Export: ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)

   /*	Write to Excel: */
   exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'eq_temp..sp_rpt_extract_walmart_disposal_4',
   	@template	     = 'sp_rpt_extract_walmart_disposal.DisposalExportWM',
   	@filename	     = @tmp_filename,
   	@added_by	     = @usr,
   	@export_desc     = @tmp_desc,
   	@report_log_id   = @report_log_id,
...
---------------------------------
   ...
   -- sp_rpt_extract_walmart_disposal_output_trip_jpb 850
	...
	TRUNCATE TABLE eq_temp..sp_rpt_extract_walmart_disposal_5

   insert eq_temp..sp_rpt_extract_walmart_disposal_5 
	select distinct   
      wte.site_code as [Facility Number], 
      wte.generator_city as City, 
      wte.generator_state as State,
      wte.shipment_date as [Shipment Date],
      wte.generation_date as [Generation Date],
      wte.epa_id as [Haz Waste Generator EPA ID],
      sum(wte.weight) as Weight,
      wte.tsdf_approval_code as [Waste Profile Number], -- tsdf_approval_code always correct?, looks ok.
      'USE' as [Vendor Name]
	FROM EQ_Extract..WMDisposalGeneration wte
	WHERE wte.added_by = @usr
	AND wte.date_added = @extract_datetime
	AND wte.exclude_flag = 'F'
	group by
      wte.site_code,
      wte.generator_city, 
      wte.generator_state,
      ...
      wte.generation_date,
      wte.epa_id,
      wte.tsdf_approval_code
    having sum(wte.weight) > 0
  order by 
      wte.site_code, 
      wte.generator_city, 
      wte.generator_state,
   ...
   select @tmp_filename = 'WM Generation Log - ' +
   		convert(varchar(4), datepart(yyyy, @start_date)) 
   		+ '-'
   		+ right('00' + convert(varchar(2), datepart(mm, @start_date)),2)
   		+ '-'
   		...
   		+ convert(varchar(10), @extract_id)
   		+ '.xls',
   	@tmp_desc = 'WM Generation Log Export: ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)

   /*	Write to Excel: */
   exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'eq_temp..sp_rpt_extract_walmart_disposal_5',
   	@template	     = 'sp_rpt_extract_walmart_disposal.GenerationLog',
   	@filename	     = @tmp_filename,
   	@added_by	     = @usr,
   	@export_desc     = @tmp_desc,
   	@report_log_id   = @report_log_id,
...
GRANT execute on sp_rpt_extract_walmart_disposal to eqweb
go
GRANT execute on sp_rpt_extract_walmart_disposal to eqai
go
GRANT execute on sp_rpt_extract_walmart_disposal to cor_user
go

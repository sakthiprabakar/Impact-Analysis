-- 
drop proc sp_COR_web_biennial_list_by_access
go

CREATE PROC sp_COR_web_biennial_list_by_access (
...
/* ********************************************************************
sp_COR_web_biennial_list_by_access
	Customer Service designated "biennial" report for web users.
	Not necessarily a REAL "biennial" report, but similar info.

IMPORANT:
	This "original" version of the COR Biennial report limits users to their
	ContactCORReceiptBucket access -- it does not take into account profile.orig_customer_id
	access, so it's not going to show MSG customers whose receipts are transacted
	under an "internal" customer number that website users don't have access to.
	For a broader version see sp_COR_web_biennial_list_by_bucket which uses
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_COR_web_biennial_list_by_access] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_COR_web_biennial_list_by_access] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_COR_web_biennial_list_by_access] TO [EQAI]
    AS [dbo];

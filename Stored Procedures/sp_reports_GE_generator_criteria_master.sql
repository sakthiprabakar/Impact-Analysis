...
create proc sp_reports_GE_generator_criteria_master (
...
as
/* ****************************************************************************
sp_reports_GE_generator_criteria_master
...
-- Handle text inputs into temp tables
	...
	INSERT #CustomerInput SELECT convert(int, row) from dbo.fn_SplitXsvText(',', 1, @customer_id_list) WHERE ISNULL(row, '') <> ''

	CREATE TABLE #GeneratorInput (generator_id int)
	INSERT #GeneratorInput SELECT convert(int, row) from dbo.fn_SplitXsvText(',', 1, @generator_id_list) WHERE ISNULL(row, '') <> ''
...
-- figure out if this user has inherent access to customers
	...
	BEGIN
		if not (@customer_id_list = '-1' and @generator_id_list = '-1' and @site_type_list = '' and @generator_state_list = '' and @generator_country_list = '') 
		begin
			insert #generator
			select cg.generator_id
			from CustomerGenerator cg
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_GE_generator_criteria_master] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_GE_generator_criteria_master] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_GE_generator_criteria_master] TO [EQAI]
    AS [dbo];

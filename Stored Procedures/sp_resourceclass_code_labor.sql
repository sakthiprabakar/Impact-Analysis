...
/****** Object:  StoredProcedure [dbo].[sp_resourceclass_code_labor]    Script Date: 1/6/2022 8:50:39 AM ******/
SET ANSI_NULLS ON
GO
...
DROP procedure IF EXISTS [dbo].[sp_resourceclass_code_labor]

GO
CREATE PROCEDURE sp_resourceclass_code_labor
...
the "labor" button on Work Order. It fetches the codes based on following order:
...
Filename:           F:\EQAI\SQL\EQAI\sp_resourceclass_code_labor.sql
PB Object(s): d_quick_select_labor
Load on PLT_AI
...
BEGIN
       ...
       INNER JOIN ResourceClassHeader 
        ...
        WHERE WorkorderQuoteDetail.resource_type = 'L'
...
INNER JOIN ProfitCenter
	ON ProfitCenter.company_id = ResourceClassDetail.company_id
	AND ProfitCenter.profit_ctr_id = ResourceClassDetail.profit_ctr_id
WHERE ResourceClass.resource_type = 'L'
       AND ResourceClass.status = 'A'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_resourceclass_code_labor] TO [EQAI]
    AS [dbo];

CREATE PROCEDURE sp_flash_report_trip_forecast2
...
------------------------------------------------------------------------------------
    ...
    sp_flash_report_trip_forecast2 history:
...
06/16/2023 Devops 65744 -- Nagaraj M Modified the input parameter @copc_list varchar(500) to @copc_list varchar(max)
		...
		sp_flash_report_trip_forecast2 '3-04-2007 00:00:00','3-6-2007 23:59:59', 10673, 10673, 0 , '99', 1
		sp_flash_report_trip_forecast2 '11-01-2006 00:00:00','3-21-2007 23:59:59', 10000, 10673, 0 , '99', 1

SAMPLE:
sp_flash_report_trip_forecast2 
	@StartDate		          = '1/4/2012', -- datetime = NULL,
	@EndDate		          = '2/1/2012', -- datetime = NULL,
    ...
    -- Apply surcharges
	    ...
	    -- ENSR
			...
			IF @ensr_flag = 'T' OR @ensr_flag = 'P'
				...
				update #tmp_revenue set ensr_amt = ROUND((r.revenue) * (ISNULL(dbo.fn_get_ensr_percent(wom.date_delivered, woh.customer_id), 0) / 100), 2)
				FROM #tmp_revenue r
				INNER JOIN WorkOrderDetail wod (nolock) 
					ON r.workorder_id = wod.workorder_id
					AND r.company_id = wod.company_id
...
BEGIN
	...
	BEGIN
		...
		BEGIN
			...
			BEGIN
				update #tmp_line SET 
					ensr_amt = ROUND(((w.revenue * ISNULL(dbo.fn_get_ensr_percent(r.receipt_date, r.customer_id), 0)) / 100), 2), progress_flag = 2
				FROM #tmp_line w
				INNER JOIN Receipt r (nolock) 
...
SELECT 
	...
	t.quantity,
	t.bill_unit_code,
	wos.date_act_arrive as trip_act_arrive,
	wos.date_act_depart as trip_act_departure,
	t.account_desc,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_flash_report_trip_forecast2] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_flash_report_trip_forecast2] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_flash_report_trip_forecast2] TO [EQAI]
    AS [dbo];

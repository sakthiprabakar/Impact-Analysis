drop PROCEDURE if exists sp_resourceclass_code_other
go
CREATE PROCEDURE sp_resourceclass_code_other
...
the "Other" button on Work Order. It fetches the codes based on following order:
...
Filename:		F:\EQAI\SQL\EQAI\sp_resourceclass_code_other.sql
PB Object(s):	d_quick_select_other_quote
Load on PLT_AI
		
06/23/2011 SK	Created SQL Server's implicit conversion from string to ints is used
07/09/2018 AM	GEM:52074 - Added emanifest_submission_type_uid join.
12/27/2021 AM	DevOps:29874 - Instead of @project_quote_id we are using @customer_id
05/26/2022 MPM  DevOps 38903 - Added item_number and pc_use_resource_class_item_flag
                to the result set.

sp_resourceclass_code_other '14', '00', '1178487', '163'
****************************************************************************/
...
BEGIN
	...
	INNER JOIN ResourceClassHeader 
		...
		WHERE WorkorderQuoteDetail.resource_type = 'O'
...
BEGIN
	...
	INNER JOIN ResourceClassHeader 
		...
		WHERE WorkorderQuoteDetail.resource_type = 'O'
		AND ResourceClass.status = 'A'
		AND WorkOrderQuoteDetail.quote_id = @project_quote_id --@customer_id
		AND WorkOrderQuoteDetail.company_id = @company_id
		AND WorkOrderQuoteDetail.profit_ctr_id = @profit_ctr_id
...
INNER JOIN ProfitCenter
	ON ProfitCenter.company_id = ResourceClassDetail.company_id
	AND ProfitCenter.profit_ctr_id = ResourceClassDetail.profit_ctr_id
	WHERE ResourceClass.resource_type = 'O'
	AND ResourceClass.status = 'A'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_resourceclass_code_other] TO [EQAI]
    AS [dbo];

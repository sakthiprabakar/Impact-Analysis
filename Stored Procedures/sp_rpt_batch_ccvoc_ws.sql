DROP PROCEDURE IF EXISTS sp_rpt_batch_ccvoc_ws
GO

CREATE PROCEDURE sp_rpt_batch_ccvoc_ws
	@date_from		datetime,
	@date_to		datetime,
	@location		varchar(15),
...
AS
/********************************************************************************************************************************
Filename:		L:\Apps\SQL-Deploy\Prod\NTSQL1\PLT_XX_AI\Procedures\sp_rpt_batch_ccvoc_ws.sql
PB Object(s):	d_rpt_batch_ccvoc_worksheet
...
11/23/2009	JDB	Created; copied from sp_rpt_ccvoc_ws
06/24/2014	AM	Moved to plt_ai
04/20/2023	MPM	DevOps 27244 - Modified the calcuation of pounds_emission so that it's done in the same way as was done to the 
				HAP Report under DevOps 17354.

SELECT * FROM BatchTransferReceipt
sp_rpt_batch_ccvoc_ws '11-1-2009', '11-27-2009', 'TDU', '100', 2, 21
sp_rpt_batch_ccvoc_ws '11-1-2009', '11-27-2009', 'TDU', '200', 2, 21
sp_rpt_batch_ccvoc_ws '11-1-2009', '11-27-2009', 'TDU', '201', 2, 21
********************************************************************************************************************************/
...
INNER JOIN Container c ON r.company_id = c.company_id
	AND r.profit_ctr_id = c.profit_ctr_id
	AND r.receipt_id = c.receipt_id
	AND r.line_id = c.line_id
INNER JOIN ContainerDestination d ON r.company_id = d.company_id
...
INNER JOIN ContainerDestination d ON r.company_id = d.company_id
	...
	AND r.line_id = d.line_id
	AND c.container_id = d.container_id
INNER JOIN BatchTransferReceipt btr ON r.company_id = btr.company_id
...
/*	MPM - 4/21/2023 - DevOps 27244 - Corrected the calculation of emissions.  
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_batch_ccvoc_ws] TO [EQAI];

if exists (select 1 from sysobjects where type = 'P' and name = 'sp_rapidtrak_save_container_size')
	drop procedure sp_rapidtrak_save_container_size
go

create procedure sp_rapidtrak_save_container_size
	@container			varchar(20),
	@new_container_size	varchar(15),
	@user_id			varchar(10)
as
--
--exec sp_rapidtrak_save_container_size
--
...
declare @company_id int,
	...
	@original_size varchar(15),
	@status varchar(7),
	@msg varchar(255)

set @status = 'OK'
set @msg = 'Container size updated successfully.'
...
select @original_size = coalesce(container_size,'')
from Container
where company_id = @company_id
and profit_ctr_id = @profit_ctr_id
and receipt_id = @receipt_id
...
if @original_size <> @new_container_size
begin
	...
	values (@company_id, @profit_ctr_id, @container_type, @receipt_id, @line_id, @container_id, 1,
			'container_size', @original_size, @new_container_size, getdate(), @user_id, 'RT', 'Container')
	...
	update Container  
	set container_size = @new_container_size,
		modified_by = @user_id,
...
grant execute on sp_rapidtrak_save_container_size to eqai
go

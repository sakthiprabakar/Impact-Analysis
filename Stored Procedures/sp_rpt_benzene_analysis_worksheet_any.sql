...
CREATE PROCEDURE sp_rpt_benzene_analysis_worksheet_any
	@company_id			int
,   @profit_ctr_id		int
,	@date_from			datetime
,	@date_to			datetime
...
AS
/***********************************************************************
This procedure runs for Benzene NESHAP TAB Worksheet

PB Object(s):	r_benzene_analysis_worksheet_any

11/18/2010 SK	Created on Plt_AI
02/03/2012 SK   Added join to Receipt to fetch Receipt Date in the result set
				Added left outer join to proces location
02/01/2013 RB   Added DISTINCT because join to ContainerDestination was retrieving duplicates
05/08/2013 SK	1)Changed to determine the value of tranship by using the check for Treatment process of Tranship
				...
				3)The treatment is taken from the Container. The report was changed to retrieve per container and then group by location and tranship flag
				...
				4)Also receipt_id, line_id fields were added to the output and 'DISTINCT' was removed from the base query as now 
...
sp_rpt_benzene_analysis_worksheet_any 21,0, '2012-01-01', '2012-12-31', 0, 'ZZZZZZZZ', '0', 'ZZZZZZZZZZZZ'
***********************************************************************/
...
WHERE	(@company_id = 0 OR Receipt.company_id = @company_id)
	...
	AND ( ProfileLab.benzene > 0 OR Generator.TAB > 0 )
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_benzene_analysis_worksheet_any] TO [EQAI]
    AS [dbo];

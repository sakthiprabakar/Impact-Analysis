/***************************************************************************************
E-waste Monthly Operating Report (MOR) detail report
Filename:	F:\EQAI\SQL\EQAI\sp_rpt_ewaste_mor_detail.sql
PB Object(s):	d_rpt_ewaste_mor_detail
...
sp_rpt_ewaste_mor_detail '1/1/05', '3/31/05', 21
****************************************************************************************/
CREATE PROCEDURE sp_rpt_ewaste_mor_detail
	@date_from	datetime,
	@date_to	datetime,
	@profit_ctr_id	int,
	@company_id int
...
SELECT	ISNULL(LTRIM(RTRIM(ProfitCenter.EPA_ID)),'') AS EQ_Site_ID,   
	...
	SUM(Receipt.manifest_quantity) AS quantity,   
...
WHERE Receipt.Receipt_date BETWEEN @date_from AND @date_to
	...
	AND IsNull(Receipt.manifest_quantity, 0) > 0
...
GROUP BY ProfitCenter.EPA_ID, 
	...
	IsNull(LTrim(RTrim(Receipt.waste_code)),''), 
	Receipt.manifest_unit,
	IsNull(LTrim(RTrim(treatment.management_code)),''), 
	Receipt.receipt_id, 
	Receipt.line_id
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_ewaste_mor_detail] TO [EQAI]
    AS [dbo];

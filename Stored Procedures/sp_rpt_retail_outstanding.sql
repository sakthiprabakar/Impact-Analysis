...
CREATE PROCEDURE sp_rpt_retail_outstanding 
	@company_id			int
,	@profit_ctr_id		int
,	@date_from			datetime
,	@date_to			datetime
...
/***********************************************************************
PB Object(s):	r_retail_outstanding
	
11/10/2010 SK	Created on Plt_AI

sp_rpt_retail_outstanding 14, -1, '05/01/2010', '05/31/2010', 1, 999999, '0', 'zzzzzzz', 1, 99999, 'GA'
***********************************************************************/
...
,	OrderDetail.price
,	Sum(1) as quantity
,	Sum(OrderDetail.price) as extended_amt
...
JOIN OrderItem
	...
	AND OrderItem.date_returned IS NULL
	AND OrderItem.date_shipped BETWEEN @date_from AND @date_to
JOIN Product
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_retail_outstanding] TO [EQAI]
    AS [dbo];

...
CREATE PROCEDURE [dbo].[sp_flash_report_outstanding_wnotes]  
	@company_id		int
,	@profit_ctr_id	int
,	@date_from		datetime
,	@date_to		datetime 
...
Filename:	L:\Apps\SQL\EQAI\sp_flash_report_outstanding_wnotes.sql
PB Object(s):	d_rpt_flash_report_outstanding_wnotes

02/16/2006 RG	Created (copied from sp_flash_report_outstanding)
11/07/2006 RG   Revised to show only note notes not audit notes
...
sp_flash_report_outstanding_wnotes 14, -1, '10/01/2007','10/31/2007', 1, 999999, '99', 1
****************************************************************************************************/
...
-- Create a holding table for the financial results so you can join this to the notes

-- Return the results
INSERT #tmp_rev_rslts
...
WHERE billing_status is null or billing_status not in ( 'I','Z')
GROUP BY 
	t.workorder_status,
	t.billing_status, 
	t.account_desc, 
...
FROM #tmp_rev_rslts
JOIN Note N
	ON N.company_id = #tmp_rev_rslts.company_id
	AND N.profit_ctr_id = #tmp_rev_rslts.profit_ctr_id
	AND N.workorder_id = #tmp_rev_rslts.workorder_id
...
FROM #tmp_rev_rslts r
LEFT OUTER JOIN #tmp_notes n
	ON n.workorder_id = r.workorder_id
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_flash_report_outstanding_wnotes] TO [EQAI]
    AS [dbo];

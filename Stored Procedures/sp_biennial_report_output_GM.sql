-- if exists (select 1 from sysobjects where name = 'sp_biennial_report_output_GM') drop proc sp_biennial_report_output_GM 
go

CREATE PROCEDURE sp_biennial_report_output_GM (
	@biennial_id	int,
	@state			varchar(2)
)
AS
...
1/21/2015 JPB	Added @int_* variables to avoid parameter sniffing slowness
*********************************************************************************** */
declare
...
create table #GMRows (rowid bigint)
insert #GMRows
SELECT DISTINCT
SD.rowid
...
SD.rowid
FROM EQ_Extract..BiennialReportSourceData SD
LEFT JOIN Generator G on SD.EPA_source_code in ('G62')
	and SD.eq_generator_id = G.generator_id
LEFT JOIN Country GC on GC.country_code = G.generator_country
WHERE biennial_id = @int_biennial_id
AND SD.TRANS_MODE = 'I'
AND SD.profit_ctr_epa_id = SD.generator_epa_id
AND SD.generator_country = 'US'


DELETE FROM EQ_Extract..BiennialReportWork_GM1 where biennial_id = @int_biennial_id

INSERT EQ_Extract..BiennialReportWork_GM1
SELECT DISTINCT
	...
	LEFT(SD.EPA_source_code + space(3), 3) as SOURCE_CODE,
		-- EPA Source Code
		-- Starts at column: 35
		-- Field length: 3
		-- Data type: Alphanumeric

	SUBSTRING(REPLICATE(' ', 18 - DATALENGTH(CONVERT(VARCHAR(18),(CONVERT(DECIMAL(18,6), SUM(SD.lbs_haz_estimated))))))
	+ CONVERT(VARCHAR(18),(CONVERT(DECIMAL(18,6), SUM(SD.lbs_haz_estimated)))), 1, 18) as GEN_QTY,
	...
	CASE WHEN (SD.trans_mode = 'I' AND SD.profit_ctr_epa_id = SD.generator_epa_id AND SD.generator_country = 'US') THEN 'Y' ELSE 'N' END as ON_SITE_MANAGEMENT,
	...
	'N' as MIXED_WASTE,
		-- Added in 2019, is this waste Hazardous mixed with a nuclear product, byproduct, etc.
		-- Starts at 539
		-- Field length: 1
		-- Data type: Alphanumeric
	...
	case when SD.EPA_source_code in ('G62') then GC.country_code else '  ' end as COUNTRY_CODE
		-- Added in 2019, for source code 'G62' only, what is the 2 digit foreign country code where the waste... arrived from.
		-- 'G62' = Hazardous Waste Received from a Foreign Country

FROM #GMRows xx
JOIN EQ_Extract..BiennialReportSourceData SD
on xx.rowid = SD.rowid
LEFT JOIN Generator G on SD.EPA_source_code in ('G62')
	and SD.eq_generator_id = G.generator_id
LEFT JOIN Country GC on GC.country_code = G.generator_country
WHERE biennial_id = @int_biennial_id
...
GROUP BY
	...
	case when SD.EPA_source_code in ('G62') then GC.country_code else '  ' end


-- Now number the rows --------------------------------------
declare @intcounter int
set @intcounter = 0
update EQ_Extract..BiennialReportWork_GM1
...
DELETE FROM EQ_Extract..BiennialReportWork_GM2 where biennial_id = @int_biennial_id

INSERT EQ_Extract..BiennialReportWork_GM2
SELECT DISTINCT
...
FROM EQ_Extract..BiennialReportWork_GM1 GM1
	JOIN EQ_Extract..BiennialReportSourceData SD ON ( GM1.biennial_id = SD.biennial_id
		AND GM1.APPROVAL_CODE = SD.APPROVAL_CODE
		AND GM1.DESCRIPTION = SD.WASTE_DESC 
			--AND SD.TRANS_MODE = 'O')
			AND SD.rowid in (select rowid from #GMRows)
		)
	JOIN EQ_Extract..BiennialReportSourceWasteCode SW ON ( SD.biennial_id = SW.biennial_id
		...
		AND SW.origin = 'F')
WHERE GM1.biennial_id = @int_biennial_id
...
-- Re-enabled 2/20/2012 - JPB when this became not JUST Ohio.
...
DELETE FROM EQ_Extract..BiennialReportWork_GM3 where biennial_id = @int_biennial_id

INSERT EQ_Extract..BiennialReportWork_GM3
SELECT DISTINCT
...
DELETE FROM EQ_Extract..BiennialReportWork_GM4 where biennial_id = @int_biennial_id

INSERT EQ_Extract..BiennialReportWork_GM4
SELECT DISTINCT
	...
	SUBSTRING(REPLICATE(' ', 18 - DATALENGTH(CONVERT(VARCHAR(18),(CONVERT(DECIMAL(18,6),SUM(SD.lbs_haz_estimated))))))
	+ CONVERT(VARCHAR(18),(CONVERT(DECIMAL(18,6),SUM(SD.lbs_haz_estimated)))), 1, 18) as IO_TDR_QTY
...
from
	EQ_Extract..BiennialReportWork_GM1 GM1
...
--declare @intcounter int
set @intcounter = 0
update EQ_Extract..BiennialReportWork_GM4
...
DELETE FROM EQ_Extract..BiennialReportWork_GM5 where biennial_id = @int_biennial_id

INSERT EQ_Extract..BiennialReportWork_GM5
SELECT DISTINCT
...
--declare @intcounter int
set @intcounter = 0
update EQ_Extract..BiennialReportWork_GM5
set @intcounter = SYS_PG_NUM_SEQ_tmp = @intcounter + 1
where biennial_id = @int_biennial_id
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_biennial_report_output_GM] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_biennial_report_output_GM] TO [COR_USER]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_biennial_report_output_GM] TO [EQAI]
    AS [dbo];
GO

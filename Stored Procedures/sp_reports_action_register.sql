-- drop proc sp_reports_action_register 
go

create proc sp_reports_action_register (
	@generator_id		int
...
as
/* ****************************************************************************
sp_reports_action_register

sp_reports_action_register 169151
**************************************************************************** */

set @status_list = replace(@status_list, '|', ',')
create table #status (status	char(1))
insert #status select left(row,1) from dbo.fn_SplitXsvText(',',1,@status_list) WHERE row is not null and @status_list like '%,%'
...
-- Action Register: Supplier Corrective Action Report
	...
	and (
		isnull(@report_type_list, '') in ('all', '')
		OR
			ar.action_type_id = case 
				when @report_type_list like '%ALL%' then ar.action_type_id
				when @report_type_list like '%SCAR%' then 3
				when @report_type_list like '%UN%' then 4
				when @report_type_list like '%LL%' and @report_type_list not like '%ALL%' then 5
				when @report_type_list like '%INC%' then 6
				when @report_type_list like '%IMP%' then 7
			else 99999 end		
	)
	and (
		isnull(@report_type_list, '') not like '%INC-%'
		OR
			isnull(@report_type_list, '') LIKE '%' + art.action_type_abbr + isnull('-' + arit.incident_type_abbr, '') + '%'
	)
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_action_register] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_action_register] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_action_register] TO [EQAI]
    AS [dbo];

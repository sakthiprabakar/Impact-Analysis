...
SET @receipt_company = CASE WHEN @receipt_company_id < 10 
    ...
    if @trans_source = 'R'
BEGIN
	SET @sql = 'INSERT #tmp SELECT DISTINCT '
		+ ' Receipt.company_id, '
		...
		+ ' CASE IsNull(Receipt.submitted_flag,''F'') WHEN ''T'' THEN ''Submitted'' ELSE '
...
BEGIN
	SET @sql = 'INSERT #tmp SELECT DISTINCT '
		...
		+ ' CASE IsNull(WorkorderHeader.submitted_flag,''F'') WHEN ''T'' THEN ''Submitted'' ELSE '
...
select #tmp.*, Customer.cust_name, Generator.epa_id, Generator.generator_name, BillingLink.link_desc, @billing_status as billing_status from #tmp
JOIN Customer

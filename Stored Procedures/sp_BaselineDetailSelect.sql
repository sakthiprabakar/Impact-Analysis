CREATE PROCEDURE [dbo].[sp_BaselineDetailSelect] 
...
SELECT	   BaselineDetail.[baseline_id],
    ...
    WHERE  ( [baseline_id] = @baseline_id
             AND BaselineDetail.baseline_category_id = @baseline_category_id
             AND BaselineDetail.generator_id = @generator_id
             AND BaselineDetail.reporting_type_id = @reporting_type_id
             AND BaselineDetail.bill_unit_code = @bill_unit_code)
            OR ( @baseline_id IS NULL
                 AND @baseline_category_id IS NULL
                 AND @generator_id IS NULL
                 AND @bill_unit_code IS NULL
                 AND @reporting_type_id IS NULL) 
            AND BaselineDetail.status ='A'

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_BaselineDetailSelect] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_BaselineDetailSelect] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_BaselineDetailSelect] TO [EQAI]
    AS [dbo];

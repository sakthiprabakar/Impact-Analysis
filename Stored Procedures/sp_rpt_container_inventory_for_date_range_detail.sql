...
CREATE PROCEDURE sp_rpt_container_inventory_for_date_range_detail
	@start_date datetime,
	@end_date datetime,
...
Examples: 
exec sp_rpt_container_inventory_for_date_range_detail '12/01/2009', '12/15/2009', '22|0'
exec sp_rpt_container_inventory_for_date_range_detail '12/01/2009', '12/15/2009', '2|21'
...
declare @debug_receipt_id int = NULL -- 7924 -- 719838 -- 735692 -- 103844 --102808 -- 7643 --736698 --735692 --NULL -- 736576 --NULL -- 736411 -- 736411 -- 82829 -- if this is specified, it will filter by this receipt_id (105974)
...
declare @debug_only_container_type varchar(10) = NULL -- 'S', 'R' if specified, will filter out for this container type
...
WHERE 
	...
	AND ContainerDestination.container_id = COALESCE(@debug_container_id, ContainerDestination.container_id)
	--AND Receipt.trans_type = COALESCE(@debug_only_transfer_type, Receipt.trans_type)	
...
--if @debug_only_transfer_type IS NOT NULL
--BEGIN
--	--SELECT @debug_only_transfer_type
--	--DELETE FROM #container_rows WHERE transfer_type <> @debug_only_transfer_type
--END	
...
ALTER TABLE #reporting_rows
	ADD CONSTRAINT PK_Reporting_Rows_cui
...
--SELECT 'rpt' as rpt, * FROM #reporting_rows
--RETURN
...
SELECT DISTINCT 
	...
	CAST(NULL as decimal(12,4)) as total_item_count, /* filled in later */
	CAST(NULL as decimal(12,4)) as containers_on_site, /* filled in later */
...
INTO #converted_rows
FROM #reporting_rows reporting_rows
INNER JOIN Container ON
	container.company_id = reporting_rows.company_id            
	AND container.profit_ctr_id = reporting_rows.profit_ctr_id
	AND container.receipt_id = reporting_rows.receipt_id
...
INNER JOIN ContainerDestination ON 
	container.company_id = ContainerDestination.company_id            
	AND container.profit_ctr_id = ContainerDestination.profit_ctr_id
	AND container.receipt_id = ContainerDestination.receipt_id
	AND container.container_id = ContainerDestination.container_id            	
	AND container.line_id = ContainerDestination.line_id
	AND container.container_type = 'R'
INNER JOIN ReceiptPrice ON 
    ...
    --AND container.price_id = ReceiptPrice.price_id
...
LEFT OUTER JOIN dbo.WasteCode w ON
        Receipt.waste_code_uid = w.waste_code_uid
    
WHERE 
	Receipt.company_id = @company_id 
	AND Receipt.profit_ctr_id = @profit_ctr_id
	AND ContainerDestination.status NOT IN ('V','R')
	AND Receipt.trans_type = 'D' -- Disposal trans type
...
--    --AND container.price_id = ReceiptPrice.price_id
INNER JOIN Receipt ON
	Receipt.receipt_id = reporting_rows.receipt_id
	AND Receipt.line_id = reporting_rows.line_id
	AND Receipt.company_id = reporting_rows.company_id
    AND Receipt.profit_ctr_id = reporting_rows.profit_ctr_id
LEFT OUTER JOIN dbo.WasteCode w ON
        Receipt.waste_code_uid = w.waste_code_uid
WHERE 
	Receipt.company_id = @company_id 
	AND Receipt.profit_ctr_id = @profit_ctr_id
	AND ContainerDestination.status NOT IN ('V','R')
	AND Receipt.trans_type = 'X' -- Disposal trans type	
...
SELECT DISTINCT	ContainerDestination.company_id,
	ContainerDestination.profit_ctr_id,
	ContainerDestination.receipt_id, 
	ContainerDestination.container_id,
	ContainerDestination.line_id,
	...
	ContainerDestination.container_type,
		...
		CAST(NULL as decimal(12,4)) as containers_on_site, /* filled in later */
		'' as bill_unit_code,
		--cast(CONVERT(varchar(20), ContainerDestination.date_added, 101) as datetime) as receipt_date,
		'calc_receipt_date' = (
				--SELECT ISNULL(MIN(b.disposal_date), '1/1/2999') FROM ContainerDestination b
				SELECT MIN(b.disposal_date) FROM ContainerDestination b
				WHERE b.base_container_id = ContainerDestination.container_id
...
/*
	Because of all the wacky left outer joins it will grab records that it should not (i.e. containers that have been disposed, etc...)
	We need to trim these records
*/

if @debug > 5
begin
	SELECT 'before #converted_rows prune', * FROM #converted_rows
end

DELETE #converted_rows FROM #converted_rows
	INNER JOIN (
			SELECT company_id,
				   profit_ctr_id,
				   receipt_id,
	...
	#converted_rows.company_id = tbl_extra.company_id
	AND #converted_rows.profit_ctr_id = tbl_extra.profit_ctr_id
	AND #converted_rows.receipt_id = tbl_extra.receipt_id
	AND #converted_rows.container_id = tbl_extra.container_id
	AND #converted_rows.line_id = tbl_extra.line_id
	AND #converted_rows.sequence_id = tbl_extra.sequence_id

--DELETE FROM #converted_rows WHERE container_percent = 0

if @debug > 5
begin
	SELECT 'after #converted_rows prune', * FROM #converted_rows
end	
...
--DELETE FROM #converted_rows WHERE
--	disposal_date > @end_date
	
--DELETE FROM #converted_rows WHERE
--	disposal_date > CAST(CONVERT(varchar(20), dateadd(dd,2,getdate()),101) as datetime)
...
begin
	...
	if @debug > 5
	begin
		SELECT 'after #container_rows records added' as [after #container_rows records added], * FROM #container_rows
		SELECT 'after #converted_rows records added' as [after #converted_rows records added], * FROM #converted_rows
	end
end	
...
/* 
	...
	we need this to know what the overall weights / container counts if we're to calculate the real average weight 
*/
...
--SELECT '#container_keys - 4', * FROM #container_keys
...
--,conversion_weight_source = 'zero weight'
...
--,conversion_weight_source = 'non zero weight'
	WHERE isnull(container_weight,0) <> 0
...
BEGIN
	...
	SELECT 
		*,
		converted_container_weight / 55.0 as fifty_five_gallon_converted_container_weight
	FROM #converted_rows 
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_container_inventory_for_date_range_detail] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_container_inventory_for_date_range_detail] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_container_inventory_for_date_range_detail] TO [EQAI]
    AS [dbo];

CREATE PROCEDURE sp_rpt_container_inbound_by_approval
	@company_id		int
,	@profit_ctr_id 	int
,	@date_from 		datetime
,	@date_to 		datetime
...
/***************************************************************************************
01/25/06 MK	Copied and modified from sp_rpt_container_inbound
10/29/10 SK added Company_id as input arg, added joins to company_id
			Moved to Plt_AI
11/16/2015 AM - Don't include void receipts to result set.

sp_rpt_container_inbound_by_approval 14, 4, '12/1/2005', '12/31/2005', 1, 999999, 'SWT', 'ALL'
sp_rpt_container_inbound_by_approval 21, 0, '2/27/2014', '2/27/2014', 50013, 50013, 'ALL', 'ALL'
****************************************************************************************/
...
ORDER BY approval_code, container_size

GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_container_inbound_by_approval] TO [EQAI]
    AS [dbo];

-- drop proc if exists sp_COR_Costco_Intelex_Waste_Profile_list
go
create proc sp_COR_Costco_Intelex_Waste_Profile_list
...
/*
sp_COR_Costco_Intelex_Waste_Profile_list

exec sp_COR_Costco_Intelex_Waste_Profile_list @web_userid = 'use@costco.com'
...
exec sp_COR_Costco_Intelex_Waste_Profile_list	@web_userid = 'svc_cvs_intelex'
...
declare 
	...
	values ('(' + convert(varchar(10), @@spid) + ') exec sp_COR_Costco_Intelex_Waste_Profile_list ''' + @i_web_userid + ''', ''' + convert(varchar(40), @start_date, 121) + ''', ''' + convert(varchar(40), @end_date, 121) + '''')
...
select distinct
	...
	*/
	...
	, [Name] = convert(varchar(255), ltrim(rtrim(replace(replace(replace(p.approval_desc, pqa.approval_code + ' - ', ''), pqa.approval_code + '- ', ''), pqa.approval_code + '-', ''))))
	...
	*/
	...
	, [DOT Description] = 
			dbo.fn_manifest_dot_description('P', p.profile_id) 
...
select distinct
	...
	*/
	...
	, [Name] = isnull(t.waste_desc, t.dot_shipping_name)
	...
	*/
	...
	, [DOT Description] = 
			dbo.fn_manifest_dot_description('T', t.tsdf_approval_id) 
...
select distinct
	...
	*/
	...
	, [Name] = ltrim(rtrim(replace(replace(replace(p.approval_desc, pqa.approval_code + ' - ', ''), pqa.approval_code + '- ', ''), pqa.approval_code + '-', '')))
...
Now, if you weren't capturing the data, we actually do want it returning...
*/


if object_id('tempdb..#profiles') is null begin

truncate table plt_export.dbo.work_Intelex_Costco_Profile_list 

insert plt_export.dbo.work_Intelex_Costco_Profile_list 
(
	[Profile Number]			,	
	[Name]						,	
	[DOT Description]				
	...
	[DOT Description]
	FROM    #output
	ORDER BY [Profile Number], [Name]

	insert plt_export..work_Intelex_Costco_Log (log_message)
	values ('(' + convert(varchar(10), @@spid) + ') sp_COR_Costco_Intelex_Waste_Profile_list populated plt_export.dbo.work_Intelex_Costco_Profile_list with ' + convert(varchar(10), @@rowcount) + ' rows and finished')
...
grant execute on sp_COR_Costco_Intelex_Waste_Profile_list
to cor_user
go

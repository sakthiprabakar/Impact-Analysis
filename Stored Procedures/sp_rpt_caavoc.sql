CREATE PROCEDURE sp_rpt_caavoc
 	@company_id		int
,	@profit_ctr_id	int
,	@date_from		datetime
,	@date_to		datetime
...
02/15/2015 AM   This is new sp created. This is replica of sp_rpt_ccvoc, but get value of calculate CAAVOC.

sp_rpt_caavoc 21, 0, '1-1-2005', '12-31-2005', 1, 999999
sp_rpt_caavoc 21, 0, '10-01-2010', '10-03-2010', 0, 999999

sp_rpt_caavoc 21, 0, '01-14-2014', '01-14-2014', 0, 999999
****************************************************************************************/
...
--	AND ((r.caavoc > 0 AND r.caavoc IS NOT NULL) OR @rpt_zero_flag = 'T')
...
SELECT
	...
	,	cd.disposal_date
	,	0 AS processed_flag
	,   Round (( dbo.fn_calculate_constituents_caavoc (@company_id,@profit_ctr_id,cd.receipt_id,cd.line_id)/1000000),8) as CAAVOC
	FROM ContainerDestination cd
	JOIN ProcessLocation pl
		ON pl.company_id = cd.company_id
		AND pl.profit_ctr_id = cd.profit_ctr_id
...
JOIN #tmp_receipts tr
	...
	AND ((tr.caavoc > 0 AND tr.caavoc IS NOT NULL) OR @rpt_zero_flag = 'T')
...
AND (( CAAVOC > 0 AND CAAVOC IS NOT NULL) OR @rpt_zero_flag = 'T')
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_caavoc] TO [EQAI]
    AS [dbo];

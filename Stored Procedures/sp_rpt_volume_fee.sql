CREATE OR ALTER PROCEDURE [dbo].[sp_rpt_volume_fee] (      
 @company_id   int      
, @profit_ctr_id  int      
, @date_from  datetime      
, @date_to  datetime      
...
03/27/2025 - Added  ax_customer_id from Customer Table to show 365 Customer number for this User Story US146442    
	...
	SELECT       
	 ...
	 f_codes =  CASE when ( select count(*) from ReceiptWasteCode rwc       
	  ...
	  where rwc.company_id = r.company_id  and rwc.profit_ctr_id = r.profit_ctr_id and rwc.receipt_id = r.receipt_id and rwc.line_id = r.line_id       
		 ...
		 ROUND( CONVERT(DECIMAL(26,8),ROUND(ROUND(dbo.fn_receipt_weight_container_hi_prec(      
		  cd.receipt_id,       
		  cd.line_id,       
		  cd.profit_ctr_id,       
		  cd.company_id,       
		  cd.container_id,       
		  cd.sequence_id),8) / 2000, 8)) * tax_rate, 8) AS Fee_Amount,
	c.ax_customer_id 
...
GRANT EXECUTE ON [dbo].[sp_rpt_volume_fee] TO EQAI
GO
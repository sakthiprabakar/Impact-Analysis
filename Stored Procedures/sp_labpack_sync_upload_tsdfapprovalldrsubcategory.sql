create procedure [dbo].[sp_labpack_sync_upload_tsdfapprovalldrsubcategory]
	@trip_sync_upload_id	int,
	@TSDF_approval_id		int,
	@ldr_subcategory_id		int
as
/***************************************************************************************
 this procedure records a connection from lab pack field device to upload tsdf approval ldr subcategory
...
set @sql = 'insert TSDFApprovalLDRSubcategory ('
+ 'tsdf_approval_id'
+ ', ldr_subcategory_id'
+ ', added_by'
+ ', date_added'
...
begin
	select @msg = '   DB Error ' + convert(varchar(10),@err) +
			' when inserting TripSyncUploadSQL record for TSDFApprovalLDRSubcategory'
	goto ON_ERROR
end

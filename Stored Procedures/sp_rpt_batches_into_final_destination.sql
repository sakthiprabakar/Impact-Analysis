...
CREATE OR ALTER PROCEDURE [dbo].[sp_rpt_batches_into_final_destination]
	@company_id							int
,	@profit_ctr_id						int
...
,	@final_destination					Char(3)  --start Code included for US146452 
,	@date_disposal_from					datetime = null
,	@date_disposal_to					datetime = null  --End Code included for US146452 
,	@debug								int = null
AS
/***************************************************************************************
...
exec sp_rpt_batches_into_final_destination 21, 0, 'martha_m', '101','106', null, null, null, null, null, null, 
4/7/2025 Included final_destination and Disposal dates are input parameters for US146452
6/13/2025 MK US151878 - Added logic to insert records into work table for Final Destination Export
****************************************************************************************/
BEGIN
	DECLARE @no_date_opened char(1),
			@no_date_closed char(1),
			@no_date_disposal char(1),
			@location		varchar(15),
	...
	--start Code included for US146452
	IF @date_disposal_from IS NULL AND @date_disposal_to IS NULL OR @date_disposal_from = '1-1-1900' AND @date_disposal_to = '1-1-3000'
		SET @no_date_disposal = 'T'
	
	IF @date_disposal_from IS NULL
		SET @date_disposal_from = '1-1-1900'

	IF @date_disposal_to IS NULL
		SET @date_disposal_to = '1-1-3000'
	--End Code included for US146452
	...
	SELECT DISTINCT 
	 ...
	 WHERE be.event_type = 'T'
		...
		AND (@final_destination = 'All' or pl.final_destination_flag = @final_destination) -- Code modified for US146452
		ORDER BY b.location
			, b.tracking_num
			, be.cycle
	...
	-- Delete records for the report user ID from the temp table for Final Destination Report
	DELETE FROM dbo.work_batchFinalDestination
	WHERE user_code = @user_code

	-- Insert into the temp table for Final Destination Report
	INSERT INTO
	dbo.work_batchFinalDestination
	(company_id, profit_ctr_id, batch_location, tracking_num, cycle, event_date, user_code)
	SELECT company_id, profit_ctr_id, location, tracking_num, cycle, transfer_out_date, @user_code
	FROM #tmp_batch
...
GRANT EXECUTE ON [dbo].[sp_rpt_batches_into_final_destination] TO EQAI
GO

...
CREATE PROCEDURE sp_rpt_territory_and_service_category_customer
	@date_from		datetime
,	@date_to		datetime
...
----------------------------------------------------------------------------------------------------------------------
/* 
Created new Detail Report by service category and customer. This pulls all the data from Territory Work Calculation and then
joins to service category to identify the revenue type and groups by per receipt per customer
Gemini: 

01/09/2015   SK	Created
...
-- NEW:
	EXECUTE dbo.sp_rpt_territory_and_service_category_customer
	  @date_from = '12-1-2014',
	  @date_to = '12-31-2014',
	  @copc_list='ALL',
...
SET total_disposal = IsNull((SELECT SUM(ISNULL(extended_amt, 0.00)) FROM #TerritoryWork TW
						...
						AND TW.trans_source = ASE.trans_source
						AND TW.company_id = ASE.company_id
						AND TW.profit_ctr_id = ASE.profit_ctr_id
						AND TW.receipt_id = ASE.receipt_id
...
order by dist_company_id, territory_code, trans_source, company_id, profit_ctr_id, receipt_id, job_type, category
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_territory_and_service_category_customer] TO [EQAI]
    AS [dbo];

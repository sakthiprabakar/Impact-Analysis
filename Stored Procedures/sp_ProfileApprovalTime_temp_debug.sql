...
CREATE PROCEDURE [sp_ProfileApprovalTime_temp_debug] (  
    @iDebugOptions int,                             -- 0: no debug, 1: display SQL only, 2: display and execute SQL    
    @customer_id_list   varchar(8000) = '',         -- numeric customer id(s) (e.g., '100','1110,2002,3987')  
 @department_id_list   varchar(8000) = '',       -- numeric department(s) (e.g., '1','1,2,3,4')  
    ...
    @generator_id_list    varchar(8000) = '',       --   
    @vcReportType varchar(30) = '',                 -- how to select (2) report types.  
    @profile_id_list    varchar(8000) = '',  
 ...
 @status_list    varchar(8000) = '',             -- string status(s)   
    @treatment_id_list    varchar(8000) = '',  
    @user_code_list   varchar(8000) = '',           -- string user(s)  
...
/* ======================================================  
 ...
 Example    :  
      sp_ProfileApprovalTime 0, '', '', '1/1/2006', '12/1/2008', 0, 32, '14|9', '', 'Facility', '', '', '', '', '', '', '', '' --  
...
====================================================== */  
 ...
 DECLARE   
  ...
  @vcSQL varchar(8000),  
    @vcInsertTable varchar(200),  
    @vcSelect varchar(4400),  
    @vcSelect_Common varchar(400),  
    @vcJoin varchar(900),  
    @vcWhere varchar(2900),  
    @vcGroupBy varchar(2600),  
    @vcR1SelectList varchar(500),  
    @vcWhere_Drilldown varchar(300),  
    @vcGroupBy_Drilldown varchar(100),  
    @vcOrderBy_Drilldown varchar(100),  
    @vcField1 varchar(50),  
    @vcField2 varchar(50),  
    @vcMinHoursSumField varchar(30),  
    @vcWhere_SessionKey varchar(300),  --See Explanation (above)   
    @vcCompanyId varchar (2),       
    @vcProfitCtrId varchar (2),     
    @vcCompanyProfitCtr_Select varchar (200),  
  
    @vcWhere_Rule1 varchar(200),  
    @vcWhere_Rule2 varchar(200),  
    @vcWhere_Rule3 varchar(200),  
    @vcWhere_Rule4 varchar(200),  
    @vcWhere_Rule5 varchar(200),  
    @vcWhere_Rule6 varchar(200),  
    @vcWhere_Rule7 varchar(200),  
    @vcWhere_Rule7_1 varchar(400),  
    @vcWhere_Rules1Thru7 varchar(1500),  
    @vcWhere_UserEntered varchar(1500),  
    @vcWhere_SubQuery varchar(4000),  
    @vcJoin_SubQuery varchar(900),  
    @vcWhere_DeleteFromProfile varchar(200),  
 ...
 CREATE TABLE #treatment (treatment_id int)  
 CREATE TABLE #users (user_code varchar(10))  
 CREATE TABLE #wastecode (waste_code varchar(4))  
 ...
 INSERT #treatment SELECT Convert(int, row) from dbo.fn_SplitXsvText(',', 1, @treatment_id_list) WHERE IsNull(row, '') <> ''      
 ...
 SET @vcWhere_Rule7_1 = '  
          /* Rule:7.1 */ (SELECT IsNull(time_out,''1/1/1900'') FROM profiletracking WHERE profile_id = pt.profile_id AND tracking_id = (IsNull((SELECT Min(tracking_id) FROM profiletracking WHERE profile_id = pt.profile_id AND tracking_status = ''COMP''),-
1))) '    
 ...
 IF IsNull(@treatment_id_list, '') <> ''      
  SET @vcWhere_UserEntered = @vcWhere_UserEntered + '   
              AND p.profile_id IN (SELECT profile_id FROM profiletreatment ptr INNER JOIN #treatment tt on ptr.treatment_id = tt.treatment_id) '     
       
 IF IsNull(@waste_code_list, '') <> ''      
  SET @vcWhere_UserEntered = @vcWhere_UserEntered + '   
              AND p.profile_id IN (SELECT profile_id FROM profilewastecode pwc INNER JOIN #wastecode tw on pwc.waste_code = tw.waste_code) '     
    ...
    BEGIN  
     ...
     SET @vcSQL = '  
      ...
      IF @iDebugOptions IN (1,2)   
        BEGIN  
          PRINT '-------------------------------------------------------'    
          PRINT '------ 3.1.B. Applies to (3) - Set Row Ordinals -------'              PRINT '------ ' + @vcInsertTable + ' ------------'    
          PRINT '-------------------------------------------------------'    
          PRINT ''    

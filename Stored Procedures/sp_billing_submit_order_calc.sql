...
CREATE PROCEDURE sp_billing_submit_order_calc
	@debug			int,
	@trans_source	char(1),
	@order_id		int,
...
03/16/2015 JPB	Copy of sp_billing_submit_order made to take a flag that indicates whether live tables should be updated.
				Also assumes an input #Billing & #BillingDetail etc exist, and returns those tables with populated data.
04/14/2017 JPB	Added column lists for inserts to #BillingDetail, #BillingComment tables because changes in other SPs to those
				tables have changed their shape and made inserts without column lists fail.
02/15/2018 MPM	Added currency_code column to #Billing and #BillingDetail.

sp_helptext sp_billing_submit_order_calc
EXEC sp_billing_unsubmit_order 1, 5069, 'O', 'JASON_B'
EXEC sp_billing_submit_order_calc 1, 'O', 6595, '07-31-2013 15:30', NULL, 'SMITA_K'
****************************************************************************************/
SET NOCOUNT ON
...
if @update_prod = 'T' BEGIN
	...
	IF @order_count > 0 AND @invalid_gl_count = 0
	BEGIN	
...
IF @sync_invoice_jde = 1 AND @invalid_gl_count > 0 and @update_prod = 'T'
BEGIN
	INSERT INTO OrderAudit (order_id, line_id, sequence_id, table_name, column_name, before_value, after_value, audit_reference, modified_by, modified_from, date_modified)
		SELECT DISTINCT bd.receipt_id, bd.line_id, NULL, '', '', bd.billing_type AS before_value, bd.JDE_BU + '-' + bd.JDE_object AS after_value, 'Submit to Billing Failed.  Invalid JDE GL Account' AS audit_reference, @user_code, 'BS SP', @submit_date
		FROM #BillingDetail bd
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_billing_submit_order_calc] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_billing_submit_order_calc] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_billing_submit_order_calc] TO [EQAI]
    AS [dbo];

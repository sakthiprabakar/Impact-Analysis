CREATE PROCEDURE [dbo].[sp_reports_manifest_lookup]
...
11/22/2017	JPB	Modified to find all instances of a manifest: receipt or work order, not avoiding duplicates or only returning "last" anymore				
...
SELECT TOP 10 * FROM scandocumenttype WHERE document_type like '%manifest%'

SELECT TOP 100 * FROM workordermanifest w (nolock)
INNER JOIN plt_image..scan s (nolock) on w.workorder_id = s.receipt_id and w.company_id = s.company_id and w.profit_ctr_id = s.profit_ctr_id and type_id in (1, 4, 28, 75, 81, 99) and document_source = 'workorder'
WHERE 1=1 and w.manifest not like 'manifest%'
and exists (select 1 FROM receipt (nolock) WHERE manifest = w.manifest and waste_accepted_flag = 'T' and trans_mode = 'I')
 ORDER BY w.date_added desc

SELECT * FROM receipt WHERE manifest = '001092679FLE'
SELECT * FROM workordermanifest WHERE manifest = '001092679FLE'
SELECT * FROM plt_image..scan WHERE manifest = '001092679FLE'
SELECT * FROM plt_image..scan WHERE receipt_id = 18300 and profit_ctr_id = 0

	exec sp_reports_manifest_lookup 0, '011639404JJK', '', '', '', '', '0'
	-- 12039 rows, 1:34 -- no indexes.

SELECT * FROM plt_image..scan WHERE image_id in (10451849, 10451850)
SELECT * FROM plt_image..scanimage WHERE image_id in (10451849, 10451850)

select top 1000 m.manifest, s.date_added, s2.date_added, s.* 
-- select count(s.manifest) -- TOTAL: 1186797  Initial after TSDF: 939
from plt_image..scan s (nolock) 
join workordermanifest m (nolock) on s.workorder_id = m.workorder_id and s.company_id = m.company_id and s.profit_ctr_id = m.profit_ctr_id and left(m.manifest, 8) <>  'manifest'
join receipt r (nolock) on m.manifest = r.manifest
join plt_image..scan s2 (nolock) on r.receipt_id = s2.receipt_id and r.company_id = s2.company_id and r.profit_ctr_id = s2.profit_ctr_id and s2.manifest = r.manifest and s2.page_number =3
where s.type_id = 81 and s.document_name like '%JJK.pdf'
and s.date_added > s2.date_added


select * from plt_image..scan (nolock) where type_id = 81 and document_name like '%011639404JJK.pdf'
SELECT * FROM workordermanifest WHERE  workorder_id = 2165500 and company_id = 25
SELECT * FROM workorderheader WHERE  workorder_id = 2165500 and company_id = 25

SELECT * FROM receipt WHERE manifest = '011639404JJK'
...
-- Setup is finished.  On to work:
 ...
 IF @debug >= 1 select @sql sql	
	...
	if @debug > 3 select '#source' as table_name, * from #source
	...
	select top 0 *, convert(int, null) as image_id into #scan from #source 
	...
	and (
		isnull(s.manifest, '') like '%' + src.manifest + '%'
		or
		isnull(s.document_name, '') like '%' + src.manifest + '%'
	)
   ...
   if @debug >= 1 select datediff(ms, @timer_start, getdate()) as total_elapsed_time, datediff(ms, @last_step, getdate()) as last_step_time, 'Receipt #scan-ing' as last_step_desc
	...
	if exists (select 1 from #source where document_source = 'receipt')
		...
		inner join plt_image..ScanDocumentType sdt with (nolock)
...
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_manifest_lookup] TO [EQAI]
    AS [dbo];
GO

GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_manifest_lookup] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_reports_manifest_lookup] TO [COR_USER]
    AS [dbo];

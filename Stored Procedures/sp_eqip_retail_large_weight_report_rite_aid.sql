...
create proc sp_eqip_retail_large_weight_report_rite_aid (
...
/* *************************************************************************
sp_eqip_retail_large_weight_report_rite_aid

Calculates weights and waste categories according to RiteAid's rules and then
lists the full pickup data for any pickup where the total weight exceeds @weight_limit

History:

	9/16/2013	JPB	Created
	04/02/2014	JPB	Revised for better speed.
	4/17/2014	JPB	Fixes for data doubling in #summary table, and incorrect weights in Work Order transactions
	8/21/2014	JPB	Fix for #detail table weight calculation

Sample:

	sp_eqip_retail_large_weight_report_rite_aid 120, '4/6/2014', '4/12/2014', 'jonathan', 159

************************************************************************* */

-- declare 	@weight_limit	float = 120,	@start_date		datetime = '12/1/2013',	@end_date		datetime = '12/31/2013',	@user_code		varchar(20) = 'jonathan',	@permission_id	int = 284,	@debug_code		int = 0	
...
-- drop table #summary
	...
	, convert(float, null) as sum_residue_weight
	...
	update #summary set
		sum_container_count = (select isnull(sum(container_count), 0) from #detail d2 where d2.site_code = s.site_code and d2.category = s.category and d2.pickup_date = s.pickup_date and d2.trans_source = 'R')
		, sum_residue_weight = (select isnull(sum(residue_weight), 0) from #detail d2 where d2.site_code = s.site_code and d2.category = s.category and d2.pickup_date = s.pickup_date and d2.trans_source = 'R')
		, sum_detail_weight = (select isnull(sum(detail_weight), 0) from #detail d2 where d2.site_code = s.site_code and d2.category = s.category and d2.pickup_date = s.pickup_date and d2.trans_source = 'R')
	from #summary s
where sum_detail_weight is null

	-- Update #summary from #detail to sum the weights per site_code & category combination.
	update #summary set
		sum_container_count = (select isnull(sum(container_count), 0) from #detail d2 where d2.site_code = s.site_code and d2.category = s.category and d2.pickup_date = s.pickup_date and d2.trans_source = 'W')
		, sum_residue_weight = (select isnull(sum(residue_weight), 0) from #detail d2 where d2.site_code = s.site_code and d2.category = s.category and d2.pickup_date = s.pickup_date and d2.trans_source = 'W')
		, sum_detail_weight = (select isnull(sum(detail_weight), 0) from #detail d2 where d2.site_code = s.site_code and d2.category = s.category and d2.pickup_date = s.pickup_date and d2.trans_source = 'W')
	from #summary s
where sum_detail_weight is null
...
/*
	-- Don't need NWP in this query.  Removing for speed.
...
-- declare @weight_limit float = 120
-- drop table #heavies
select 
	d.generator_id
	, d.pickup_date 
...
group by
	...
	, d.pickup_date 
having
	 sum(case when residue_weight > 0 then residue_weight else detail_weight end) > @weight_limit
...
select 
	...
	, p.RX_total_weight
	, p.FRONT_END_total_weight + p.RX_total_weight as total_weight
from #prebilldata p
inner join #heavies h on p.generator_id = h.generator_id
	and p.pickup_date = h.pickup_date
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_eqip_retail_large_weight_report_rite_aid] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_eqip_retail_large_weight_report_rite_aid] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_eqip_retail_large_weight_report_rite_aid] TO [EQAI]
    AS [dbo];

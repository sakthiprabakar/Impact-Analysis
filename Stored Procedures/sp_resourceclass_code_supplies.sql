...
/****** Object:  StoredProcedure [dbo].[sp_resourceclass_code_supplies]    Script Date: 1/6/2022 8:49:46 AM ******/
SET ANSI_NULLS ON
GO
...
drop PROCEDURE IF EXISTS sp_resourceclass_code_supplies
go
CREATE PROCEDURE [dbo].[sp_resourceclass_code_supplies]
...
the "supplies" button on Work Order. It fetches the codes based on following order:
...
Filename:           F:\EQAI\SQL\EQAI\sp_resourceclass_code_supplies.sql
PB Object(s): d_quick_select_supplies_quote
Load on PLT_AI
             
12/07/2021 AM   DevOps:29685 - New procedure created
...
BEGIN
       ...
       INNER JOIN ResourceClassHeader 
        ...
        WHERE WorkorderQuoteDetail.resource_type = 'S'
...
INNER JOIN ProfitCenter
	ON ProfitCenter.company_id = ResourceClassDetail.company_id
	AND ProfitCenter.profit_ctr_id = ResourceClassDetail.profit_ctr_id
     WHERE ResourceClass.resource_type = 'S'
       AND ResourceClass.status = 'A'
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_resourceclass_code_supplies] TO [EQAI]
    AS [dbo];
GO

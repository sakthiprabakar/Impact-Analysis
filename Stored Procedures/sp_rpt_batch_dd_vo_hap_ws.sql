DROP PROCEDURE IF EXISTS [dbo].[sp_rpt_batch_dd_vo_hap_ws]
GO

CREATE PROCEDURE [dbo].[sp_rpt_batch_dd_vo_hap_ws]
	@company_id		int
,	@profit_ctr_id	int
,	@date_from		datetime
,	@date_to		datetime
...
AS
/***************************************************************************************
Filename:		L:\Apps\SQL-Deploy\Prod\NTSQL1\PLT_AI\Procedures\sp_rpt_batch_dd_vo_hap_ws.sql
PB Object(s):	r_batch_dd_vo_hap_worksheet
...
11/23/2009 JDB	Created; copied from sp_rpt_dd_vo_hap_ws
...
01/26/2022 MPM	DevOps 27246 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).
				Also noticed that VOC and dependent numbers were displaying as empty space on the report instead of "0.0000", so I 
				changed "r.ddvoc" in the statements below with "ISNULL(r.ddvoc, 0)".
06/29/2022 GDE DevOps 42736 - Batch DD VO HAP Worksheet - Report update
08/02/2023 Dipankar - DevOps 50330 - Implemented DBA Observation comments

sp_rpt_batch_dd_vo_hap_ws 14, -1, '10-1-2008', '10-27-2008', 'ALL', 'ALL'
sp_rpt_batch_dd_vo_hap_ws 21, 00, '10-1-2011', '10-27-2011', 'ALL', 'ALL'
****************************************************************************************/
DECLARE	
...
--rb	ISNULL(r.bill_unit_code, 'VAR') AS bill_unit_code,
	...
	ROUND((ISNULL(r.ddvoc, 0)/1000000), 8) AS VOC, 
	r.location, 
...
-- 50330 - Combined two update statements into one
UPDATE #tmp 
SET tons_voc  = ROUND((pounds * VOC * @pound_ton_conversion), 8),
    emissions = ROUND(ROUND((pounds * VOC * @pound_ton_conversion), 8) * emission_factor * (1 - control_efficiency_value), 8)
...
/*	MPM - 1/27/2022 - DevOps 27246 - Modified the calculation of emissions to be the same as in the HAP Report (DevOps 17534).  
...
SELECT @rolling_emissions_nr = SUM(emissions) FROM #tmp WHERE location_report_flag = 'N' AND disposal_date BETWEEN @date_from_12 AND @date_to
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_batch_dd_vo_hap_ws] TO [EQAI]
    AS [dbo];

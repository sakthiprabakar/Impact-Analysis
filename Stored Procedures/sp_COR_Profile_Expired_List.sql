...
--DROP PROCEDURE [dbo].[sp_COR_Profile_Expired_List]
--GO

CREATE  PROCEDURE [dbo].[sp_COR_Profile_Expired_List]
...
/* ****************************************************************
sp_COR_Profile_Expired_List

History:

	10/15/2019	MPM	DevOps 11585: Added logic to filter the result set
...
from (
	...
	join FormDisplayStatus pds 
		on p.display_status_uid = pds.display_status_uid
		and pds.display_status in (
			select row from dbo.fn_SplitXsvText(',', 1, @i_profile_status) where row is not null
			union select display_status from FormDisplayStatus WHERE isnull(@i_profile_status, '') in ('', 'all')
		)
	join Customer cn 
		on p.customer_id = cn.customer_id
	join Generator gn 
		on p.generator_id = gn.generator_id
	left join generatortype gt on gn.generator_type_id = gt.generator_type_id
	where 1=1 AND CONVERT(DATE, p.ap_expiration_date,103) < CONVERT(DATE, GETDATE(),103)
	and 
	(
		isnull(@i_search, '') = ''
		or
...
UPDATE #TMP SET total_profile_list=  ( Select COUNT(total_profile_list) FROM #TMP WHERE  CONVERT(DATE, ap_expiration_date,103) < CONVERT(DATE, GETDATE(),103)
)

--select * from #TMP  
IF (ISNULL(@i_search,'')<>'' OR ISNULL(@i_adv_search,'') <>'')
BEGIN
...
cust_name AS updated_by,ap_expiration_date AS expired_date, copy_source AS profile,
  display_status as status,total_profile_list AS totalcount FROM #TMP 
END
ELSE
BEGIN
SELECT	profile_id, approval_desc AS pro_name,generator_name AS gen_by,generator_type AS RCRA_status,date_modified AS updated_date, 
...
grant execute on sp_COR_Profile_Expired_List to eqweb,COR_USER
go
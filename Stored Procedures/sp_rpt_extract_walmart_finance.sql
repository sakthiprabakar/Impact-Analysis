...
CREATE PROCEDURE sp_rpt_extract_walmart_finance (
	@start_date				datetime,
	@end_date				datetime,
...
/* ***********************************************************
Procedure    : sp_rpt_extract_walmart_finance
Database     : PLT_AI
Created      : Jan 25 2008 - Jonathan Broome
Description  : Creates a Wal-Mart Financial Extract

Examples:
	sp_rpt_extract_walmart_finance '4/1/2009 00:00', '4/30/2009 23:59', 'validation'
	sp_rpt_extract_walmart_finance '7/1/2008 00:00', '7/31/2008 23:59', 'validation'
	sp_rpt_extract_walmart_finance '1/1/2011 00:00', '1/31/2011 23:59', 'wm-extract'
...
History:
   ...
   9/16/2011 - CRG
	 Change  to insert to a "temp" table in EQ_TEMP. For each output result 
		that is returned - the query uses the sp_export_to_text and 
		sp_export_to_excel procedure
	07/08/2024 KS	Rally116985 - Modified item_desc datatype to VARCHAR(100) for #Extract table.
...
CREATE TABLE #Extract (
	...
	item_desc 					varchar(250) NULL,	-- Item Description
...
if object_id('EQ_TEMP..WalmartExtractFinancial1') is not null drop table EQ_TEMP..WalmartExtractFinancial1
if object_id('EQ_TEMP..WalmartExtractFinancial2') is not null drop table EQ_TEMP..WalmartExtractFinancial2
if object_id('EQ_TEMP..WalmartExtractFinancial3') is not null drop table EQ_TEMP..WalmartExtractFinancial3
...
SELECT DISTINCT
	...
	extract_value,
	@usr as usr,
	@extract_datetime as extract_datetime
INTO EQ_TEMP..WalmartExtractFinancial1
FROM #validation
...
-- Walmart Fields:
	...
	site_type_abbr as [Facility Type],
	generator_city as [City],
	generator_state as [State],
	service_date as [Shipment Date],
	epa_id as [Haz Waste Generator EPA ID],
	manifest as [Manifest Number],
	manifest_line as [Manifest Line],
	pounds as [Weight],
	bill_unit_desc as [Container Type],
	quantity as [Container Quantity],
	billing_quantity as [Billing Quantity],
	item_desc as [Item Description],
	waste_desc as [Waste Description],
	approval_or_resource as [Waste Profile Number],
	dot_description as [DOT Description],
	...
	tax as [Tax],
	extended_cost as [Extended Tax]
INTO EQ_TEMP..WalmartExtractFinancial2
FROM #Extract
...
--IF @output_mode = 'eq-extract' BEGIN
	...
	SELECT DISTINCT
		*
	INTO EQ_TEMP..WalmartExtractFinancial3
	FROM #Extract
...
-----------------------------------------------------
--
-- Export to EQ_TEMP tables
--
--
-----------------------------------------------------

declare @tmp_desc varchar(255) = 'Walmart Finance Validation - ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)

/*	Write to Excel: */
   exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'EQ_TEMP..WalmartExtractFinancial1',
   	@template	     = 'sp_rpt_extract_walmart_financial1',
   	@filename	     = NULL,
   	@added_by	     = @usr,
...
set @tmp_desc = 'Walmart Finance Extract - WM Format - ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)
   	
   	exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'EQ_TEMP..WalmartExtractFinancial2',
   	@template	     = 'sp_rpt_extract_walmart_financial2',
   	@filename	     = NULL,
   	@added_by	     = @usr,
   	@export_desc     = @tmp_desc,
   	@report_log_id   = @report_log_id,
...
set @tmp_desc = 'Walmart Finance Extract - EQ Format - ' + convert(varchar(10), @start_date, 110) + ' - ' + convert(varchar(12), @end_date, 110)
   	
   	exec plt_export.dbo.sp_export_to_excel
   	@table_name	     = 'EQ_TEMP..WalmartExtractFinancial3',
   	@template	     = 'sp_rpt_extract_walmart_financial3',
   	@filename	     = NULL,
   	@added_by	     = @usr,
   	@export_desc     = @tmp_desc,
   	@report_log_id   = @report_log_id,
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_extract_walmart_finance] TO [EQWEB]
    AS [dbo];
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_extract_walmart_finance] TO [COR_USER]
    AS [dbo];
...
GO
GRANT EXECUTE
    ON OBJECT::[dbo].[sp_rpt_extract_walmart_finance] TO [EQAI]
    AS [dbo];

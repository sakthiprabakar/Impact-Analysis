...
try:
    ...
    model = genai.GenerativeModel("gemini-2.0-flash")  # Using gemini-1.0-pro
except Exception as e:
    logging.error(f"Error configuring Gemini API: {e}")
    exit()

# Folder where stored procedure SQL files are located
# STORED_PROC_FOLDER = "stored_procedures_CustomerBilling_2"  # Moved inside run_impact_analysis
...
def extract_valid_json(response_text):
    """
    Extracts the first valid JSON block from the Gemini response.
    ...
    """
        ...
        for attempt in range(retries):
            try:
                ...
                logging.info(f"Raw Response from Gemini for {proc_name}:\n{raw_response}")
                ...
                logging.error(f"Unexpected error during Gemini API call for {proc_name}: {e}")
                time.sleep(2)
        else:
